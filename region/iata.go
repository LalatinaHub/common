package region

import "strings"

var iataCode map[string]string = map[string]string{
	"AAF": "APALACHICOLA",
	"AAH": "AACHEN",
	"AAI": "ARRAIAS",
	"AAJ": "AWARADAM",
	"AAK": "ARANUKA",
	"AAL": "AALBORG",
	"AAM": "MALA MALA",
	"AAN": "AL AIN",
	"AAO": "ANACO",
	"AAQ": "ANAPA",
	"AAR": "AARHUS",
	"AAS": "APALAPSILI",
	"AAT": "ALTAY",
	"AAU": "ASAU",
	"AAV": "ALAH",
	"AAW": "ABBOTTABAD",
	"AAX": "ARAXA",
	"AAY": "AL GHAYDAH",
	"ABA": "ABAKAN",
	"ABD": "ABADAN",
	"ABE": "ALLENTOWN",
	"ABF": "ABAIANG",
	"ABG": "ABINGDON",
	"ABH": "ALPHA",
	"ABI": "ABILENE",
	"ABJ": "ABIDJAN",
	"ABK": "KABRI DAR",
	"ABL": "AMBLER",
	"ABM": "BAMAGA",
	"ABN": "ALBINA",
	"ABO": "ABOISSO",
	"ABP": "ATKAMBA",
	"ABQ": "ALBUQUERQUE",
	"ABR": "ABERDEEN",
	"ABS": "ABU SIMBEL",
	"ABT": "AL-BAHA",
	"ABU": "ATAMBUA",
	"ABV": "ABUJA",
	"ABW": "ABAU",
	"ABX": "ALBURY",
	"ABY": "ALBANY",
	"ABZ": "ABERDEEN",
	"ACA": "ACAPULCO",
	"ACB": "BELLAIRE",
	"ACC": "ACCRA",
	"ACD": "ACANDI",
	"ACE": "ARRECIFE",
	"ACH": "ALTENRHEIN",
	"ACI": "ALDERNEY",
	"ACK": "NANTUCKET",
	"ACL": "AGUA CLARA",
	"ACM": "ARICA",
	"ACN": "CIUDAD ACUNA",
	"ACO": "ASCONA",
	"ACR": "ARARACUARA",
	"ACS": "ACHINSK",
	"ACT": "WACO",
	"ACU": "ACHUTUPO",
	"ACV": "EUREKA/ARCATA",
	"ACY": "ATLANTIC CITY",
	"ADA": "ADANA",
	"ADB": "IZMIR-ADNAN MEND",
	"ADD": "ADDIS ABABA",
	"ADE": "ADEN",
	"ADG": "ADRIAN",
	"ADH": "ALDAN",
	"ADI": "ARANDIS",
	"ADJ": "AMMAN-CIVIL",
	"ADK": "ADAK ISLAND",
	"ADL": "ADELAIDE",
	"ADM": "ARDMORE",
	"ADN": "ANDES",
	"ADO": "ANDAMOOKA",
	"ADP": "ANURADHAPURA",
	"ADQ": "KODIAK",
	"ADR": "ANDREWS",
	"ADS": "DALLAS/FT. WORTH",
	"ADT": "ADA",
	"ADU": "ARDABIL",
	"ADV": "ANDOVER",
	"ADW": "ANDREWS AFB",
	"ADX": "ST. ANDREWS",
	"ADY": "ALLDAYS",
	"ADZ": "SAN ANDRES IS.",
	"AEA": "ABEMAMA",
	"AEG": "AEK GODANG",
	"AEH": "ABECHER",
	"AEK": "ASEKI",
	"AEL": "ALBERT LEA",
	"AEO": "AIOUN EL ATROUSS",
	"AEP": "BUENOS AIRES",
	"AER": "ADLER/SOCHI",
	"AES": "AALESUND",
	"AET": "ALLAKAKET",
	"AEX": "ALEXANDRIA",
	"AEY": "AKUREYRI",
	"AFA": "SAN RAFAEL",
	"AFD": "PORT ALFRED",
	"AFI": "AMALFI",
	"AFL": "ALTA FLORESTA",
	"AFN": "JAFFREY",
	"AFO": "AFTON",
	"AFR": "AFORE",
	"AFT": "AFUTARA",
	"AFW": "DALLAS/FT. WORTH",
	"AFY": "AFYON",
	"AGA": "AGADIR",
	"AGB": "AUGSBURG",
	"AGC": "PITTSBURGH",
	"AGD": "ANGGI",
	"AGE": "WANGEROOGE",
	"AGF": "AGEN",
	"AGG": "ANGORAM",
	"AGH": "ANGELHOLM/HELSINGBORG",
	"AGI": "WAGENINGEN",
	"AGJ": "AGUNI",
	"AGK": "KAGUA",
	"AGL": "WANIGELA",
	"AGM": "ANGMAGSSALIK",
	"AGN": "ANGOON",
	"AGO": "MAGNOLIA",
	"AGP": "MALAGA",
	"AGQ": "AGRINION",
	"AGR": "AGRA",
	"AGS": "AUGUSTA",
	"AGT": "CIUDAD PRES. STROESSNER",
	"AGU": "AGUASCALIENTES",
	"AGV": "ACARIGUA",
	"AGW": "AGNEW",
	"AGX": "AGATTI ISLAND",
	"AGY": "ARGYLE DOWNS",
	"AGZ": "AGGENEYS",
	"AHA": "OKINAWA",
	"AHB": "ABHA",
	"AHD": "ARDMORE",
	"AHF": "ARAPAHOE",
	"AHH": "AMERY",
	"AHI": "AMAHAI",
	"AHL": "AISHALTON",
	"AHN": "ATHENS",
	"AHO": "ALGHERO",
	"AHS": "AHUAS",
	"AHT": "AMCHITKA",
	"AHU": "AL HOCEIMA",
	"AHY": "AMBATOLAHY",
	"AHZ": "ALPE D'HUEZ",
	"AIA": "ALLIANCE",
	"AIB": "ANITA BAY",
	"AIC": "AIROK",
	"AID": "ANDERSON",
	"AIE": "AIOME",
	"AIF": "ASSIS",
	"AIG": "YALINGA",
	"AII": "ALI SABIEH",
	"AIK": "AIKEN",
	"AIL": "AILIGANDI",
	"AIM": "AILUK",
	"AIN": "WAINWRIGHT",
	"AIO": "ATLANTIC",
	"AIP": "AILINGLAPALAP",
	"AIR": "ARIPUANA",
	"AIS": "ARORAE",
	"AIT": "AITUTAKI",
	"AIU": "ATIU",
	"AIV": "ALICEVILLE",
	"AIY": "ATLANTIC CITY",
	"AIZ": "LAKE OF THE OZARKS",
	"AJA": "AJACCIO",
	"AJF": "JOUF",
	"AJJ": "AKJOUJT",
	"AJL": "AIJAL",
	"AJN": "ANJOUAN",
	"AJO": "ALJOUF",
	"AJR": "ARVIDSJAUR",
	"AJS": "ABREOJOS",
	"AJU": "ARACAJU",
	"AJY": "AGADES",
	"AKA": "ANKANG",
	"AKB": "ATKA ISLAND",
	"AKC": "AKRON/CANTON",
	"AKD": "AKOLA",
	"AKE": "AKIENI",
	"AKF": "KUFRAH",
	"AKG": "ANGUGANAK",
	"AKI": "AKIAK",
	"AKJ": "ASAHIGAWA",
	"AKK": "AKHIOK",
	"AKL": "AUCKLAND",
	"AKM": "ZAKOUMA",
	"AKN": "KING SALMON",
	"AKO": "AKRON",
	"AKP": "ANAKTUVUK PASS",
	"AKQ": "ASTRAKSETRA",
	"AKR": "AKURE",
	"AKS": "AUKI",
	"AKT": "AKROTIRI",
	"AKU": "AKSU",
	"AKV": "AKULIVIK",
	"AKX": "AKTYUBINSK",
	"AKY": "SITTWE",
	"ALA": "ALMA-ATA",
	"ALB": "ALBANY",
	"ALC": "ALICANTE",
	"ALD": "ALERTA",
	"ALE": "ALPINE",
	"ALF": "ALTA",
	"ALG": "ALGIERS",
	"ALH": "ALBANY",
	"ALI": "ALICE",
	"ALJ": "ALEXANDER BAY",
	"ALK": "ASELA",
	"ALL": "ALBENGA",
	"ALM": "ALAMOGORDO",
	"ALN": "ALTON",
	"ALO": "WATERLOO",
	"ALP": "ALEPPO",
	"ALQ": "ALEGRETE",
	"ALR": "ALEXANDRA",
	"ALS": "ALAMOSA",
	"ALT": "ALENQUER",
	"ALU": "ALULA",
	"ALV": "ANDORRA LA VELLA",
	"ALW": "WALLA WALLA",
	"ALX": "ALEXANDER CITY",
	"ALY": "ALEXANDRIA",
	"ALZ": "ALITAK",
	"AMA": "AMARILLO",
	"AMB": "AMBILOBE",
	"AMC": "AM TIMAN",
	"AMD": "AHMEDABAD",
	"AME": "ALTO MOLOCUE",
	"AMF": "AMA",
	"AMG": "AMBOIN",
	"AMH": "ARBA MINTCH",
	"AMI": "AMPENAN",
	"AMJ": "ALMENARA",
	"AMK": "DURANGO",
	"AML": "PUERTO ARMUELLES",
	"AMM": "AMMAN",
	"AMN": "ALMA",
	"AMO": "MAO",
	"AMP": "AMPANIHY",
	"AMQ": "AMBON",
	"AMR": "ARNO",
	"AMS": "AMSTERDAM",
	"AMT": "AMATA",
	"AMU": "AMANAB",
	"AMV": "AMDERMA",
	"AMW": "AMES",
	"AMX": "AMMAROO",
	"AMY": "AMBATOMAINTY",
	"AMZ": "ARDMORE",
	"ana": "ANAA",
	"ANB": "ANNISTON",
	"ANC": "ANCHORAGE",
	"AND": "ANDERSON",
	"ANE": "ANGERS",
	"ANF": "ANTOFAGASTA",
	"ANG": "ANGOULEME",
	"ANH": "ANUHA ISLAND RESORT",
	"ANI": "ANIAK",
	"ANJ": "ZANAGA",
	"ANL": "ANDULO",
	"ANM": "ANTALAHA",
	"ANN": "ANNETTE ISLAND",
	"ANO": "ANGOCHE",
	"ANP": "ANNAPOLIS",
	"ANQ": "ANGOLA",
	"ANR": "ANTWERP",
	"ANS": "ANDAHUAYLAS",
	"ANU": "ANTIGUA",
	"ANV": "ANVIK",
	"ANW": "AINSWORTH",
	"ANX": "ANDENES",
	"ANY": "ANTHONY",
	"ANZ": "ANGAS DOWNS",
	"AOA": "AROA",
	"AOB": "ANNANBERG",
	"AOC": "ALTENBURG",
	"AOD": "ABOU DEIA",
	"AOG": "ANSHAN",
	"AOH": "LIMA",
	"AOI": "ANCONA",
	"AOJ": "AOMORI",
	"AOK": "KARPATHOS",
	"AOL": "PASO DE LOS LIBRES",
	"AON": "ARONA",
	"AOO": "ALTOONA",
	"AOR": "ALOR STAR",
	"AOS": "AMOOK",
	"AOU": "ATTOPEU",
	"APA": "DENVER",
	"APB": "APOLO",
	"APC": "NAPA",
	"APE": "SAN JUAN APOSENTO",
	"APF": "NAPLES",
	"APG": "ABERDEEN",
	"APH": "BOWLING GREEN",
	"API": "APIAY",
	"APK": "APATAKI",
	"APL": "NAMPULA",
	"APN": "ALPENA",
	"APO": "APARTADO",
	"APP": "ASAPA",
	"APQ": "ARAPIRACA",
	"APR": "APRIL RIVER",
	"APS": "ANAPOLIS",
	"APT": "JASPER",
	"APU": "APUCARANA",
	"APV": "APPLE VALLEY",
	"APW": "APIA",
	"APX": "ARAPONGAS",
	"APY": "ALTO PARNAIBA",
	"APZ": "ZAPALA",
	"AQA": "ARARAQUARA",
	"AQG": "ANQING",
	"AQI": "QAISUMAH",
	"AQJ": "AQABA",
	"AQM": "ARIQUEMES",
	"AQP": "AREQUIPA",
	"AQS": "SAQANI",
	"AQY": "ALYESKA",
	"ARA": "NEW IBERIA",
	"ARB": "ANN ARBOR",
	"ARC": "ARCTIC VILLAGE",
	"ARD": "ALOR",
	"ARE": "ARECIBO",
	"ARF": "ACARICUARA",
	"ARG": "WALNUT RIDGE",
	"ARH": "ARKHANGEL'SK",
	"ARI": "ARICA",
	"ARJ": "ARSO",
	"ARK": "ARUSHA",
	"ARL": "ARLY",
	"ARM": "ARMIDALE",
	"ARN": "STOCKHOLM",
	"ARO": "ARBOLETAS",
	"ARP": "ARAGIP",
	"ARQ": "ARAUQUITA",
	"ARR": "ALTO RIO SENGUERR",
	"ARS": "ARAGARCAS",
	"ART": "WATERTOWN",
	"ARU": "ARACATUBA",
	"ARV": "MINOCQUA",
	"ARW": "ARAD",
	"ARX": "ASBURY PARK",
	"ARY": "ARARAT",
	"ARZ": "N'ZETO",
	"ASA": "ASSAB",
	"ASB": "ASHKHABAD",
	"ASC": "ASCENSION",
	"ASD": "ANDROS TOWN",
	"ASE": "ASPEN",
	"ASF": "ASTRAKHAN",
	"ASG": "ASHBURTON",
	"ASH": "NASHUA",
	"ASI": "ASCENSION ISLAND",
	"ASJ": "AMAMI O SHIMA",
	"ASK": "YAMOUSSOUKRO",
	"ASL": "MARSHALL",
	"ASM": "ASMARA",
	"ASN": "TALLADEGA",
	"ASO": "ASOSA",
	"ASP": "ALICE SPRINGS",
	"ASQ": "AUSTIN",
	"ASR": "KAYSERI",
	"AST": "ASTORIA",
	"ASU": "ASUNCION",
	"ASV": "AMBOSELI",
	"ASW": "ASWAN",
	"ASX": "ASHLAND",
	"ASY": "ASHLEY",
	"ASZ": "ASIRIM",
	"ATA": "ANTA",
	"ATB": "ATBARA",
	"ATC": "ARTHUR'S TOWN",
	"ATD": "ATOIFI",
	"ATE": "ANTLERS",
	"ATF": "AMBATO",
	"ATG": "ATTOCK",
	"ATH": "ATHENS",
	"ATI": "ARTIGAS",
	"ATJ": "ANTSIRABE",
	"ATK": "ATQASUK",
	"ATL": "ATLANTA",
	"ATM": "ALTAMIRA",
	"ATN": "NAMATANAI",
	"ATO": "ATHENS",
	"ATP": "AITAPE",
	"ATQ": "AMRITSAR",
	"ATR": "ATAR",
	"ATS": "ARTESIA",
	"ATT": "ATMAUTLUAK",
	"ATU": "ATTU ISLAND",
	"ATV": "ATI",
	"ATW": "APPLETON",
	"ATX": "ATBASAR",
	"ATY": "WATERTOWN",
	"ATZ": "ASSIUT",
	"AUA": "ARUBA",
	"AUC": "ARAUCA",
	"AUD": "AUGUSTUS DOWNS",
	"AUE": "ABU RUEDIS",
	"AUF": "AUXERRE",
	"AUG": "AUGUSTA",
	"AUH": "ABU DHABI",
	"AUI": "AUA",
	"AUJ": "AMBUNTI",
	"AUK": "ALAKANUK",
	"AUL": "AUR",
	"AUM": "AUSTIN",
	"AUN": "AUBURN",
	"AUO": "AUBURN/OPELIKA",
	"AUP": "AGUAN",
	"AUQ": "ATUONA",
	"AUR": "AURILLAC",
	"AUS": "AUSTIN",
	"AUT": "ATAURO",
	"AUU": "AURUKUN MISSION",
	"AUV": "AUMO",
	"AUW": "WAUSAU",
	"AUX": "ARAGUAINA",
	"AUY": "ANEITYUM",
	"AUZ": "AURORA",
	"AVB": "AVIANO",
	"AVC": "AVON",
	"AVG": "AUVERGNE",
	"AVI": "CIEGO DE AVILA",
	"AVK": "ARVAIKHEER",
	"AVL": "ASHEVILLE",
	"AVN": "AVIGNON",
	"AVO": "AVON PARK",
	"AVP": "SCRANTON/WILKES-BARRE",
	"AVU": "AVUAVU",
	"AVV": "AVALON",
	"AVW": "TUCSON",
	"AVX": "CATALINA ISLAND",
	"AWA": "AWASA",
	"AWB": "AWABA",
	"AWD": "ANIWA",
	"AWE": "ALOWE",
	"AWH": "AWAREH",
	"AWK": "WAKE ISLAND",
	"AWM": "WEST MEMPHIS",
	"AWN": "ALTON DOWNS",
	"AWP": "AUSTRAL DOWNS",
	"AWR": "AWAR",
	"AWZ": "AHWAZ",
	"AXA": "ANGUILLA",
	"AXB": "ALEXANDRIA BAY",
	"AXC": "ARAMAC",
	"AXD": "ALEXANDROUPOLIS",
	"AXG": "ALGONA",
	"AXK": "ATAQ",
	"AXL": "ALEXANDRIA",
	"AXM": "ARMENIA",
	"AXN": "ALEXANDRIA",
	"AXP": "SPRING POINT",
	"AXR": "ARUTUA",
	"AXS": "ALTUS",
	"AXT": "AKITA",
	"AXU": "AXUM",
	"AXV": "WAPAKONETA",
	"AXX": "ANGEL FIRE",
	"AYA": "AYAPEL",
	"AYC": "AYACUCHO",
	"AYD": "ALROY DOWNS",
	"AYG": "YAGUARA",
	"AYI": "YARI",
	"AYK": "ARKALYK",
	"AYL": "ANTHONY LAGOON",
	"AYN": "ANYANG",
	"AYO": "AYOLAS",
	"AYP": "AYACUCHO",
	"AYQ": "AYERS ROCK",
	"AYR": "AYR",
	"AYS": "WAYCROSS",
	"AYT": "ANTALYA",
	"AYU": "AIYURA",
	"AYW": "AYAWASI",
	"AYZ": "AMITYVILLE",
	"AZB": "AMAZON BAY",
	"AZD": "YAZD",
	"AZG": "APATZINGAN",
	"AZI": "ABU DHABI-BATEEN",
	"AZN": "ANDIZHAN",
	"AZO": "KALAMAZOO",
	"AZR": "ADRAR",
	"AZT": "ZAPATOCA",
	"AZZ": "AMBRIZ",
	"BAA": "BIALLA",
	"BAB": "MARYSVILLE",
	"BAC": "BARRANCA DE UPIA",
	"BAD": "SHREVEPORT",
	"BAE": "BARCELONNETTE",
	"BAF": "WESTFIELD",
	"BAG": "BAGUIO",
	"BAH": "BAHRAIN IS.",
	"BAI": "BUENOS AIRES",
	"BAJ": "BALI",
	"BAK": "BAKU",
	"BAL": "ATMAN",
	"BAM": "BATTLE MOUNTAIN",
	"BAN": "BASONGO",
	"BAO": "BAN MAK KHAENG",
	"BAP": "BAIBARA",
	"BAQ": "BARRANQUILLA",
	"BAR": "BAKER ISLAND",
	"BAS": "BALALAE",
	"BAT": "BARRETOS",
	"BAU": "BAURU",
	"BAV": "BAOTOU",
	"BAW": "BIAWONQUE",
	"BAX": "BARNAUL",
	"BAY": "BAIA MARE",
	"BAZ": "BARCELOS",
	"BBA": "BALMACEDA",
	"BBB": "BENSON",
	"BBC": "BAY CITY",
	"BBD": "BRADY",
	"BBE": "BIG BELL",
	"BBF": "BURLINGTON",
	"BBG": "BUTARITARI",
	"BBH": "BARTH",
	"BBI": "BHUBANESWAR",
	"BBK": "KASANE",
	"BBL": "BABOL SAR",
	"BBM": "BATTAMBANG",
	"BBN": "BARIO",
	"BBO": "BERBERA",
	"BBP": "BEMBRIDGE",
	"BBQ": "BARBUDA",
	"BBR": "BASSE TERRE",
	"BBS": "BLACKBUSH",
	"BBT": "BERBERATI",
	"BBU": "BUCHAREST",
	"BBV": "BEREBY",
	"BBW": "BROKEN BOW",
	"BBX": "BLUE BELL",
	"BBY": "BAMBARI",
	"BBZ": "ZAMBEZI",
	"BCA": "BARACOA",
	"BCB": "BLACKSBURG",
	"BCC": "BEAR CREEK",
	"BCD": "BACOLOD",
	"BCE": "BRYCE CANYON",
	"BCF": "BOUCA",
	"BCG": "BEMICHI",
	"BCH": "BAUCAU",
	"BCI": "BARCALDINE",
	"BCJ": "BACA GRANDE",
	"BCK": "BOLWORRA",
	"BCL": "BARRA COLORADO",
	"BCM": "BACAU",
	"BCN": "BARCELONA",
	"BCO": "BACO",
	"BCP": "BAMBU",
	"BCQ": "BRACH",
	"BCR": "BOCA DO ACRE",
	"BCS": "BELLE CHASSE",
	"BCT": "BOCA RATON",
	"BCU": "BAUCHI",
	"BCV": "BELMOPAN",
	"BCX": "BELORECK",
	"BCY": "BULCHI",
	"BCZ": "BICKERTON ISLAND",
	"BDA": "BERMUDA",
	"BDB": "BUNDABERG",
	"BDC": "BARRA DO CORDA",
	"BDD": "BADU ISLAND",
	"BDE": "BAUDETTE",
	"BDF": "BRADFORD",
	"BDG": "BLANDING",
	"BDH": "BANDAR LENGEH",
	"BDI": "BIRD IS.",
	"BDJ": "BANJARMASIN",
	"BDK": "BONDOUKOU",
	"BDL": "HARTFORD",
	"BDM": "BANDIRMA",
	"BDN": "ADIN",
	"BDO": "BANDUNG",
	"BDP": "BHADRAPUR",
	"BDQ": "VADODARA",
	"BDR": "BRIDGEPORT",
	"BDS": "BRINDISI",
	"BDT": "BADO LITE",
	"BDU": "BARDUFOSS",
	"BDV": "MOBA",
	"BDW": "BEDFORD DOWNS",
	"BDX": "BROADUS",
	"BDY": "BANDON",
	"BDZ": "BAINDOUNG",
	"BEA": "BEREINA",
	"BEB": "BENBECULA",
	"BED": "BEDFORD",
	"BEE": "BEAGLE",
	"BEF": "BLUEFIELDS",
	"BEG": "BELGRADE",
	"BEH": "BENTON HARBOR",
	"BEI": "BEICA",
	"BEJ": "BERAU",
	"BEK": "RAE BARELI",
	"BEL": "BELEM",
	"BEM": "BOSSEMBELE",
	"BEN": "BENGHAZI",
	"BEO": "NEWCASTLE",
	"BEP": "BELLARY",
	"BEQ": "BURY ST. EDMUNDS",
	"BER": "BERLIN",
	"BES": "BREST",
	"BET": "BETHEL",
	"BEU": "BEDOURIE",
	"BEV": "BEERSHEBA",
	"BEW": "BEIRA",
	"BEY": "BEIRUT",
	"BEZ": "BERU",
	"BFB": "BLUE FOX BAY",
	"BFC": "BLOOMFIELD",
	"BFD": "BRADFORD",
	"BFE": "BIELEFELD",
	"BFF": "SCOTTSBLUFF",
	"BFG": "BULLFROG BASIN",
	"BFI": "SEATTLE",
	"BFJ": "BA",
	"BFK": "DENVER",
	"BFL": "BAKERSFIELD",
	"BFM": "MOBILE",
	"BFN": "BLOEMFONTEIN",
	"BFO": "BUFFALO RANGE",
	"BFP": "TRENTON",
	"BFR": "BEDFORD",
	"BFS": "BELFAST",
	"BFT": "BEAUFORT",
	"BFU": "BENGBU",
	"BFX": "BAFOUSSAM",
	"BGA": "BUCARAMANGA",
	"BGB": "BOOUE",
	"BGC": "BRAGANCA",
	"BGD": "BORGER",
	"BGE": "BAINBRIDGE",
	"BGF": "BANGUI",
	"BGG": "BONGOUANOU",
	"BGH": "BOGHE",
	"BGI": "BARBADOS",
	"BGJ": "BORGARFJORDUR",
	"BGK": "BIG CREEK",
	"BGL": "BAGLUNG",
	"BGM": "BINGHAMTON",
	"BGN": "BRUEGGEN",
	"BGO": "BERGEN",
	"BGP": "BONGO",
	"BGQ": "BIG LAKE",
	"BGR": "BANGOR",
	"BGS": "BIG SPRING",
	"BGT": "BAGDAD",
	"BGU": "BANGASSOU",
	"BGV": "BENTO GONCALVES",
	"BGW": "BAGHDAD",
	"BGX": "BAGE",
	"BGY": "BERGAMO",
	"BGZ": "BRAGA",
	"BHA": "BAHIA",
	"BHB": "BAR HARBOR",
	"BHC": "BHURBAN",
	"BHD": "BELFAST",
	"BHE": "BLENHEIM",
	"BHF": "BAHIA CUPICA",
	"BHG": "BRUS LAGUNA",
	"BHH": "BISHA",
	"BHI": "BAHIA BLANCA",
	"BHJ": "BHUJ",
	"BHK": "BUKHARA",
	"BHL": "BAHIA DE LOS ANGELES",
	"BHM": "BIRMINGHAM",
	"BHN": "BEIHAN",
	"BHO": "BHOPAL",
	"BHP": "BHOJPUR",
	"BHQ": "BROKEN HILL",
	"BHR": "BHARATPUR",
	"BHS": "BATHURST",
	"BHT": "BRIGHTON DOWNS",
	"BHU": "BHAVNAGAR",
	"BHV": "BAHAWALPUR",
	"BHX": "BIRMINGHAM",
	"BHY": "BEIHAI",
	"BHZ": "BELO HORIZONTE",
	"BIA": "BASTIA",
	"BIB": "BAIDOA",
	"BIC": "BIG CREEK",
	"BID": "BLOCK ISLAND",
	"BIE": "BEATRICE",
	"BIF": "EL PASO",
	"BIG": "BIG DELTA",
	"BIH": "BISHOP",
	"BII": "BIKINI ATOLL",
	"BIJ": "BILIAU",
	"BIK": "BIAK",
	"BIL": "BILLINGS",
	"BIM": "BIMINI",
	"BIN": "BAMIAN",
	"BIO": "BILBAO",
	"BIP": "BULIMBA",
	"BIQ": "BIARRITZ",
	"BIR": "BIRATNAGAR",
	"BIS": "BISMARCK",
	"BIT": "BAITADI",
	"BIU": "BILDUDALUR",
	"BIV": "BRIA",
	"BIW": "BILLILUNA",
	"BIX": "BILOXI",
	"BIY": "BISHO",
	"BIZ": "BIMIN",
	"BJA": "BEJAIA",
	"BJC": "BROOMFIELD",
	"BJD": "BAKKAFJORDUR",
	"BJF": "BATSFJORD",
	"BJG": "BOLAANG",
	"BJH": "BAJHANG",
	"BJI": "BEMIDJI",
	"BJJ": "WOOSTER",
	"BJK": "BENJINA",
	"BJL": "BANJUL",
	"BJM": "BUJUMBURA",
	"BJN": "BAJONE",
	"BJO": "BERMEJO",
	"BJR": "BAHAR DAR",
	"BJS": "BEIJING",
	"BJU": "BAJURA",
	"BJW": "BAJAWA",
	"BJX": "LEON-GUANAJUATO",
	"BJZ": "BADAJOZ",
	"BKA": "BYKOVO",
	"BKB": "BIKANER",
	"BKC": "BUCKLAND",
	"BKD": "BRECKENRIDGE",
	"BKE": "BAKER",
	"BKF": "BROOKS LAKE",
	"BKH": "KEKAHA",
	"BKI": "KOTA KINABALU",
	"BKJ": "BOKE",
	"BKK": "BANGKOK",
	"BKL": "CLEVELAND",
	"BKM": "BAKALALAN",
	"BKN": "BIRNI NKONI",
	"BKO": "BAMAKO",
	"BKP": "BARKLY DOWNS",
	"BKQ": "BLACKALL",
	"BKR": "BOKORO",
	"BKS": "BENGKULU",
	"BKT": "BLACKSTONE",
	"BKU": "BETIOKY",
	"BKW": "BECKLEY",
	"BKX": "BROOKINGS",
	"BKY": "BUKAVU",
	"BKZ": "BUKOBA",
	"BLA": "BARCELONA",
	"BLB": "BALBOA",
	"BLC": "BALI",
	"BLE": "BORLANGE",
	"BLF": "PRINCETON/BLUEFIELD",
	"BLG": "BELAGA",
	"BLH": "BLYTHE",
	"BLI": "BELLINGHAM",
	"BLJ": "BATNA",
	"BLK": "BLACKPOOL",
	"BLL": "BILLUND",
	"BLM": "BELMAR",
	"BLN": "BENALLA",
	"BLO": "BLONDUOS",
	"BLP": "BELLAVISTA",
	"BLQ": "BOLOGNA",
	"BLR": "BANGALORE",
	"BLS": "BOLLON",
	"BLT": "BLACKWATER",
	"BLV": "BELLEVILLE",
	"BLW": "WAIMANALO",
	"BLX": "BELLUNO",
	"BLY": "BELMULLET",
	"BLZ": "BLANTYRE",
	"BMA": "STOCKHOLM",
	"BMB": "BUMBA",
	"BMC": "BRIGHAM CITY",
	"BMD": "BELO",
	"BME": "BROOME",
	"BMF": "BAKOUMA",
	"BMG": "BLOOMINGTON",
	"BMH": "BOMAI",
	"BMI": "BLOOMINGTON",
	"BMJ": "BARAMITA",
	"BMK": "BORKUM",
	"BML": "BERLIN",
	"BMM": "BITAM",
	"BMN": "BAMERNY",
	"BMO": "BHAMO",
	"BMP": "BRAMPTON ISLAND",
	"BMQ": "BAMBURI",
	"BMR": "BALTRUM",
	"BMS": "BRUMADO",
	"BMU": "BIMA",
	"BMV": "BAN ME THUOT",
	"BMW": "BORDJ BADJI MOKHTAR",
	"BMX": "BIG MOUNTAIN",
	"BMY": "BELEP",
	"BMZ": "BAMU",
	"BNA": "NASHVILLE",
	"BNB": "BOENDE",
	"BNC": "BENI",
	"BND": "BANDAR ABBAS",
	"BNE": "BRISBANE",
	"BNF": "BARANOF",
	"BNG": "BANNING",
	"BNI": "BENIN CITY",
	"BNJ": "BONN",
	"BNK": "BALLINA",
	"BNL": "BARNWELL",
	"BNM": "BODINUMU",
	"BNN": "BRONNOYSUND",
	"BNO": "BURNS",
	"BNP": "BANNU",
	"BNQ": "BAGANGA",
	"BNR": "BANFORA",
	"BNS": "BARINAS",
	"BNT": "BUNDI",
	"BNU": "BLUMENAU",
	"BNV": "BOANA",
	"BNW": "BOONE",
	"BNX": "BANJA LUKA",
	"BNY": "BELLONA IS.",
	"BNZ": "BANZ",
	"BOA": "BOMA",
	"BOB": "BORA BORA",
	"BOC": "BOCAS DEL TORO",
	"BOD": "BORDEAUX",
	"BOE": "BOUNDJI",
	"BOG": "BOGOTA",
	"BOH": "BOURNEMOUTH",
	"BOI": "BOISE",
	"BOJ": "BURGAS",
	"BOK": "BROOKINGS",
	"BOM": "BOMBAY",
	"BON": "BONAIRE",
	"BOO": "BODO",
	"BOP": "BOUAR",
	"BOQ": "BOKU",
	"BOR": "BELFORT",
	"BOS": "BOSTON",
	"BOT": "BOSET",
	"BOU": "BOURGES",
	"BOV": "BOANG",
	"BOW": "BARTOW",
	"BOX": "BORROLOOLA",
	"BOY": "BOBO DIOULASSO",
	"BOZ": "BOZOUM",
	"BPA": "BETHPAGE",
	"BPB": "BORIDI",
	"BPC": "BAMENDA",
	"BPD": "BAPI",
	"BPE": "BAGAN",
	"BPF": "BATUNA",
	"BPG": "BARRA DO GARCAS",
	"BPH": "BISLIG",
	"BPI": "BIG PINEY",
	"BPN": "BALIKPAPAN",
	"BPS": "PORTO SEGURO",
	"BPT": "BEAUMONT/PORT ARTHUR",
	"BPU": "BEPPU",
	"BPY": "BESALAMPY",
	"BQA": "BALER",
	"BQE": "BUBAQUE",
	"BQH": "BIGGIN HILL",
	"BQK": "ST. SIMONS ISLAND",
	"BQL": "BOULIA",
	"BQN": "AGUADILLA",
	"BQO": "BOUNA",
	"BQQ": "BARRA",
	"BQS": "BLAGOVESCHENSK",
	"BQT": "BREST",
	"BQU": "PORT ELIZABETH",
	"BQV": "GUSTAVUS",
	"BQW": "BALGO HILLS",
	"BRA": "BARREIRAS",
	"BRB": "BARREIRINHAS",
	"BRC": "SAN CARLOS DE BARILOCHE",
	"BRD": "BRAINERD",
	"BRE": "BREMEN",
	"BRG": "WHITESBURG",
	"BRH": "BRAHMAN",
	"BRI": "BARI",
	"BRJ": "BRIGHT",
	"BRK": "BOURKE",
	"BRL": "BURLINGTON",
	"BRM": "BARQUISIMETO",
	"BRN": "BERNE",
	"BRO": "BROWNSVILLE",
	"BRP": "BIARU",
	"BRQ": "BRNO",
	"BRR": "BARRA",
	"BRS": "BRISTOL",
	"BRT": "BATHURST ISLAND",
	"BRU": "BRUSSELS",
	"BRV": "BREMERHAVEN",
	"BRW": "BARROW",
	"BRX": "BARAHONA",
	"BRY": "BARDSTOWN",
	"BRZ": "BOROTOU",
	"BSA": "BOSASO",
	"BSB": "BRASILIA",
	"BSC": "BAHIA SOLANO",
	"BSD": "BAOSHAN",
	"BSE": "SEMATAN",
	"BSG": "BATA",
	"BSH": "BRIGHTON",
	"BSI": "BLAIRSVILLE",
	"BSJ": "BAIRNSDALE",
	"BSK": "BISKRA",
	"BSL": "BASEL",
	"BSM": "AUSTIN",
	"BSN": "BOSSANGOA",
	"BSO": "BASCO",
	"BSP": "BENSBACH",
	"BSQ": "BISBEE",
	"BSR": "BASRA",
	"BSS": "BALSAS",
	"BST": "BOST",
	"BSU": "BASANKUSU",
	"BSV": "BOSSET",
	"BSW": "BOSWELL BAY",
	"BSX": "BASSEIN",
	"BSY": "BARDERA",
	"BSZ": "BARTLETTS",
	"BTA": "BERTOUA",
	"BTB": "BETOU",
	"BTC": "BATTICALOA",
	"BTD": "BRUNETTE DOWNS",
	"BTE": "BONTHE",
	"BTF": "BOUNTIFUL",
	"BTG": "BATANGAFO",
	"BTH": "BATU BESAR",
	"BTI": "BARTER ISLAND",
	"BTJ": "BANDA ACEH",
	"BTK": "BRATSK",
	"BTL": "BATTLE CREEK",
	"BTM": "BUTTE",
	"BTN": "BENNETTSVILLE",
	"BTO": "BOTOPASIE",
	"BTP": "BUTLER",
	"BTQ": "BUTARE",
	"BTR": "BATON ROUGE",
	"BTS": "BRATISLAVA",
	"BTT": "BETTLES",
	"BTU": "BINTULU",
	"BTV": "BURLINGTON",
	"BTW": "BATU LICIN",
	"BTX": "BETOOTA",
	"BTY": "BEATTY",
	"BTZ": "BURSA",
	"BUA": "BUKA IS.",
	"BUB": "BURWELL",
	"BUC": "BURKETOWN",
	"BUD": "BUDAPEST",
	"BUE": "BUENOS AIRES",
	"BUF": "BUFFALO",
	"BUG": "BENGUELA",
	"BUH": "BUCHAREST",
	"BUI": "BOKOUDINI",
	"BUJ": "BOUSSAADA",
	"BUK": "ALBUQ",
	"BUL": "BULOLO",
	"BUM": "BUTLER",
	"BUN": "BUENAVENTURA",
	"BUO": "BURAO",
	"BUP": "BHATINDA",
	"BUQ": "BULAWAYO",
	"BUR": "BURBANK",
	"BUS": "BATUMI",
	"BUT": "BURTONWOOD",
	"BUU": "BUYO",
	"BUV": "BELLA UNION",
	"BUW": "BAUBAU",
	"BUX": "BUNIA",
	"BUY": "BUNBURY",
	"BUZ": "BUSHEHR",
	"BVA": "BEAUVAIS",
	"BVB": "BOA VISTA",
	"BVC": "BOA VISTA",
	"BVD": "BEAVER INLET",
	"BVE": "BRIVE",
	"BVF": "BUA",
	"BVG": "BERLEVAG",
	"BVH": "VILHENA",
	"BVI": "BIRDSVILLE",
	"BVK": "HUACARAJE",
	"BVL": "BAURES",
	"BVM": "BELMONTE",
	"BVO": "BARTLESVILLE",
	"BVP": "BOLOVIP",
	"BVR": "BRAVA",
	"BVS": "BELA VISTA",
	"BVU": "BELUGA",
	"BVW": "BATAVIA DOWNS",
	"BVX": "BATESVILLE",
	"BVY": "BEVERLY",
	"BVZ": "BEVERLEY SPRINGS",
	"BWA": "BHAIRAWA",
	"BWB": "BARROW ISLAND",
	"BWC": "BRAWLEY",
	"BWD": "BROWNWOOD",
	"BWE": "BRAUNSCHWEIG",
	"BWF": "BARROW-IN-FURNESS",
	"BWG": "BOWLING GREEN",
	"BWH": "BUTTERWORTH",
	"BWI": "BALTIMORE",
	"BWJ": "BAWAN",
	"BWK": "BOL",
	"BWL": "BLACKWELL",
	"BWM": "BOWMAN",
	"BWN": "BANDAR SERI BEGAWAN",
	"BWO": "BALAKOVO",
	"BWP": "BEWANI",
	"BWQ": "BREWARRINA",
	"BWS": "BLAINE",
	"BWT": "BURNIE",
	"BWU": "BANKSTOWN",
	"BWY": "WOODBRIDGE",
	"BXA": "BOGALUSA",
	"BXB": "BABO",
	"BXC": "BOXBOROUGH",
	"BXD": "BADE",
	"BXE": "BAKEL",
	"BXH": "BALHASH",
	"BXI": "BOUNDIALI",
	"BXK": "BUCKEYE",
	"BXL": "BLUE LAGOON",
	"BXM": "BATOM",
	"BXN": "BODRUM",
	"BXO": "BISSAU",
	"BXS": "BORREGO SPRINGS",
	"BXT": "BONTANG",
	"BXU": "BUTUAN",
	"BXV": "BREIDDALSVIK",
	"BXX": "BORAMA",
	"BYA": "BOUNDARY",
	"BYB": "BAYA",
	"BYC": "YACUIBA",
	"BYD": "BEIDAH",
	"BYG": "BUFFALO",
	"BYH": "BLYTHEVILLE",
	"BYI": "BURLEY",
	"BYK": "BOUAKE",
	"BYL": "BELLE YELLA",
	"BYM": "BAYAMO",
	"BYN": "BAYANKHONGOR",
	"BYQ": "BUNYU",
	"BYR": "LAESO ISLAND",
	"BYT": "BANTRY",
	"BYU": "BAYREUTH",
	"BYW": "BLAKELY ISLAND",
	"BYX": "BANIYALA",
	"BZA": "BONANZA",
	"BZC": "BUZIOS",
	"BZD": "BALRANALD",
	"BZE": "BELIZE CITY",
	"BZG": "BYDGOSZCZ",
	"BZI": "BALIKESIR",
	"BZK": "BRIANSK",
	"BZL": "BARISAL",
	"BZM": "BERGEN OP ZOOM",
	"BZN": "BOZEMAN",
	"BZO": "BOLZANO",
	"BZP": "BIZANT",
	"BZR": "BEZIERS",
	"BZT": "BRAZORIA",
	"BZU": "BUTA",
	"BZV": "BRAZZAVILLE",
	"BZY": "BELTSY",
	"BZZ": "BRIZE NORTON",
	"CAA": "CATACAMAS",
	"CAB": "CABINDA",
	"CAC": "CASCAVEL",
	"CAD": "CADILLAC",
	"CAE": "COLUMBIA",
	"CAF": "CARAUARI",
	"CAG": "CAGLIARI",
	"CAH": "CA MAU",
	"CAI": "CAIRO",
	"CAJ": "CANAIMA",
	"CAK": "AKRON/CANTON",
	"CAL": "CAMPBELTOWN",
	"CAM": "CAMIRI",
	"CAN": "GUANGZHOU",
	"CAO": "CLAYTON",
	"CAP": "CAP HAITIEN",
	"CAQ": "CAUCASIA",
	"CAR": "CARIBOU",
	"CAS": "CASABLANCA",
	"CAT": "CAT ISLAND",
	"CAU": "CARUARU",
	"CAV": "CAZOMBO",
	"CAW": "CAMPOS",
	"CAX": "CARLISLE",
	"CAY": "CAYENNE",
	"CAZ": "COBAR",
	"CBA": "CORNER BAY",
	"CBB": "COCHABAMBA",
	"CBC": "CHERRABUN",
	"CBD": "CAR NICOBAR",
	"CBE": "CUMBERLAND",
	"CBF": "COUNCIL BLUFFS",
	"CBG": "CAMBRIDGE",
	"CBH": "COLOMB BECHAR",
	"CBI": "CAPE BARREN ISLAND",
	"CBJ": "CABO ROJO",
	"CBK": "COLBY",
	"CBL": "CIUDAD BOLIVAR",
	"CBM": "COLUMBUS",
	"CBN": "CIREBON",
	"CBO": "COTABATO",
	"CBP": "COIMBRA",
	"CBQ": "CALABAR",
	"CBR": "CANBERRA",
	"CBS": "CABIMAS",
	"CBT": "CATUMBELA",
	"CBU": "COTTBUS",
	"CBV": "COBAN",
	"CBX": "CONDOBOLIN",
	"CBY": "CANOBIE",
	"CBZ": "CABIN CREEK",
	"CCB": "UPLAND",
	"CCC": "CAYO COCO",
	"CCE": "ST. MAARTEN NETH. AN",
	"CCF": "CARCASSONNE",
	"CCG": "CRANE",
	"CCH": "CHILE CHICO",
	"CCI": "CONCORDIA",
	"CCJ": "CALICUT",
	"CCK": "COCOS-KEELING IS.",
	"CCL": "CHINCHILLA",
	"CCM": "CRICIUMA",
	"CCN": "CHAKCHARAN",
	"CCO": "CARIMAGUA",
	"CCP": "CONCEPCION",
	"CCQ": "CACHOEIRA",
	"CCR": "CONCORD",
	"CCS": "CARACAS",
	"CCT": "COLONIA CATRIEL",
	"CCU": "CALCUTTA",
	"CCV": "CRAIG COVE",
	"CCW": "COWELL",
	"CCX": "CACERES",
	"CCY": "CHARLES CITY",
	"CCZ": "CHUB CAY",
	"CDA": "COOINDA",
	"CDB": "COLD BAY",
	"CDC": "CEDAR CITY",
	"CDE": "CALEDONIA",
	"CDF": "CORTINA D'AMPEZZO",
	"CDG": "PARIS",
	"CDH": "CAMDEN",
	"CDJ": "CONCEICAO DO ARAGUAIA",
	"CDK": "CEDAR KEY",
	"CDL": "CANDLE",
	"CDN": "CAMDEN",
	"CDO": "CRADOCK",
	"CDP": "CUDDAPAH",
	"CDQ": "CROYDON",
	"CDR": "CHADRON",
	"CDS": "CHILDRESS",
	"CDU": "CAMDEN",
	"CDV": "CORDOVA",
	"CDW": "CALDWELL",
	"CDY": "CAGAYAN DE SULU",
	"CEB": "CEBU",
	"CEC": "CRESCENT CITY",
	"CED": "CEDUNA",
	"CEE": "CHEREPOVETS",
	"CEF": "CHICOPEE",
	"CEG": "CHESTER",
	"CEI": "CHIANG RAI",
	"CEJ": "CHERNIGOV",
	"CEK": "CHELYABINSK",
	"CEL": "CAPE ELEUTHERA",
	"CEM": "CENTRAL",
	"CEN": "CIUDAD OBREGON",
	"CEO": "WACO KUNGO",
	"CEP": "CONCEPCION",
	"CEQ": "CANNES",
	"CER": "CHERBOURG",
	"CES": "CESSNOCK",
	"CET": "CHOLET",
	"CEU": "CLEMSON",
	"CEV": "CONNERSVILLE",
	"CEW": "CRESTVIEW",
	"CEX": "CHENA HOT SPGS",
	"CEY": "MURRAY",
	"CEZ": "CORTEZ",
	"CFA": "COFFEE POINT",
	"CFE": "CLERMONT-FERRAND",
	"CFF": "CAFUNFO",
	"CFG": "CIENFUEGOS",
	"CFH": "CLIFTON HILLS",
	"CFI": "CAMFIELD",
	"CFN": "CARRICKFINN",
	"CFO": "CONFREZA",
	"CFP": "CARPENTARIA DOWNS",
	"CFR": "CAEN",
	"CFS": "COFFS HARBOUR",
	"CFT": "CLIFTON",
	"CFU": "CORFU",
	"CGA": "CRAIG",
	"CGB": "CUIABA",
	"CGC": "CAPE GLOUCESTER",
	"CGD": "CHANGDE",
	"CGE": "CAMBRIDGE",
	"CGF": "CLEVELAND",
	"CGH": "SAO PAULO",
	"CGI": "CAPE GIRARDEAU",
	"CGJ": "CHINGOLA",
	"CGK": "JAKARTA-CENGKARENG",
	"CGM": "CAMIGUIN",
	"CGN": "COLOGNE",
	"CGO": "ZHENGZHOU",
	"CGP": "CHITTAGONG",
	"CGQ": "CHANGCHUN",
	"CGR": "CAMPO GRANDE",
	"CGS": "COLLEGE PARK",
	"CGT": "CHEGUITTI",
	"CGU": "CIUDAD GUAYANA",
	"CGV": "CAIGUNA",
	"CGX": "CHICAGO",
	"CGY": "CAGAYAN DE ORO",
	"CGZ": "CASA GRANDE",
	"CHA": "CHATTANOOGA",
	"CHB": "CHILAS",
	"CHC": "CHRISTCHURCH",
	"CHD": "CHANDLER",
	"CHE": "CAHERCIVEEN",
	"CHF": "CHINHAE",
	"CHG": "CHAOYANG",
	"CHH": "CHACHAPOYAS",
	"CHI": "CHICAGO",
	"CHJ": "CHIPINGE",
	"CHK": "CHICKASHA",
	"CHL": "CHALLIS",
	"CHM": "CHIMBOTE",
	"CHN": "CHONJU",
	"CHO": "CHARLOTTESVILLE",
	"CHP": "CIRCLE HOT SPRINGS",
	"CHQ": "CHANIA",
	"CHR": "CHATEAUROUX",
	"CHS": "CHARLESTON",
	"CHT": "CHATHAM ISLAND",
	"CHU": "CHUATHBALUK",
	"CHV": "CHAVES",
	"CHW": "JIUQUAN",
	"CHX": "CHANGUINOLA",
	"CHY": "CHOISEUL BAY",
	"CHZ": "CHILOQUIN",
	"CIA": "ROME",
	"CIB": "CATALINA ISLAND",
	"CIC": "CHICO",
	"CID": "CEDAR RAPIDS/IOWA CITY",
	"CIE": "COLLIE",
	"CIF": "CHIFENG",
	"CIG": "CRAIG",
	"CIH": "CHANGZHI",
	"CIJ": "COBIJA",
	"CIK": "CHALKYITSIK",
	"CIL": "COUNCIL",
	"CIM": "CIMITARRA",
	"CIN": "CARROLL",
	"CIP": "CHIPATA",
	"CIQ": "CHIQUIMULA",
	"CIR": "CAIRO",
	"CIS": "CANTON IS.",
	"CIT": "CHIMKENT",
	"CIU": "SAULT STE. MARIE",
	"CIV": "CHOMLEY",
	"CIW": "CANOUAN ISLAND",
	"CIX": "CHICLAYO",
	"CIY": "COMISO",
	"CIZ": "COARI",
	"CJA": "CAJAMARCA",
	"CJB": "COIMBATORE",
	"CJC": "CALAMA",
	"CJD": "CANDILEJAS",
	"CJH": "CHILKO LAKE",
	"CJL": "CHITRAL",
	"CJN": "EL CAJON",
	"CJS": "CIUDAD JUAREZ",
	"CJU": "CHEJU",
	"CKB": "CLARKSBURG",
	"CKC": "CHERKASSY",
	"CKD": "CROOKED CREEK",
	"CKE": "CLEAR LAKE",
	"CKG": "CHONGQING",
	"CKH": "CHOKURDAH",
	"CKI": "CROKER IS.",
	"CKK": "CHEROKEE",
	"CKM": "CLARKSDALE",
	"CKN": "CROOKSTON",
	"CKO": "CORNELIO PROCOPIO",
	"CKR": "CRANE ISLAND",
	"CKS": "CARAJAS",
	"CKV": "CLARKSVILLE",
	"CKX": "CHICKEN",
	"CKY": "CONAKRY",
	"CKZ": "CANAKKALE",
	"CLA": "COMILLA",
	"CLB": "CASTLEBAR",
	"CLC": "CLEAR LAKE CITY",
	"CLD": "CARLSBAD",
	"CLE": "CLEVELAND",
	"CLG": "COALINGA",
	"CLH": "COOLAH",
	"CLJ": "CLUJ",
	"CLK": "CLINTON",
	"CLL": "COLLEGE STATION",
	"CLM": "PORT ANGELES",
	"CLN": "CAROLINA",
	"CLO": "CALI",
	"CLP": "CLARKS POINT",
	"CLQ": "COLIMA",
	"CLR": "CALIPATRIA",
	"CLS": "CHEHALIS",
	"CLT": "CHARLOTTE",
	"CLU": "COLUMBUS",
	"CLV": "CALDAS NOVAS",
	"CLX": "CLORINDA",
	"CLY": "CALVI",
	"CLZ": "CALABOZO",
	"CMA": "CUNNAMULLA",
	"CMB": "COLOMBO",
	"CMC": "CAMOCIM",
	"CMD": "COOTAMUNDRA",
	"CME": "CIUDAD DEL CARMEN",
	"CMF": "CHAMBERY",
	"CMG": "CORUMBA",
	"CMH": "COLUMBUS",
	"CMI": "CHAMPAIGN",
	"CMJ": "CHI MEI",
	"CMK": "MAKOKOLA",
	"CML": "CAMOOWEAL",
	"CMM": "CARMELITA",
	"CMN": "CASABLANCA",
	"CMO": "OBBIA",
	"CMP": "CAMPO ALEGRE",
	"CMQ": "CLERMONT",
	"CMR": "COLMAR",
	"CMS": "SCUSCIUBAN",
	"CMT": "CAMETA",
	"CMU": "KUNDIAWA",
	"CMV": "COROMANDEL",
	"CMW": "CAMAGUEY",
	"CMX": "HANCOCK",
	"CMY": "SPARTA",
	"CNA": "CANANEA",
	"CNB": "COONAMBLE",
	"CNC": "COCONUT ISLAND",
	"CND": "CONSTANTA",
	"CNE": "CANON CITY",
	"CNF": "BELO HORIZONTE-CONFINS",
	"CNG": "COGNAC",
	"CNH": "CLAREMONT",
	"CNI": "CHANGHAI",
	"CNJ": "CLONCURRY",
	"CNK": "CONCORDIA",
	"CNL": "SINDAL",
	"CNM": "CARLSBAD",
	"CNN": "CHULMAN",
	"CNO": "CHINO",
	"CNP": "EASTGREENLAND",
	"CNQ": "CORRIENTES",
	"CNR": "CHANARAL",
	"CNS": "CAIRNS",
	"CNT": "CHARATA",
	"CNU": "CHANUTE",
	"CNV": "CANAVIEIRAS",
	"CNW": "WACO",
	"CNX": "CHIANG MAI",
	"CNY": "MOAB",
	"CNZ": "CANGAMBA",
	"COA": "COLUMBIA",
	"COB": "COOLIBAH",
	"COC": "CONCORDIA",
	"COD": "LOVELL/CODY",
	"COE": "COEUR D'ALENE",
	"COF": "COCOA BEACH",
	"COG": "CONDOTO",
	"COH": "COOCH BEHAR",
	"COI": "COCOA",
	"COJ": "COONABARABRAN",
	"COK": "COCHIN",
	"COL": "COLL ISLAND",
	"COM": "COLEMAN",
	"CON": "CONCORD",
	"COO": "COTONOU",
	"COQ": "CHOIBALSAN",
	"COR": "CORDOBA",
	"COS": "COLORADO SPRINGS",
	"COT": "COTULLA",
	"COU": "COLUMBIA",
	"COV": "COVILHA",
	"COW": "COOLANGATTA",
	"COX": "CONGO TOWN",
	"COY": "COOLAWANYAH",
	"COZ": "CONSTANZA",
	"CPA": "CAPE PALMAS",
	"CPB": "CAPURGANA",
	"CPC": "CHAPELCO",
	"CPD": "COOBER PEDY",
	"CPE": "CAMPECHE",
	"CPF": "CEPU",
	"CPG": "CARMEN DE PATAGONES",
	"CPH": "COPENHAGEN",
	"CPL": "CHAPARRAL",
	"CPM": "COMPTON",
	"CPN": "CAPE RODNEY",
	"CPO": "COPIAPO",
	"CPQ": "CAMPINAS",
	"CPR": "CASPER",
	"CPS": "ST. LOUIS",
	"CPT": "CAPE TOWN",
	"CPU": "CURURUPU",
	"CPV": "CAMPINA GRANDE",
	"CPX": "CULEBRA",
	"CQF": "CALAIS",
	"CQP": "CAPE FLATTERY",
	"CQT": "CAQUETANIA",
	"CRA": "CRAIOVA",
	"CRB": "COLLARENEBRI",
	"CRC": "CARTAGO",
	"CRD": "COMODORO RIVADAVIA",
	"CRE": "MYRTLE BEACH",
	"CRF": "CARNOT",
	"CRG": "JACKSONVILLE",
	"CRH": "CHERRIBAH",
	"CRI": "CROOKED ISLAND",
	"CRJ": "COORABIE",
	"CRK": "LUZON ISLAND",
	"CRL": "CHARLEROI",
	"CRM": "CATARMAN",
	"CRN": "CROMARTY",
	"CRO": "CORCORAN",
	"CRP": "CORPUS CHRISTI",
	"CRQ": "CARAVELAS",
	"CRR": "CERES",
	"CRS": "CORSICANA",
	"CRT": "CROSSETT",
	"CRU": "CARRIACOU",
	"CRV": "CROTONE",
	"CRW": "CHARLESTON",
	"CRX": "CORINTH",
	"CRY": "CARLTON",
	"CRZ": "CHARDZHOU",
	"CSA": "COLONSAY IS.",
	"CSB": "CARANSEBES",
	"CSC": "CANAS",
	"CSD": "CRESSWELL DOWNS",
	"CSE": "CRESTED BUTTE",
	"CSF": "CREIL",
	"CSG": "COLUMBUS",
	"CSH": "CAPE SARICHEF",
	"CSI": "CASINO",
	"CSJ": "CAPE ST JACQUES",
	"CSK": "CAP SKIRRING",
	"CSM": "CLINTON",
	"CSN": "CARSON CITY",
	"CSQ": "CRESTON",
	"CSR": "CASUARITO",
	"CSS": "CASSILANDIA",
	"CST": "CASTAWAY",
	"CSU": "SANTA CRUZ DO SUL",
	"CSV": "CROSSVILLE",
	"CSW": "COLORADO DO OESTE",
	"CSX": "CHANGSHA",
	"CSY": "CHEBOKSARY",
	"CTA": "CATANIA",
	"CTB": "CUT BANK",
	"CTC": "CATAMARCA",
	"CTD": "CHITRE",
	"CTE": "CARTI",
	"CTG": "CARTAGENA",
	"CTH": "COATESVILLE",
	"CTI": "CUITO-CUANAVALE",
	"CTK": "CANTON",
	"CTL": "CHARLEVILLE",
	"CTM": "CHETUMAL",
	"CTN": "COOKTOWN",
	"CTO": "CALVERTON",
	"CTP": "CARUTAPERA",
	"CTQ": "SANTA VITORIA",
	"CTR": "CATTLE CREEK",
	"CTS": "SAPPORO",
	"CTT": "LE CASTELLET",
	"CTU": "CHENGDU",
	"CTW": "COTTONWOOD",
	"CTX": "CORTLAND",
	"CTY": "CROSS CITY",
	"CTZ": "CLINTON",
	"CUA": "CIUDAD CONSTITUCION",
	"CUB": "COLUMBIA",
	"CUC": "CUCUTA",
	"CUD": "CALOUNDRA",
	"CUE": "CUENCA",
	"CUF": "CUNEO",
	"CUG": "ORANGE",
	"CUH": "CUSHING",
	"CUI": "CURRILLO",
	"CUJ": "CULION",
	"CUK": "CAYE CAULKER",
	"CUL": "CULIACAN",
	"CUM": "CUMANA",
	"CUN": "CANCUN",
	"CUO": "CARURU",
	"CUP": "CARUPANO",
	"CUQ": "COEN",
	"CUR": "CURACAO",
	"CUS": "COLUMBUS",
	"CUT": "CUTRAL-CO",
	"CUU": "CHIHUAHUA",
	"CUV": "CASIGUA",
	"CUW": "CUBE COVE",
	"CUY": "CUE",
	"CUZ": "CUZCO",
	"CVB": "CHUNGRIBU",
	"CVC": "CLEVE",
	"CVE": "COVENAS",
	"CVF": "COURCHEVEL",
	"CVG": "CINCINNATI",
	"CVH": "CAVIAHUE",
	"CVI": "CALETA OLIVIA",
	"CVJ": "CUERNAVACA",
	"CVL": "CAPE VOGEL",
	"CVM": "CIUDAD VICTORIA",
	"CVN": "CLOVIS",
	"CVO": "CORVALLIS",
	"CVQ": "CARNARVON",
	"CVR": "CULVER CITY",
	"CVS": "CLOVIS",
	"CVT": "COVENTRY",
	"CVU": "CORVO ISLAND",
	"CWA": "WAUSAU",
	"CWB": "CURITIBA",
	"CWC": "CHERNOVTSY",
	"CWG": "CALLAWAY GARDENS",
	"CWI": "CLINTON",
	"CWL": "CARDIFF",
	"CWP": "CAMPBELLPORE",
	"CWR": "COWARIE",
	"CWS": "CENTER ISLAND",
	"CWT": "COWRA",
	"CWW": "COROWA",
	"CXA": "CAICARA",
	"CXB": "COX'S BAZAR",
	"CXC": "CHITINA",
	"CXF": "COLDFOOT",
	"CXH": "VANCOUVER",
	"CXI": "CHRISTMAS ISLAND",
	"CXJ": "CAXIAS DO SUL",
	"CXL": "CALEXICO",
	"CXN": "CANDALA",
	"CXO": "CONROE",
	"CXP": "CILACAP",
	"CXQ": "CHRISTMAS CREEK",
	"CXT": "CHARTERS TOWERS",
	"CXY": "CAT CAY",
	"CYA": "CAYES",
	"CYB": "CAYMAN BRAC",
	"CYC": "CAYE CHAPEL",
	"CYE": "CRYSTAL LAKE",
	"CYF": "CHEFORNAK",
	"CYG": "CORRYONG",
	"CYI": "CHIAYI",
	"CYL": "COYOLES",
	"CYM": "CHATHAM",
	"CYO": "CAYO LARGO DEL SUR",
	"CYP": "CALBAYOG",
	"CYR": "COLONIA",
	"CYS": "CHEYENNE",
	"CYT": "CAPE YAKATAGA",
	"CYU": "CUYO",
	"CYX": "CHERSKIY",
	"CYZ": "CAUAYAN",
	"CZA": "CHICHEN ITZA",
	"CZB": "CRUZ ALTA",
	"CZC": "COPPER CENTRE",
	"CZE": "CORO",
	"CZF": "CAPE ROMANZOF",
	"CZH": "COROZAL",
	"CZJ": "CORAZON DE JESUS",
	"CZK": "CASCADE LOCKS",
	"CZL": "CONSTANTINE",
	"CZM": "COZUMEL",
	"CZN": "CHISANA",
	"CZO": "CHISTOCHINA",
	"CZP": "CAPE POLE",
	"CZS": "CRUZEIRO DO SUL",
	"CZT": "CARRIZO SPRINGS",
	"CZU": "COROZAL",
	"CZW": "CZESTOCHOWA",
	"CZX": "CHANGZHOU",
	"CZY": "CLUNY",
	"DAB": "DAYTONA BEACH",
	"DAC": "DACCA",
	"DAD": "DANANG",
	"DAE": "DAPARIZO",
	"DAF": "DAUP",
	"DAG": "DAGGETT",
	"DAH": "DATHINA",
	"DAJ": "DAUAN ISLAND",
	"DAL": "DALLAS/FT.WORTH",
	"DAM": "DAMASCUS",
	"DAN": "DANVILLE",
	"DAP": "DARCHULA",
	"DAR": "DAR ES SALAAM",
	"DAS": "GREAT BEAR LAKE",
	"DAT": "DATONG",
	"DAU": "DARU",
	"DAV": "DAVID",
	"DAX": "DAXIAN",
	"DAY": "DAYTON",
	"DAZ": "DARWAZ",
	"DBA": "DALBANDIN",
	"DBD": "DHANBAD",
	"DBM": "DEBRA MARCOS",
	"DBN": "DUBLIN",
	"DBO": "DUBBO",
	"DBP": "DEBEPARE",
	"DBQ": "DUBUQUE",
	"DBS": "DUBOIS",
	"DBT": "DEBRA TABOR",
	"DBV": "DUBROVNIK",
	"DBY": "DALBY",
	"DCA": "WASHINGTON",
	"DCF": "DOMINICA",
	"DCI": "DECIMOMANNU",
	"DCK": "DAHL CREEK",
	"DCM": "CASTRES",
	"DCR": "DECATUR",
	"DCS": "DONCASTER",
	"DCT": "DUNCAN TOWN",
	"DCU": "DECATUR",
	"DDC": "DODGE CITY",
	"DDG": "DANDONG",
	"DDI": "DAYDREAM ISLAND",
	"DDM": "DODOIMA",
	"DDN": "DELTA DOWNS",
	"DDP": "DORADO",
	"DDU": "DADU",
	"DEB": "DEBRECEN",
	"DEC": "DECATUR",
	"DED": "DEHRADUN",
	"DEH": "DECORAH",
	"DEI": "DENIS IS.",
	"DEL": "DELHI",
	"DEM": "DEMBIDOLLO",
	"DEN": "DENVER",
	"DEO": "DEARBORN",
	"DER": "DERIM",
	"DES": "DESROCHES",
	"DET": "DETROIT",
	"DEZ": "DEIR EZZOR",
	"DFB": "DRIFTWOOD BAY",
	"DFI": "DEFIANCE",
	"DFP": "DRUMDUFF",
	"DFW": "DALLAS/FT.WORTH",
	"DGA": "DANGRIGA",
	"DGB": "DANGER BAY",
	"DGC": "DEGAHBUR",
	"DGE": "MUDGEE",
	"DGF": "DOUGLAS LAKE",
	"DGL": "DOUGLAS",
	"DGO": "DURANGO",
	"DGP": "DAUGAVPILS",
	"DGR": "DARGAVILLE",
	"DGT": "DUMAGUETE",
	"DGU": "DEDOUGOU",
	"DGW": "DOUGLAS",
	"DHA": "DHAHRAN",
	"DHD": "DURHAM DOWNS",
	"DHF": "ABU DHABI",
	"DHI": "DHANGARHI",
	"DHL": "DHALA",
	"DHM": "DHARAMSALA",
	"DHN": "DOTHAN",
	"DHR": "DEN HELDER",
	"DHT": "DALHART",
	"DIB": "DIBRUGARH",
	"DIE": "DIEGO SUAREZ",
	"DIJ": "DIJON",
	"DIK": "DICKINSON",
	"DIL": "DILI",
	"DIM": "DIMBOKRO",
	"DIN": "DIEN-BIEN-PHU",
	"DIO": "DIOMEDE ISLAND",
	"DIP": "DIAPAGA",
	"DIQ": "DIVINOPOLIS",
	"DIR": "DIRE DAWA",
	"DIS": "LOUBOMO",
	"DIU": "DIU",
	"DIV": "DIVO",
	"DIY": "DIYARBAKIR",
	"DJA": "DJOUGOU",
	"DJB": "JAMBI",
	"DJE": "DJERBA",
	"DJG": "DJANET",
	"DJJ": "JAYAPURA",
	"DJM": "DJAMBALA",
	"DJN": "DELTA JUNCTION",
	"DJO": "DALOA",
	"DJU": "DJUPIVOGUR",
	"DKI": "DUNK ISLAND",
	"DKK": "DUNKIRK",
	"DKR": "DAKAR",
	"DKS": "DIKSON",
	"DKV": "DOCKER RIVER",
	"DLA": "DOUALA",
	"DLB": "DALBERTIS",
	"DLC": "DALIAN",
	"DLD": "GEILO",
	"DLE": "DOLE",
	"DLF": "DEL RIO",
	"DLG": "DILLINGHAM",
	"DLH": "DULUTH",
	"DLI": "DALAT",
	"DLK": "DULKANINNA",
	"DLL": "DILLON",
	"DLM": "DALAMAN",
	"DLN": "DILLON",
	"DLO": "DOLOMITE",
	"DLS": "THE DALLES",
	"DLU": "DALI CITY",
	"DLV": "DELISSAVILLE",
	"DLY": "DILLONS BAY",
	"DLZ": "DALANZADGAD",
	"DMA": "TUCSON",
	"DMB": "DZHAMBUL",
	"DMD": "DOOMADGEE MISSION",
	"DME": "MOSCOW",
	"DMK": "BANGKOK",
	"DMO": "SEDALIA",
	"DMR": "DHAMAR",
	"DMT": "DIAMANTINO",
	"DMU": "DIMAPUR",
	"DNA": "KADENA AFB",
	"DNB": "DUNBAR",
	"DNC": "DANANE",
	"DND": "DUNDEE",
	"DNF": "DERNA",
	"DNH": "DUNHUANG",
	"DNI": "WAD MEDANI",
	"DNK": "DNEPROPETROVSK",
	"DNL": "AUGUSTA",
	"DNM": "DENHAM",
	"DNN": "DALTON",
	"DNO": "DIANOPOLIS",
	"DNP": "DANG",
	"DNQ": "DENILIQUIN",
	"DNR": "DINARD",
	"DNS": "DENISON",
	"DNU": "DINANGAT",
	"DNV": "DANVILLE",
	"DNX": "DINDER",
	"DNZ": "DENIZLI",
	"DOA": "DOANY",
	"DOB": "DOBO",
	"DOC": "DORNOCH",
	"DOD": "DODOMA",
	"DOE": "DJOEMOE",
	"DOF": "DORA BAY",
	"DOG": "DONGOLA",
	"DOH": "DOHA",
	"DOI": "DOINI",
	"DOK": "DONETSK",
	"DOL": "DEAUVILLE",
	"DOM": "DOMINICA",
	"DON": "DOS LAGUNAS",
	"DOO": "DOROBISORO",
	"DOP": "DOLPA",
	"DOR": "DORI",
	"DOS": "DIOS",
	"DOU": "DOURADOS",
	"DOV": "DOVER",
	"DOX": "DONGARA",
	"DPA": "CHICAGO",
	"DPE": "DIEPPE",
	"DPG": "DUGWAY",
	"DPK": "DEER PARK",
	"DPL": "DIPOLOG",
	"DPO": "DEVONPORT",
	"DPS": "DENPASAR",
	"DPU": "DUMPU",
	"DRA": "MERCURY",
	"DRB": "DERBY",
	"DRC": "DIRICO",
	"DRD": "DORUNDA STATION",
	"DRE": "DRUMMOND ISLAND",
	"DRF": "DRIFT RIVER",
	"DRG": "DEERING",
	"DRH": "DABRA",
	"DRJ": "DRIETABBETJE",
	"DRM": "DRAMA",
	"DRN": "DIRRANBANDI",
	"DRO": "DURANGO",
	"DRR": "DURRIE",
	"DRS": "DRESDEN",
	"DRT": "DEL RIO",
	"DRU": "DRUMMOND",
	"DRW": "DARWIN",
	"DSC": "DSCHANG",
	"DSD": "LA DESIRADE",
	"DSE": "DESSIE",
	"DSI": "DESTIN",
	"DSK": "DERA ISMAIL KHAN",
	"DSL": "DARU",
	"DSM": "DES MOINES",
	"DSN": "DONGSHENG",
	"DSV": "DANSVILLE",
	"DTA": "DELTA",
	"DTD": "DATADAWAI",
	"DTE": "DAET",
	"DTH": "DEATH VALLEY",
	"DTL": "DETROIT LAKES",
	"DTM": "DORTMUND",
	"DTR": "DECATUR ISLAND",
	"DTT": "DETROIT",
	"DTW": "DETROIT",
	"DUA": "DURANT",
	"DUB": "DUBLIN",
	"DUC": "DUNCAN",
	"DUD": "DUNEDIN",
	"DUE": "DUNDO",
	"DUF": "DUCK",
	"DUG": "DOUGLAS",
	"DUI": "DUISBURG",
	"DUJ": "DU BOIS",
	"DUK": "DUKUDUK",
	"DUM": "DUMAI",
	"DUN": "DUNDAS",
	"DUQ": "DUNCAN/QUAMICHAN LAKE",
	"DUR": "DURBAN",
	"DUS": "DUSSELDORF",
	"DUT": "DUTCH HARBOR",
	"DVA": "DEVA",
	"DVL": "DEVILS LAKE",
	"DVN": "DAVENPORT",
	"DVO": "DAVAO",
	"DVP": "DAVENPORT DOWNS",
	"DVR": "DALY RIVER",
	"DVT": "PHOENIX",
	"DVX": "DOVER",
	"DWA": "DWANGWA",
	"DWB": "SOALALA",
	"DXB": "DUBAI",
	"DXD": "DIXIE",
	"DXR": "DANBURY",
	"DYA": "DYSART",
	"DYG": "DAYONG",
	"DYL": "DOYLESTOWN",
	"DYM": "DIAMANTINA LAKES",
	"DYR": "ANADYR",
	"DYS": "ABILENE",
	"DYU": "DUSHANBE",
	"DYW": "DALY WATERS",
	"DZA": "DZAOUDZI",
	"DZI": "CODAZZI",
	"DZN": "DZEZKAZGAN",
	"DZO": "DURAZNO",
	"DZU": "DAZU",
	"EAA": "EAGLE",
	"EAB": "ABBSE",
	"EAE": "EMAE",
	"EAM": "NEJRAN",
	"EAN": "WHEATLAND",
	"EAR": "KEARNEY",
	"EAS": "SAN SEBASTIAN",
	"EAT": "WENATCHEE",
	"EAU": "EAU CLAIRE",
	"EBA": "ELBA ISLAND",
	"EBB": "ENTEBBE/KAMPALA",
	"EBD": "EL OBEID",
	"EBG": "EL BAGRE",
	"EBJ": "ESBJERG",
	"EBM": "EL BORMA",
	"EBN": "EBADON",
	"EBO": "EBON",
	"EBS": "WEBSTER CITY",
	"EBU": "ST. ETIENNE",
	"EBW": "EBOLOWA",
	"ECA": "EAST TAWAS",
	"ECG": "ELIZABETH CITY",
	"ECH": "ECHUCA",
	"ECN": "ERCAN",
	"ECO": "EL ENCANTO",
	"ECR": "EL CHARCO",
	"ECS": "NEWCASTLE",
	"EDA": "EDNA BAY",
	"EDB": "EL DEBBA",
	"EDD": "ERLDUNDA",
	"EDE": "EDENTON",
	"EDF": "ANCHORAGE",
	"EDI": "EDINBURGH",
	"EDK": "EL DORADO",
	"EDL": "ELDORET",
	"EDM": "LA ROCHE",
	"EDQ": "ERANDIQUE",
	"EDR": "EDWARD RIVER",
	"EDW": "EDWARDS",
	"EED": "NEEDLES",
	"EEK": "EEK",
	"EEN": "KEENE",
	"EFB": "EIGHT FATHOM BIGHT",
	"EFD": "HOUSTON",
	"EFG": "EFOGI",
	"EFK": "NEWPORT",
	"EFL": "ARGOSTOLION",
	"EFW": "JEFFERSON",
	"EGA": "ENGATI",
	"EGC": "BERGERAC",
	"EGE": "EAGLE",
	"EGI": "VALPARAISO",
	"EGL": "NEGHELLI",
	"EGM": "SEGE",
	"EGN": "EL GENEINA",
	"EGO": "BELGOROD",
	"EGP": "EAGLE PASS",
	"EGS": "EGILSSTADIR",
	"EGV": "EAGLE RIVER",
	"EGX": "EGEGIK",
	"EHL": "EL BOLSON",
	"EHM": "CAPE NEWENHAM",
	"EHT": "EAST HARTFORD",
	"EIA": "EIA",
	"EIE": "ENISEYSK",
	"EIH": "EINASLEIGH",
	"EIL": "FAIRBANKS",
	"EIN": "EINDHOVEN",
	"EIS": "TORTOLA",
	"EIY": "EIN YAHAV",
	"EJA": "BARRANCABERMEJA",
	"EJH": "WEDJH",
	"EKA": "EUREKA/ARCATA",
	"EKB": "EKIBASTUZ",
	"EKD": "ELKEDRA",
	"EKE": "EKEREKU",
	"EKI": "ELKHART",
	"EKN": "ELKINS",
	"EKO": "ELKO",
	"EKT": "ESKILSTUNA",
	"EKX": "ELIZABETHTOWN/FT. KNOX",
	"ELA": "EAGLE LAKE",
	"ELB": "EL BANCO",
	"ELC": "ELCHO ISLAND",
	"ELD": "EL DORADO",
	"ELE": "EL REAL",
	"ELF": "EL FASHER",
	"ELG": "EL GOLEA",
	"ELH": "NORTH ELEUTHERA",
	"ELI": "ELIM",
	"ELJ": "EL RECREO",
	"ELK": "ELK CITY",
	"ELL": "ELLISRAS",
	"ELM": "ELMIRA",
	"ELN": "ELLENSBURG",
	"ELO": "EL DORADO",
	"ELP": "EL PASO",
	"ELQ": "GASSIM",
	"ELR": "ELELIM",
	"ELS": "EAST LONDON",
	"ELT": "EL TOR",
	"ELU": "EL OUED",
	"ELV": "ELFIN COVE",
	"ELW": "ELLAMAR",
	"ELY": "ELY",
	"ELZ": "WELLSVILLE",
	"EMA": "EAST MIDLANDS",
	"EMD": "EMERALD",
	"EME": "EMDEN",
	"EMG": "EMPANGENI",
	"EMI": "EMIRAU",
	"EMK": "EMMONAK",
	"EMM": "KEMMERER",
	"EMN": "NEMA",
	"EMO": "EMO",
	"EMP": "EMPORIA",
	"EMS": "EMBESSA",
	"EMT": "EL MONTE",
	"EMX": "EL MAITEN",
	"EMY": "EL MINYA",
	"ENA": "KENAI",
	"ENB": "ENEABBA",
	"ENC": "NANCY",
	"END": "ENID",
	"ENE": "ENDE",
	"ENF": "ENONTEKIO",
	"ENH": "ENSHI",
	"ENI": "EL NIDO",
	"ENJ": "EL NARANJO",
	"ENK": "ENNISKILLEN",
	"ENL": "CENTRALIA",
	"ENN": "NENANA",
	"ENO": "ENCARNACION",
	"ENQ": "CORONEL E. SOTO CANO AB",
	"ENS": "ENSCHEDE",
	"ENT": "ENIWETOK",
	"ENU": "ENUGU",
	"ENV": "WENDOVER",
	"ENW": "KENOSHA",
	"ENY": "YANAN",
	"EOH": "MEDELLIN-HERRERA",
	"EOI": "EDAY",
	"EOK": "KEOKUK",
	"EOR": "EL DORADO",
	"EOS": "NEOSHO",
	"EOZ": "ELORZA",
	"EPG": "WEEPING WATER",
	"EPH": "EPHRATA",
	"EPI": "EPI",
	"EPK": "EPISKOPI",
	"EPL": "EPINAL",
	"EPN": "EPENA",
	"EPR": "ESPERANCE",
	"EPT": "ELIPTAMIN",
	"EQS": "ESQUEL",
	"ERA": "ERIGAVO",
	"ERB": "ERNABELLA",
	"ERC": "ERZINCAN",
	"ERD": "BERDYANSK",
	"ERE": "ERAVE",
	"ERF": "ERFURT",
	"ERH": "ERRACHIDIA",
	"ERI": "ERIE",
	"ERM": "ERECHIM",
	"ERN": "EIRUNEPE",
	"ERR": "ERROL",
	"ERS": "WINDHOEK",
	"ERT": "ERDENET",
	"ERU": "ERUME",
	"ERV": "KERRVILLE",
	"ERZ": "ERZURUM",
	"ESA": "ESA'ALA",
	"ESB": "ANKARA",
	"ESC": "ESCANABA",
	"ESD": "EASTSOUND",
	"ESE": "ENSENADA",
	"ESF": "ALEXANDRIA",
	"ESG": "MARISCAL ESTIGARRIBIA",
	"ESH": "SHOREHAM",
	"ESI": "ESPINOSA",
	"ESK": "ESKISEHIR",
	"ESL": "ELISTA",
	"ESM": "ESMERALDAS",
	"ESN": "EASTON",
	"ESO": "ESPANOLA",
	"ESP": "EAST STROUDSBURG",
	"ESR": "EL SALVADOR",
	"ESS": "ESSEN",
	"EST": "ESTHERVILLE",
	"ESW": "EASTON",
	"ETB": "WEST BEND",
	"ETD": "ETADUNNA",
	"ETE": "METEMMA",
	"ETH": "ELAT",
	"ETN": "EASTLAND",
	"ETS": "ENTERPRISE",
	"ETZ": "METZ/NANCY",
	"EUA": "EUA",
	"EUC": "EUCLA",
	"EUE": "EUREKA",
	"EUF": "EUFAULA",
	"EUG": "EUGENE",
	"EUM": "NEUMUNSTER",
	"EUN": "EL AAIUN",
	"EUX": "ST. EUSTATIUS",
	"EVA": "EVADALE",
	"EVD": "EVA DOWNS",
	"EVE": "EVENES",
	"EVG": "SVEG",
	"EVH": "EVANS HEAD",
	"EVM": "EVELETH",
	"EVN": "EREVAN",
	"EVV": "EVANSVILLE",
	"EVW": "EVANSTON",
	"EVX": "EVREUX",
	"EWB": "NEW BEDFORD",
	"EWE": "EWER",
	"EWI": "ENAROTALI",
	"EWK": "NEWTON",
	"EWN": "NEW BERN",
	"EWO": "EWO",
	"EWR": "NEWARK",
	"EXI": "EXCURSION INLET",
	"EXM": "EXMOUTH GULF",
	"EXT": "EXETER",
	"EYL": "YELIMANE",
	"EYP": "EL YOPAL",
	"EYR": "YERINGTON",
	"EYS": "ELIYE SPRINGS",
	"EYW": "KEY WEST",
	"EZE": "BUENOS AIRES",
	"EZS": "ELAZIG",
	"FAA": "FARANAH",
	"FAB": "FARNBOROUGH",
	"FAC": "FAAITE",
	"FAE": "FAEROE ISLANDS",
	"FAG": "FAGURHOLSMYRI",
	"FAH": "FARAH",
	"FAI": "FAIRBANKS",
	"FAJ": "FAJARDO",
	"FAK": "FALSE ISLAND",
	"FAL": "ROMA",
	"FAM": "FARMINGTON",
	"FAN": "FARSUND",
	"FAO": "FARO",
	"FAR": "FARGO",
	"FAS": "FASKRUDSFJORDUR",
	"FAT": "FRESNO",
	"FAV": "FAKARAVA",
	"FAY": "FAYETTEVILLE",
	"FBD": "FAIZABAD",
	"FBE": "FRANCISCO BELTRAO",
	"FBK": "FAIRBANKS",
	"FBL": "FARIBAULT",
	"FBM": "LUBUMBASHI",
	"FBR": "FORT BRIDGER",
	"FBU": "OSLO-FORNEBU",
	"FBY": "FAIRBURY",
	"FCA": "KALISPELL",
	"FCB": "FICKSBURG",
	"FCH": "FRESNO",
	"FCM": "MINNEAPOLIS/ST. PAUL",
	"FCO": "ROME",
	"FCY": "FORREST CITY",
	"FDE": "FORDE",
	"FDF": "FORT DE FRANCE",
	"FDH": "FRIEDRICHSHAFEN",
	"FDK": "FREDERICK",
	"FDR": "FREDERICK",
	"FDU": "BANDUNDU",
	"FDY": "FINDLAY",
	"FEA": "FETLAR",
	"FEB": "SANFEBAGAR",
	"FEG": "FERGANA",
	"FEK": "FERKESSEDOUGOU",
	"FEL": "FURSTENFELDBRUCK",
	"FEN": "FERNANDO DE NORONHA",
	"FEP": "FREEPORT",
	"FET": "FREMONT",
	"FEW": "CHEYENNE",
	"FEZ": "FEZ",
	"FFA": "KILL DEVIL HILLS",
	"FFD": "FAIRFORD",
	"FFL": "FAIRFIELD",
	"FFM": "FERGUS FALLS",
	"FFO": "DAYTON",
	"FFT": "FRANKFORT",
	"FFU": "FUTALEUFU",
	"FGD": "F'DERIK",
	"FGI": "APIA",
	"FGL": "FOX GLACIER",
	"FGU": "FANGATAU",
	"FHU": "FT. HUACHUCA",
	"FHZ": "FAKAHINA",
	"FIC": "FIRE COVE",
	"FID": "FISHERS ISLAND",
	"FIE": "FAIR ISLE",
	"FIG": "FRIA",
	"FIH": "KINSHASA",
	"FIK": "FINKE",
	"FIL": "FILLMORE",
	"FIN": "FINSCHHAFEN",
	"FIV": "FIVE FINGER",
	"FIZ": "FITZROY CROSSING",
	"FJR": "AL-FUJAIRAH",
	"FKI": "KISANGANI",
	"FKJ": "FUKUI",
	"FKL": "OIL CITY/FRANKLIN",
	"FKN": "FRANKLIN",
	"FKQ": "FAK-FAK",
	"FKS": "FUKUSHIMA",
	"FLA": "FLORENCIA",
	"FLB": "FLORIANO",
	"FLC": "FALLS CREEK",
	"FLD": "FOND DU LAC",
	"FLF": "FLENSBURG",
	"FLG": "FLAGSTAFF",
	"FLH": "FLOTTA ISLE",
	"FLI": "FLATEYRI",
	"FLJ": "FALLS BAY",
	"FLL": "FT. LAUDERDALE",
	"FLM": "FILADELFIA",
	"FLN": "FLORIANOPOLIS",
	"FLO": "FLORENCE",
	"FLP": "FLIPPIN",
	"FLR": "FLORENCE",
	"FLS": "FLINDERS IS.",
	"FLT": "FLAT",
	"FLW": "SANTA CRUZ",
	"FLX": "FALLON",
	"FLY": "FINLEY",
	"FMA": "FORMOSA",
	"FMC": "FIVE MILE",
	"FMG": "FLAMINGO",
	"FMH": "FALMOUTH",
	"FMI": "KALEMIE",
	"FMN": "FARMINGTON",
	"FMO": "MUENSTER",
	"FMS": "FT. MADISON",
	"FMY": "FT. MYERS",
	"FNA": "FREETOWN",
	"FNC": "FUNCHAL",
	"FNE": "FANE",
	"FNG": "FADA NGOURMA",
	"FNH": "FINCHA",
	"FNI": "NIMES",
	"FNJ": "PYONGYANG",
	"FNK": "FIN CREEK",
	"FNL": "FORT COLLINS/LOVELAND",
	"FNR": "FUNTER BAY",
	"FNT": "FLINT",
	"FOA": "FOULA",
	"FOB": "FORT BRAGG",
	"FOC": "FUZHOU",
	"FOD": "FT. DODGE",
	"FOE": "TOPEKA",
	"FOG": "FOGGIA",
	"FOK": "WESTHAMPTON",
	"FOM": "FOUMBAN",
	"FOO": "NUMFUR",
	"FOR": "FORTALEZA",
	"FOS": "FORREST",
	"FOT": "FORSTER",
	"FOU": "FOUGAMOU",
	"FOX": "FOX",
	"FOY": "FOYA",
	"FPO": "FREEPORT",
	"FPR": "FT. PIERCE",
	"FPY": "PERRY",
	"FRA": "FRANKFURT",
	"FRB": "FORBES",
	"FRC": "FRANCA",
	"FRD": "FRIDAY HARBOR",
	"FRE": "FERA IS.",
	"FRF": "FRANKFURT",
	"FRG": "FARMINGDALE",
	"FRH": "FRENCH LICK",
	"FRJ": "FREJUS",
	"FRK": "FREGATE IS.",
	"FRL": "FORLI",
	"FRM": "FAIRMONT",
	"FRO": "FLORO",
	"FRP": "FRESH WATER BAY",
	"FRQ": "FERAMIN",
	"FRR": "FRONT ROYAL",
	"FRS": "FLORES",
	"FRT": "FRUTILLAR",
	"FRU": "BISHKEK",
	"FRV": "FRANCEVILLE",
	"FRW": "FRANCISTOWN",
	"FRY": "FRYEBURG",
	"FSC": "FIGARI",
	"FSD": "SIOUX FALLS",
	"FSI": "FORT SILL",
	"FSK": "FT. SCOTT",
	"FSL": "FOSSIL DOWNS",
	"FSM": "FT. SMITH",
	"FSP": "ST. PIERRE",
	"FSS": "FORRES",
	"FST": "FT. STOCKTON",
	"FSU": "FORT SUMNER",
	"FTA": "FUTUNA ISLAND",
	"FTI": "FITIUTA",
	"FTL": "FORTUNA LEDGE",
	"FTU": "FT. DAUPHIN",
	"FTW": "DALLAS/FT.WORTH",
	"FTX": "OWANDO",
	"FTY": "ATLANTA",
	"FUB": "FULLEBORN",
	"FUE": "FUERTEVENTURA",
	"FUG": "FUYANG",
	"FUJ": "FUKUE",
	"FUK": "FUKUOKA",
	"FUL": "FULLERTON",
	"FUM": "FUMA",
	"FUN": "FUNAFUTI",
	"FUO": "FUOSHAN",
	"FUT": "FUTUNA",
	"FVA": "TUZLA",
	"FVB": "BALEDOGLE",
	"FVC": "BAIKONOUR",
	"FVD": "SONORA",
	"FVE": "FALLBROOK",
	"FWA": "FT. WAYNE",
	"FWH": "DALLAS/FT.WORTH",
	"FWL": "FAREWELL",
	"FWM": "FORT WILLIAM",
	"FXE": "FT. LAUDERDALE",
	"FXM": "FLAXMAN ISLAND",
	"FXO": "CUAMBA",
	"FXY": "FOREST CITY",
	"FYM": "FAYETTEVILLE",
	"FYN": "FUYUN",
	"FYT": "FAYA",
	"FYU": "FT. YUKON",
	"FYV": "FAYETTEVILLE",
	"FZO": "FILTON",
	"GAA": "GUAMAL",
	"GAB": "GABBS",
	"GAC": "GRACIAS",
	"GAD": "GADSDEN",
	"GAE": "GABES",
	"GAF": "GAFSA",
	"GAG": "GAGE",
	"GAH": "GAYNDAH",
	"GAI": "GAITHERSBURG",
	"GAJ": "YAMAGATA",
	"GAK": "GAKONA",
	"GAL": "GALENA",
	"GAM": "GAMBELL",
	"GAN": "GAN ISLAND",
	"GAO": "GUANTANAMO",
	"GAP": "G",
	"GAQ": "GAO",
	"GAR": "GARAINA",
	"GAS": "GARISSA",
	"GAT": "GAP",
	"GAU": "GAUHATI",
	"GAV": "GAG ISLAND",
	"GAW": "GANGAW",
	"GAX": "GAMBA",
	"GAY": "GAYA",
	"GAZ": "GUASOPA",
	"GBA": "BIG BAY",
	"GBB": "GARA DJEBILET",
	"GBC": "GASUKE",
	"GBD": "GREAT BEND",
	"GBE": "GABORONE",
	"GBF": "NEGARBO",
	"GBG": "GALESBURG",
	"GBH": "GALBRAITH LAKE",
	"GBJ": "MARIE GALANTE",
	"GBK": "GBANGBATOK",
	"GBL": "GOULBURN IS.",
	"GBM": "GARBAHAREY",
	"GBO": "BALTIMORE",
	"GBP": "GAMBOOLA",
	"GBR": "GREAT BARRINGTON",
	"GBU": "KHASHM EL GIRBA",
	"GBV": "GIBB RIVER",
	"GBY": "GALKAYO",
	"GBZ": "GREAT BARRIER IS.",
	"GCA": "GUACAMAYAS",
	"GCC": "GILLETTE",
	"GCI": "GUERNSEY",
	"GCJ": "JOHANNESBURG-GRAND CNTL",
	"GCK": "GARDEN CITY",
	"GCM": "GRAND CAYMAN",
	"GCN": "GRAND CANYON",
	"GCY": "GREENVILLE",
	"GDA": "GOUNDA",
	"GDC": "GREENVILLE/SPARTANBURG",
	"GDD": "GORDON DOWNS",
	"GDE": "GODE",
	"GDG": "MAGDAGACHI",
	"GDH": "GOLDEN HORN",
	"GDI": "GORDIL",
	"GDJ": "GANDAJIKA",
	"GDL": "GUADALAJARA",
	"GDM": "GARDNER",
	"GDN": "GDANSK",
	"GDO": "GUASDUALITO",
	"GDP": "GUADALUPE",
	"GDQ": "GONDAR",
	"GDT": "GRAND TURK",
	"GDV": "GLENDIVE",
	"GDW": "GLADWIN",
	"GDX": "MAGADAN",
	"GDZ": "GELENDZIK",
	"GEA": "NOUMEA",
	"GEB": "GEBE",
	"GEC": "GECITKALE",
	"GED": "GEORGETOWN",
	"GEE": "GEORGE TOWN",
	"GEF": "GEVA",
	"GEG": "SPOKANE",
	"GEI": "GREEN ISLANDS",
	"GEK": "GANES CREEK",
	"GEL": "SANTO ANGELO",
	"GEN": "OSLO",
	"GEO": "GEORGETOWN",
	"GER": "NUEVA GERONA",
	"GES": "GENERAL SANTOS",
	"GET": "GERALDTON",
	"GEV": "GALLIVARE",
	"GEW": "GEWOYA",
	"GEX": "GEELONG",
	"GEY": "GREYBULL",
	"GFA": "GREAT FALLS",
	"GFB": "TOGIAK FISH",
	"GFD": "GREENFIELD",
	"GFE": "GRENFELL",
	"GFF": "GRIFFITH",
	"GFK": "GRAND FORKS",
	"GFL": "GLENS FALLS",
	"GFN": "GRAFTON",
	"GFO": "BARTICA",
	"GFR": "GRANVILLE",
	"GFY": "GROOTFONTEIN",
	"GGC": "LUMBALA",
	"GGD": "GREGORY DOWNS",
	"GGE": "GEORGETOWN",
	"GGG": "LONGVIEW",
	"GGL": "GILGAL",
	"GGN": "GAGNOA",
	"GGO": "GUIGLO",
	"GGR": "GAROE",
	"GGS": "GOBERNADOR GREGORES",
	"GGT": "GEORGE TOWN",
	"GGW": "GLASGOW",
	"GHA": "GHARDAIA",
	"GHB": "GOVERNORS HARBOUR",
	"GHC": "GREAT HARBOUR CAY",
	"GHD": "GHIMBI",
	"GHE": "GARACHINE",
	"GHK": "GUSH KATIF",
	"GHL": "GARDEN HILL",
	"GHM": "CENTERVILLE",
	"GHN": "GUANGHAN",
	"GHO": "GRAHAMSTOWN",
	"GHT": "GHAT",
	"GHU": "GUALEGUAYCHU",
	"GIB": "GIBRALTAR",
	"GIC": "BOIGU ISLAND",
	"GID": "GITEGA",
	"GIF": "WINTER HAVEN",
	"GIG": "RIO DE JANEIRO",
	"GII": "SIGUIRI",
	"GIL": "GILGIT",
	"GIM": "MIELE",
	"GIO": "GEILO",
	"GIR": "GIRARDOT",
	"GIS": "GISBORNE",
	"GIT": "GEITA",
	"GIY": "GIYANI",
	"GIZ": "GIZAN",
	"GJA": "GUANAJA IS.",
	"GJL": "JIJEL",
	"GJM": "GUAJARA-MIRIM",
	"GJR": "GJOGUR",
	"GJT": "GRAND JUNCTION",
	"GKA": "GOROKA",
	"GKE": "GEILENKIRCHEN",
	"GKH": "GORKHA",
	"GKL": "GREAT KEPPEL ISLAND",
	"GKN": "GULKANA",
	"GKO": "KONGO-BOUMBA",
	"GKT": "GATLINBURG",
	"GLA": "GLASGOW",
	"GLB": "GILBUES",
	"GLC": "GELADI",
	"GLD": "GOODLAND",
	"GLE": "GAINESVILLE",
	"GLF": "GOLFITO",
	"GLG": "GLENGYLE",
	"GLH": "GREENVILLE",
	"GLI": "GLEN INNES",
	"GLK": "GALCAIO",
	"GLL": "GOL",
	"GLM": "GLENORMISTON",
	"GLN": "GOULIMIME",
	"GLO": "GLOUCESTER/CHELTENHAM",
	"GLP": "GULGUBIP",
	"GLQ": "GLENNALLEN",
	"GLR": "GAYLORD",
	"GLS": "GALVESTON",
	"GLT": "GLADSTONE",
	"GLV": "GOLOVIN",
	"GLW": "GLASGOW",
	"GLX": "GALELA",
	"GLY": "MT. GOLDSWORTHY",
	"GLZ": "BREDA",
	"GMA": "GEMENA",
	"GMB": "GAMBELA",
	"GMC": "GUERIMA",
	"GME": "GOMEL",
	"GMI": "GASMATA",
	"GMM": "GAMBOMA",
	"GMN": "GREYMOUTH",
	"GMR": "GAMBIER IS.",
	"GMS": "GUIMARAES",
	"GMT": "GRANITE MOUNTAIN",
	"GMU": "GREENVILLE/SPARTANBURG",
	"GMV": "MONUMENT VALLEY GOULDINGS",
	"GNA": "GRODNO",
	"GNB": "GRENOBLE",
	"GND": "GRENADA",
	"GNE": "GHENT",
	"GNG": "GOODING",
	"GNI": "GREEN ISLAND",
	"GNM": "GUANAMBI",
	"GNN": "GHINNIR",
	"GNR": "GENERAL ROCA",
	"GNS": "GUNUNGSITOLI",
	"GNT": "GRANTS",
	"GNU": "GOODNEWS BAY",
	"GNV": "GAINESVILLE",
	"GNY": "GRANBY",
	"GNZ": "GHANZI",
	"GOA": "GENOA",
	"GOB": "GOBA",
	"GOC": "GORA",
	"GOE": "GONALIA",
	"GOH": "GODTHAB",
	"GOI": "GOA",
	"GOJ": "NIZHNIY NOVGOROD (GORKIJ)",
	"GOK": "GUTHRIE",
	"GOL": "GOLD BEACH",
	"GOM": "GOMA",
	"GON": "NEW LONDON",
	"GOO": "GOONDIWINDI",
	"GOP": "GORAKHPUR",
	"GOQ": "GOLMUD",
	"GOR": "GORE",
	"GOS": "GOSFORD",
	"GOT": "GOTHENBURG",
	"GOU": "GAROUA",
	"GOV": "GOVE",
	"GOY": "GAL OYA",
	"GOZ": "GORNA ORJACHOVICA",
	"GPA": "PATRAS",
	"GPB": "GUARAPUAVA",
	"GPI": "GUAPI",
	"GPL": "GUAPILES",
	"GPN": "GARDEN POINT",
	"GPO": "GENERAL PICO",
	"GPS": "GALAPAGOS ISLANDS",
	"GPT": "GULFPORT/BILOXI",
	"GPZ": "GRAND RAPIDS",
	"GQQ": "GALION",
	"GRA": "GAMARRA",
	"GRB": "GREEN BAY",
	"GRC": "GRAND CESS",
	"GRD": "GREENWOOD",
	"GRE": "GREENVILLE",
	"GRF": "FORT LEWIS",
	"GRG": "GARDEZ",
	"GRH": "GARUAHI",
	"GRI": "GRAND ISLAND",
	"GRJ": "GEORGE",
	"GRK": "KILLEEN",
	"GRL": "GRENAATBOSKOLK",
	"GRM": "GRAND MARAIS",
	"GRN": "GORDON",
	"GRO": "GERONA",
	"GRP": "GURUPI",
	"GRQ": "GRONINGEN",
	"GRR": "GRAND RAPIDS",
	"GRS": "GROSSETO",
	"GRT": "GUJRAT",
	"GRU": "SAO PAULO",
	"GRV": "GROZNYJ",
	"GRW": "GRACIOSA ISLAND",
	"GRX": "GRANADA",
	"GRY": "GRIMSEY",
	"GRZ": "GRAZ",
	"GSA": "LONG PASIA",
	"GSB": "GOLDSBORO",
	"GSC": "GASCOYNE JUNCTION",
	"GSE": "GOTHENBURG-SAEVE",
	"GSH": "GOSHEN",
	"GSI": "GUADALCANAL",
	"GSL": "TALTHEILEI NARROWS",
	"GSN": "MOUNT GUNSON",
	"GSO": "GREENSBORO/HIGH POINT",
	"GSP": "GREENVILLE/SPARTANBURG",
	"GSR": "GARDO",
	"GSS": "SABI SABI",
	"GST": "GUSTAVUS",
	"GSU": "GEDAREF",
	"GSY": "GRIMSBY",
	"GTA": "GATOKAE",
	"GTB": "GENTING",
	"GTC": "GREEN TURTLE",
	"GTE": "GROOTE IS.",
	"GTF": "GREAT FALLS",
	"GTG": "GRANTSBURG",
	"GTI": "GUETTIN",
	"GTK": "SUNGEI TEKAI",
	"GTN": "MOUNT COOK-GLENTANNER",
	"GTO": "GORONTALO",
	"GTR": "COLUMBUS",
	"GTS": "GRANITES",
	"GTT": "GEORGETOWN",
	"GTW": "ZLIN",
	"GTY": "GETTYSBURG",
	"GUA": "GUATEMALA CITY",
	"GUB": "GUERRERO NEGRO",
	"GUC": "GUNNISON",
	"GUD": "GOUNDAM",
	"GUE": "GURIASO",
	"GUF": "GULF SHORES",
	"GUG": "GUARI",
	"GUH": "GUNNEDAH",
	"GUI": "GUIRIA",
	"GUJ": "GUARATINGUETA",
	"GUL": "GOULBURN",
	"GUM": "GUAM ISLAND",
	"GUO": "GUALACO",
	"GUP": "GALLUP",
	"GUQ": "GUANARE",
	"GUR": "ALOTAU",
	"GUS": "PERU",
	"GUT": "GUTERSLOH",
	"GUU": "GRUNDARFJORDUR",
	"GUV": "MOUGULU",
	"GUW": "GURYEV",
	"GUX": "GUNA",
	"GUY": "GUYMON",
	"GUZ": "GUIRATINGA",
	"GVA": "GENEVA",
	"GVE": "GORDONSVILLE",
	"GVI": "GREEN RIVER",
	"GVL": "GAINESVILLE",
	"GVP": "GREENVALE",
	"GVR": "GOVERNADOR VALADARES",
	"GVT": "GREENVILLE",
	"GVW": "GRANDVIEW",
	"GVX": "GAVLE",
	"GWA": "GWA",
	"GWD": "GWADAR",
	"GWE": "GWERU",
	"GWL": "GWALIOR",
	"GWN": "GNAROWEIN",
	"GWO": "GREENWOOD",
	"GWS": "GLENWOOD SPRINGS",
	"GWT": "WESTERLAND",
	"GWY": "GALWAY",
	"GXF": "GHURAF",
	"GXG": "NEGAGE",
	"GXH": "MILDENHALL",
	"GXQ": "COYHAIQUE",
	"GXX": "YAGOUA",
	"GXY": "GREELEY",
	"GYA": "GUAYARAMERIN",
	"GYE": "GUAYAQUIL",
	"GYI": "GISENYI",
	"GYL": "ARGYLE",
	"GYM": "GUAYMAS",
	"GYN": "GOIANIA",
	"GYP": "GYMPIE",
	"GYR": "GOODYEAR",
	"GYY": "GARY",
	"GZI": "GHAZNI",
	"GZM": "GOZO",
	"GZO": "GIZO",
	"GZT": "GAZIANTEP",
	"HAA": "HASVIK",
	"HAB": "HAMILTON",
	"HAC": "HACHIJO JIMA ISLAND",
	"HAD": "HALMSTAD",
	"HAE": "HAVASUPAI",
	"HAF": "HALF MOON",
	"HAG": "THE HAGUE",
	"HAH": "MORONI",
	"HAJ": "HANNOVER",
	"HAK": "HAIKOU",
	"HAL": "HALALI",
	"HAM": "HAMBURG",
	"HAN": "HANOI",
	"HAO": "HAMILTON",
	"HAP": "HAPPY BAY",
	"HAQ": "HANIMAADHOO",
	"HAR": "HARRISBURG",
	"HAS": "HAIL",
	"HAT": "HEATHLANDS",
	"HAU": "HAUGESUND",
	"HAV": "HAVANA",
	"HAW": "HAVERFORDWEST",
	"HAY": "HAYCOCK",
	"HAZ": "HATZFELDTHAVEN",
	"HBA": "HOBART",
	"HBB": "HOBBS",
	"HBC": "HANUS BAY",
	"HBH": "HOBART BAY",
	"HBI": "HARBOUR ISLAND",
	"HBK": "HINCHINBROOK",
	"HBN": "PHUBON",
	"HBO": "HUMBOLDT",
	"HBR": "HOBART",
	"HBT": "HAFR ALBATIN",
	"HBX": "HUBLI",
	"HCA": "BIG SPRING",
	"HCB": "SHOAL COVE",
	"HCC": "HUDSON",
	"HCM": "EIL",
	"HCN": "HENGCHUN",
	"HCQ": "HALLS CREEK",
	"HCR": "HOLY CROSS",
	"HCS": "JOHANNESBURG",
	"HCW": "CHERAW",
	"HDA": "HIDDEN FALLS",
	"HDB": "HEIDELBERG",
	"HDD": "HYDERABAD",
	"HDE": "HOLDREGE",
	"HDF": "HERINGSDORF",
	"HDM": "HAMADAN",
	"HDN": "STEAMBOAT SPRINGS",
	"HDS": "HOEDSPRUIT",
	"HDY": "HAT YAI",
	"HEA": "HERAT",
	"HEB": "HENZADA",
	"HED": "HERENDEEN",
	"HEE": "HELENA",
	"HEH": "HEHO",
	"HEI": "HEIDE/BUESUM",
	"HEK": "HEIHE",
	"HEL": "HELSINKI",
	"HEM": "HELSINKI",
	"HEN": "HENDON",
	"HEO": "HAELOGO",
	"HER": "HERAKLION",
	"HES": "HERMISTON",
	"HET": "HOHHOT",
	"HEX": "SANTO DOMINGO",
	"HEZ": "NATCHEZ",
	"HFA": "HAIFA",
	"HFD": "HARTFORD",
	"HFE": "HEFEI",
	"HFF": "HOFFMAN",
	"HFN": "HOFN",
	"HFS": "HAGFORS",
	"HFT": "HAMMERFEST",
	"HGA": "HARGEISA",
	"HGD": "HUGHENDEN",
	"HGH": "HANGZHOU",
	"HGL": "HELGOLAND",
	"HGN": "MAE HONG SON",
	"HGO": "KORHOGO",
	"HGR": "HAGERSTOWN",
	"HGS": "FREETOWN",
	"HGU": "MT. HAGEN",
	"HGZ": "HOGATZA",
	"HHA": "HUANGHUA",
	"HHE": "HACHINOHE",
	"HHH": "HILTON HEAD ISLAND",
	"HHN": "HAHN",
	"HHP": "HONG KONG.",
	"HHQ": "HUA HIN",
	"HHR": "HAWTHORNE",
	"HHZ": "HIKUERU",
	"HIB": "CHISHOLM/HIBBING",
	"HIC": "PRETORIA",
	"HID": "HORN ISLAND",
	"HIE": "WHITEFIELD",
	"HIF": "OGDEN",
	"HIG": "HIGHBURY",
	"HIH": "HOOK ISLAND",
	"HII": "LAKE HAVASU CITY",
	"HIJ": "HIROSHIMA",
	"HIK": "HONOLULU",
	"HIL": "HILONGOS",
	"HIN": "CHINJU",
	"HIO": "HILLSBORO",
	"HIP": "HEADINGLY",
	"HIR": "HONIARA",
	"HIS": "HAYMAN IS",
	"HIT": "HIVARO",
	"HIW": "HIROSHIMA-WEST",
	"HIX": "HIVA OA",
	"HJR": "KHAJURAHO",
	"HJT": "KHUJIRT",
	"HKA": "BLYTHEVILLE",
	"HKB": "HEALY LAKE",
	"HKD": "HAKODATE",
	"HKG": "HONG KONG",
	"HKK": "HOKITIKA",
	"HKN": "HOSKINS",
	"HKP": "KAANAPALI",
	"HKS": "JACKSON",
	"HKT": "PHUKET",
	"HKV": "HASKOVO",
	"HKY": "HICKORY",
	"HLA": "LANSERIA",
	"HLB": "BATESVILLE",
	"HLC": "HILL CITY",
	"HLD": "HAILAR",
	"HLF": "HULTSFRED",
	"HLG": "WHEELING",
	"HLH": "ULANHOT",
	"HLI": "HOLLISTER",
	"HLJ": "SHAULIAJ",
	"HLL": "HILLSIDE",
	"HLM": "HOLLAND",
	"HLN": "HELENA",
	"HLP": "JAKARTA",
	"HLS": "ST. HELENES",
	"HLT": "HAMILTON",
	"HLU": "HOUAILOU",
	"HLV": "HELENVALE",
	"HLW": "HLUHLUWE",
	"HLY": "HOLYHEAD",
	"HLZ": "HAMILTON",
	"HMA": "MALMO-HARBOUR",
	"HME": "HASSI MESSAOUD",
	"HMG": "HERMANNSBURG",
	"HMI": "HAMI",
	"HMJ": "KHMELNITSKIY",
	"HMN": "ALAMOGORDO",
	"HMO": "HERMOSILLO",
	"HMR": "HAMAR",
	"HMS": "HOMESHORE",
	"HMT": "HEMET",
	"HMV": "HEMAVAN",
	"HNA": "MORIOKA",
	"HNB": "HUNTINGBURG",
	"HNC": "HATTERAS",
	"HND": "TOKYO",
	"HNE": "TAHNETA PASS LODGE",
	"HNG": "HIENGHENE",
	"HNH": "HOONAH",
	"HNI": "HEIWENI",
	"HNK": "HINCHINBROOK ISLAND",
	"HNL": "HONOLULU",
	"HNM": "HANA",
	"HNN": "HONINABI",
	"HNS": "HAINES",
	"HNX": "HANNA",
	"HNY": "HENGYANG",
	"HOA": "HOLA",
	"HOB": "HOBBS",
	"HOC": "KOMAKO",
	"HOD": "HODEIDA",
	"HOE": "HOUEISAY",
	"HOF": "HAFUF",
	"HOG": "HOLGUIN",
	"HOH": "HOHENEMS",
	"HOI": "HAO IS.",
	"HOK": "HOOKER CREEK",
	"HOL": "HOLIKACHUK",
	"HOM": "HOMER",
	"HON": "HURON",
	"HOO": "QUANG DUC",
	"HOP": "HOPKINSVILLE",
	"HOQ": "HOF",
	"HOR": "HORTA",
	"HOS": "HOSANA",
	"HOT": "HOT SPRINGS",
	"HOU": "HOUSTON",
	"HOV": "ORSTA/VOLDA",
	"HOW": "FORT KOBBE",
	"HOX": "HOMALIN",
	"HOY": "HOY IS.",
	"HPA": "LIFUKA",
	"HPB": "HOOPER BAY",
	"HPE": "HOPE VALE",
	"HPH": "HAIPHONG",
	"HPN": "WHITE PLAINS",
	"HPR": "PRETORIA",
	"HPT": "HAMPTON",
	"HPV": "HANALEI",
	"HPY": "BAYTOWN",
	"HQM": "HOQUIAM/ABERDEEN",
	"HRA": "MANSEHRA",
	"HRB": "HARBIN",
	"HRD": "HARSTAD",
	"HRE": "HARARE",
	"HRG": "HURGHADA",
	"HRJ": "CHAURJHARI",
	"HRK": "KHARKOV",
	"HRL": "HARLINGEN",
	"HRM": "HASSI R MEL",
	"HRN": "HERON ISLAND",
	"HRO": "HARRISON",
	"HRR": "HERRERA",
	"HRS": "HARRISMITH",
	"HRT": "HARROGATE",
	"HRY": "HENBURY",
	"HSB": "HARRISBURG",
	"HSC": "SHAOGUAN",
	"HSH": "LAS VEGAS",
	"HSI": "HASTINGS",
	"HSL": "HUSLIA",
	"HSM": "HORSHAM",
	"HSP": "HOT SPRINGS",
	"HSS": "HISSAR",
	"HST": "HOMESTEAD",
	"HSV": "HUNTSVILLE/DECATUR",
	"HSZ": "HSINCHU",
	"HTA": "CHITA",
	"HTB": "TERRE-DE-BAS",
	"HTF": "HATFIELD",
	"HTG": "HATANGA",
	"HTH": "HAWTHORNE",
	"HTI": "HAMILTON ISLAND",
	"HTN": "KHOTAN",
	"HTO": "EAST HAMPTON",
	"HTR": "HATERUMA",
	"HTS": "ASHLAND",
	"HTU": "HOPETOUN",
	"HTV": "HUNTSVILLE",
	"HTW": "CHESAPEAKE",
	"HTZ": "HATO COROZAL",
	"HUA": "HUNTSVILLE",
	"HUB": "HUMBERT RIVER",
	"HUC": "HUMACAO",
	"HUD": "HUMBOLDT",
	"HUE": "HUMERA",
	"HUF": "TERRE HAUTE",
	"HUG": "HUEHUETENANGO",
	"HUH": "HUAHINE",
	"HUI": "HUE",
	"HUJ": "HUGO",
	"HUK": "HUKUNTSI",
	"HUL": "HOULTON",
	"HUM": "HOUMA",
	"HUN": "HUALIEN",
	"HUQ": "HOUN",
	"HUS": "HUGHES",
	"HUT": "HUTCHINSON",
	"HUU": "HUANUCO",
	"HUV": "HUDIKSVALL",
	"HUX": "SANTA CRUZ HUATULCO",
	"HUY": "HUMBERSIDE",
	"HUZ": "HUIZHOU",
	"HVA": "ANALALAVA",
	"HVB": "HERVEY BAY",
	"HVD": "KHOVD",
	"HVE": "HANKSVILLE",
	"HVG": "HONNINGSVAG",
	"HVK": "HOLMAVIK",
	"HVM": "HVAMMSTANGI",
	"HVN": "NEW HAVEN",
	"HVR": "HAVRE",
	"HVS": "HARTSVILLE",
	"HWA": "HAWABANGO",
	"HWD": "HAYWARD",
	"HWI": "HAWK INLET",
	"HWK": "HAWKER S. AUSTRALIA",
	"HWN": "HWANGE NATIONAL PARK",
	"HXX": "HAY",
	"HYA": "HYANNIS",
	"HYC": "HIGH WYCOMBE",
	"HYD": "HYDERABAD",
	"HYF": "HAYFIELDS",
	"HYG": "HYDABURG",
	"HYL": "HOLLIS",
	"HYN": "HUANGYAN",
	"HYR": "HAYWARD",
	"HYS": "HAYS",
	"HYT": "HUMAITA",
	"HYV": "HYVINKAA",
	"HZB": "HAZEBROUCK",
	"HZG": "HANZHONG",
	"HZK": "H",
	"HZL": "HAZLETON",
	"HZV": "HAZYVIEW",
	"IAA": "IGARKA",
	"IAB": "WICHITA",
	"IAD": "WASHINGTON",
	"IAG": "NIAGARA FALLS",
	"IAH": "HOUSTON",
	"IAM": "IN AMENAS",
	"IAN": "KIANA",
	"IAQ": "BAHREGAN",
	"IAR": "YAROSLAVL",
	"IAS": "IASI",
	"IAU": "IAURA",
	"IBA": "IBADAN",
	"IBE": "IBAGUE",
	"IBI": "IBOKI",
	"IBO": "IBO",
	"IBP": "IBERIA",
	"IBZ": "IBIZA",
	"ICA": "ICABARU",
	"ICI": "CICIA",
	"ICK": "NIEUW NICKERIE",
	"ICL": "CLARINDA",
	"ICO": "SICOGON ISLAND",
	"ICR": "NICARO",
	"ICT": "WICHITA",
	"ICY": "ICY BAY",
	"IDA": "IDAHO FALLS",
	"IDB": "IDRE",
	"IDF": "IDIOFA",
	"IDG": "IDA GROVE",
	"IDI": "INDIANA",
	"IDK": "INDULKANA",
	"IDN": "INDAGEN",
	"IDO": "SANTA ISABEL DO MORRO",
	"IDR": "INDORE",
	"IDY": "ILE D YEU",
	"IEG": "ZIELONA GORA",
	"IEJ": "IEJIMA",
	"IES": "RIESA",
	"IEV": "KIEV",
	"IFA": "IOWA FALLS",
	"IFF": "IFFLEY",
	"IFJ": "ISAFJORDUR",
	"IFL": "INNISFAIL",
	"IFN": "ISFAHAN",
	"IFO": "IVANO-FRANKOVSK",
	"IFP": "BULLHEAD CITY",
	"IGA": "INAGUA",
	"IGB": "INGENIERO JACOBACCI",
	"IGE": "IGUELA",
	"IGG": "IGIUGIG",
	"IGH": "INGHAM",
	"IGL": "IZMIR",
	"IGM": "KINGMAN",
	"IGN": "ILIGAN",
	"IGO": "CHIGORODO",
	"IGR": "IGUASSU FALLS",
	"IGU": "IGUASSU FALLS",
	"IGZ": "IGUATU",
	"IHA": "NIIHAMA",
	"IHN": "QISHN",
	"IHO": "IHOSY",
	"IHU": "IHU",
	"IIA": "INISHMAAN",
	"IIN": "NISHINOOMOTE",
	"IIS": "NISSAN ISLAND",
	"IJK": "IZHEVSK",
	"IJU": "IJUI",
	"IJX": "JACKSONVILLE",
	"IKB": "WILKESBORO",
	"IKI": "IKI",
	"IKK": "KANKAKEE",
	"IKL": "IKELA",
	"IKO": "NIKOLSKI",
	"IKP": "INKERMAN",
	"IKS": "TIKSI",
	"IKT": "IRKUTSK",
	"ILA": "ILLAGA",
	"ILB": "ILHA SOLTEIRA",
	"ILE": "KILLEEN",
	"ILF": "ILFORD",
	"ILG": "PHILADELPHIA",
	"ILI": "ILIAMNA",
	"ILK": "ILAKA",
	"ILL": "WILLMAR",
	"ILM": "WILMINGTON",
	"ILN": "WILMINGTON",
	"ILO": "ILOILO",
	"ILP": "ILE DES PINS",
	"ILQ": "ILO",
	"ILR": "ILORIN",
	"ILU": "KILAGUNI",
	"ILX": "ILEG",
	"ILY": "ISLAY",
	"ILZ": "ZILINA",
	"IMA": "IAMALELE",
	"IMB": "IMBAIMADAI",
	"IMD": "IMONDA",
	"IMF": "IMPHAL",
	"IMG": "INHAMINGA",
	"IMI": "INE",
	"IMK": "SIMIKOT",
	"IML": "IMPERIAL",
	"IMM": "IMMOKALEE",
	"IMN": "IMANE",
	"IMO": "ZEMIO",
	"IMP": "IMPERATRIZ",
	"IMT": "IRON MOUNTAIN",
	"IMZ": "NIMROZ",
	"INA": "INTA",
	"INB": "INDEPENDENCE",
	"INC": "YINCHUAN",
	"IND": "INDIANAPOLIS",
	"INE": "CHINDE",
	"INF": "IN GUEZZAM",
	"ING": "LAGO ARGENTINO",
	"INH": "INHAMBANE",
	"INI": "NIS",
	"INJ": "INJUNE",
	"INK": "WINK",
	"INL": "INTERNATIONAL FALLS",
	"INM": "INNAMINCKA",
	"INN": "INNSBRUCK",
	"INO": "INONGO",
	"INQ": "INISHEER",
	"INS": "INDIAN SPRINGS",
	"INT": "WINSTON-SALEM",
	"INU": "NAURU",
	"INV": "INVERNESS",
	"INW": "WINSLOW",
	"INX": "INANWATAN",
	"INY": "INYATI",
	"INZ": "IN SALAH",
	"IOA": "IOANNINA",
	"IOK": "IOKEA",
	"IOM": "ISLE OF MAN",
	"ION": "IMPFONDO",
	"IOP": "IOMA",
	"IOR": "INISHMORE",
	"IOS": "ILHEUS",
	"IOU": "ILE OUEN",
	"IOW": "IOWA CITY",
	"IPA": "IPOTA",
	"IPC": "EASTER ISLAND",
	"IPE": "IPIL",
	"IPG": "IPIRANGA",
	"IPH": "IPOH",
	"IPI": "IPIALES",
	"IPL": "EL CENTRO",
	"IPN": "IPATINGA",
	"IPO": "IPORA",
	"IPT": "WILLIAMSPORT",
	"IPU": "IPIAU",
	"IPW": "IPSWICH",
	"IQM": "QIEMO",
	"IQN": "QINGYANG",
	"IQQ": "IQUIQUE",
	"IQT": "IQUITOS",
	"IRA": "KIRA KIRA",
	"IRB": "IRAAN",
	"IRC": "CIRCLE",
	"IRD": "ISHURDI",
	"IRE": "IRECE",
	"IRG": "IRON RANGE",
	"IRI": "IRINGA",
	"IRJ": "LA RIOJA",
	"IRK": "KIRKSVILLE",
	"IRN": "IRIONA",
	"IRO": "BIRAO",
	"IRP": "ISIRO",
	"IRS": "STURGIS",
	"ISA": "MT. ISA",
	"ISB": "ISLAMABAD/RAWALPINDI",
	"ISC": "ISLES OF SCILLY",
	"ISD": "ISCUANDE",
	"ISG": "ISHIGAKI",
	"ISH": "ISCHIA",
	"ISI": "ISISFORD",
	"ISJ": "ISLA MUJERES",
	"ISK": "NASIK",
	"ISL": "ISABEL PASS",
	"ISM": "KISSIMMEE",
	"ISN": "WILLISTON",
	"ISO": "KINSTON",
	"ISP": "ISLIP/LONG ISLAND",
	"ISQ": "MANISTIQUE",
	"ISS": "WISCASSET",
	"IST": "ISTANBUL",
	"ISW": "WISCONSIN RAPIDS",
	"ITA": "ITACOATIARA",
	"ITB": "ITAITUBA",
	"ITE": "ITUBERA",
	"ITH": "ITHACA",
	"ITI": "ITAMBACURI",
	"ITJ": "ITAJAI",
	"ITK": "ITOKAMA",
	"ITM": "OSAKA",
	"ITN": "ITABUNA",
	"ITO": "HILO",
	"ITP": "ITAPETINGA",
	"ITQ": "ITAQUI",
	"ITU": "ITUIUTABA",
	"IUE": "NIUE ISLAND",
	"IUL": "ILU",
	"IUM": "SUMMIT LAKE",
	"IUS": "INUS",
	"IVA": "AMBANJA",
	"IVC": "INVERCARGILL",
	"IVG": "IVANGRAD",
	"IVH": "IVISHAK",
	"IVL": "IVALO",
	"IVO": "CHIVOLO",
	"IVR": "INVERELL",
	"IVW": "INVERWAY",
	"IWA": "IVANOVO",
	"IWD": "IRONWOOD",
	"IWJ": "IWAMI",
	"IWO": "IWO JIMA",
	"IWS": "HOUSTON",
	"IXA": "AGARTALA",
	"IXB": "BAGDOGRA",
	"IXC": "CHANDIGARH",
	"IXD": "ALLAHABAD",
	"IXE": "MANGALORE",
	"IXG": "BELGAUM",
	"IXH": "KAILASHAHAR",
	"IXI": "LILABARI",
	"IXJ": "JAMMU",
	"IXK": "KESHOD",
	"IXL": "LEH",
	"IXM": "MADURAI",
	"IXN": "KHOWAI",
	"IXP": "PATHANKOT",
	"IXQ": "KAMALPUR",
	"IXR": "RANCHI",
	"IXS": "SILCHAR",
	"IXT": "PASSIGHAT",
	"IXU": "AURANGABAD",
	"IXV": "ALONG",
	"IXW": "JAMSHEDPUR",
	"IXY": "KANDLA",
	"IXZ": "PORT BLAIR",
	"IYK": "INYOKERN",
	"IZM": "IZMIR",
	"IZO": "IZUMO",
	"IZT": "IXTEPEC",
	"JAA": "JALALABAD",
	"JAB": "JABIRU",
	"JAC": "JACKSON",
	"JAD": "JANDAKOT",
	"JAE": "ATLANTA",
	"JAF": "JAFFNA",
	"JAG": "JACOBABAD",
	"JAH": "AUBAGNE",
	"JAI": "JAIPUR",
	"JAJ": "ATLANTA",
	"JAK": "JACMEL",
	"JAL": "JALAPA",
	"JAM": "JAMBOL",
	"JAN": "JACKSON/VICKSBURG",
	"JAO": "ATLANTA",
	"JAQ": "JACQUINOT BAY",
	"JAR": "ARCADIA",
	"JAS": "JASPER",
	"JAT": "JABAT",
	"JAU": "JAUJA",
	"JAV": "JAKOBSHAVN",
	"JAX": "JACKSONVILLE",
	"JBC": "BOSTON",
	"JBK": "BERKELEY",
	"JBP": "LOS ANGELES",
	"JBR": "JONESBORO",
	"JBS": "PLEASANTON",
	"JBT": "BETHEL",
	"JCA": "CANNES",
	"JCB": "JOACABA",
	"JCC": "SAN FRANCISCO",
	"JCD": "ST.CROIX",
	"JCE": "SAN FRANCISCO",
	"JCH": "CHRISTIANSHAB",
	"JCI": "KANSAS CITY",
	"JCK": "JULIA CREEK",
	"JCM": "JACOBINA",
	"JCN": "INCHEON",
	"JCO": "COMINO",
	"JCR": "JACAREACANGA",
	"JCT": "JUNCTION",
	"JCY": "JOHNSON",
	"JDA": "JOHN DAY",
	"JDB": "DALLAS/FT. WORTH",
	"JDF": "JUIZ DE FORA",
	"JDH": "JODHPUR",
	"JDM": "MIAMI",
	"JDN": "JORDAN",
	"JDO": "JUAZEIRO DO NORTE",
	"JDP": "PARIS-DE PARIS",
	"JDT": "MINNEAPOLIS/ST PAUL",
	"JDX": "HOUSTON",
	"JDY": "DOWNEY",
	"JDZ": "JINGDEZHEN",
	"JED": "JEDDAH",
	"JEE": "JEREMIE",
	"JEF": "JEFFERSON CITY",
	"JEG": "EGEDESMINDE",
	"JEJ": "JEH",
	"JEM": "EMERYVILLE",
	"JEQ": "JEQUIE",
	"JER": "JERSEY",
	"JEV": "EVRY",
	"JFK": "NEW YORK",
	"JFM": "FREMANTLE",
	"JFN": "JEFFERSON",
	"JFR": "FREDERIKSHAB",
	"JGA": "JAMNAGAR",
	"JGB": "JAGDALPUR",
	"JGC": "GRAND CANYON",
	"JGE": "GEOJE",
	"JGL": "ATLANTA",
	"JGN": "JIAYUGUAN",
	"JGO": "GODHAVN",
	"JGP": "HOUSTON",
	"JGQ": "HOUSTON",
	"JGR": "GRONNEDAL/IVIGTUT",
	"JGX": "GLENDALE",
	"JHB": "JOHORE BHARU",
	"JHC": "GARDEN CITY",
	"JHE": "HELSINGBORG-HLPT",
	"JHG": "JINGHONG",
	"JHM": "KAPALUA",
	"JHQ": "SHUTE HARBOUR",
	"JHS": "HOLSTEINSBORG",
	"JHW": "JAMESTOWN",
	"JHY": "CAMBRIDGE",
	"JIA": "JUINA",
	"JIB": "DJIBOUTI",
	"JID": "LOS ANGELES",
	"JIJ": "JIJIGA",
	"JIK": "IKARIA ISLAND",
	"JIL": "JILIN",
	"JIM": "JIMMA",
	"JIN": "JINJA",
	"JIP": "JIPIJAPA",
	"JIR": "JIRI",
	"JIU": "JIUJIANG",
	"JIW": "JIWANI",
	"JJB": "JOHN BREWER REEF",
	"JJI": "JUANJUI",
	"JJN": "JINJIANG",
	"JJU": "JULIANEHAB",
	"JKG": "JONKOPING",
	"JKH": "CHIOS",
	"JKR": "JANAKPUR",
	"JKT": "AKARTA",
	"JKV": "JACKSONVILLE",
	"JLA": "COOPER LODGE",
	"JLC": "HOUSTON",
	"JLD": "HELSINGBORG-LANDSKRONA",
	"JLN": "JOPLIN",
	"JLO": "JESOLO",
	"JLP": "JUAN LES PINS",
	"JLR": "JABALPUR",
	"JLX": "LOS ANGELES",
	"JMA": "HOUSTON",
	"JMB": "JAMBA",
	"JMC": "MARIN CITY",
	"JMH": "SCHAUMBURG",
	"JMK": "MIKONOS",
	"JMM": "MALMO-HARBOUR",
	"JMO": "JOMSOM",
	"JMS": "JAMESTOWN",
	"JMU": "JIAMUSI",
	"JMY": "FREETOWN-MAMMY YOKO",
	"JNA": "JANUARIA",
	"JNB": "JOHANNESBURG",
	"JNG": "JINING",
	"JNI": "JUNIN",
	"JNN": "NANORTALIK",
	"JNP": "NEWPORT BEACH",
	"JNS": "NARSSAQ",
	"JNU": "JUNEAU",
	"JNX": "NAXOS",
	"JNZ": "JINZHOU",
	"JOC": "SANTA ANA",
	"JOE": "JOENSUU",
	"JOG": "JOGYAKARTA",
	"JOH": "PORT SAINT JOHNS",
	"JOI": "JOINVILLE",
	"JOK": "JOSHKAR-OLA",
	"JOL": "JOLO",
	"JOM": "NJOMBE",
	"JON": "JOHNSTON ISLAND",
	"JOP": "JOSEPHSTAAL",
	"JOR": "ORANGE",
	"JOS": "JOS",
	"JOT": "JOLIET",
	"JPA": "JOAO PESSOA",
	"JPD": "PASADENA",
	"JPJ": "PATERSON",
	"JPR": "JI-PARANA",
	"JPT": "HOUSTON",
	"JPU": "PARIS-LA DEFENSE",
	"JQE": "JAQUE",
	"JRA": "NEW YORK",
	"JRB": "NEW YORK",
	"JRE": "NEW YORK",
	"JRH": "JORHAT",
	"JRK": "ARSUK",
	"JRN": "JURUENA",
	"JRO": "KILIMANJARO",
	"JRS": "JERUSALEM",
	"JSA": "JAISALMER",
	"JSD": "STRATFORD",
	"JSH": "SITIA",
	"JSI": "SKIATHOS",
	"JSK": "ST CLOUD",
	"JSL": "ATLANTIC CITY",
	"JSM": "JOSE DE SAN MARTIN",
	"JSO": "SODERTALJE",
	"JSR": "JESSORE",
	"JSS": "SPETSAI",
	"JST": "JOHNSTOWN",
	"JSU": "SUKKERTOPPEN",
	"JSY": "SYROS ISLAND",
	"JSZ": "ST. TROPEZ",
	"JTC": "HOUSTON",
	"JTI": "JATAI",
	"JTO": "THOUSAND OAKS",
	"JTR": "SANTORINI",
	"JTY": "ASTYPALEA IS.",
	"JUA": "JUARA",
	"JUB": "JUBA",
	"JUI": "JUIST",
	"JUJ": "JUJUY",
	"JUL": "JULIACA",
	"JUM": "JUMLA",
	"JUN": "JUNDAH",
	"JUO": "JURADO",
	"JUR": "JURIEN BAY",
	"JUT": "JUSTICALPA",
	"JUV": "UPERNAVIK",
	"JUZ": "JUZHOU",
	"JVA": "ANKAVANDRA",
	"JVI": "MANVILLE",
	"JVL": "BELOIT/JANESVILLE",
	"JWA": "JWANENG",
	"JWH": "HOUSTON",
	"JWL": "HOUSTON",
	"JXN": "JACKSON",
	"JYV": "JYVASKYLA",
	"JZA": "NEWTON",
	"JZB": "ORANGE",
	"JZC": "STANLEY",
	"JZD": "GANDA",
	"JZE": "TRADING BAY",
	"JZF": "GAOXIONG",
	"JZG": "CHAMBERLAIN",
	"JZH": "ONIDA",
	"JZI": "PORTLAND",
	"JZJ": "DULUTH",
	"JZK": "CLOQUET",
	"JZL": "SAN CLEMENTE ISLND",
	"JZM": "HOG RIVER",
	"JZN": "QUINCY",
	"JZO": "AMERICAN FALLS",
	"JZP": "PROSPECT",
	"JZQ": "BROOKSVILLE",
	"JZR": "LAUREL",
	"JZS": "BAKERSFIELD",
	"JZT": "ARLINGTON",
	"JZU": "BENNINGTON",
	"JZV": "LITTLE ROCK",
	"JZW": "EVERGREEN",
	"JZX": "ANGUIN",
	"JZY": "GRANGEVILLE",
	"JZZ": "TASZAR",
	"KAA": "KASAMA",
	"KAB": "KARIBA DAM",
	"KAC": "KAMESHLI",
	"KAD": "KADUNA",
	"KAE": "KAKE",
	"KAF": "KARATO",
	"KAG": "KANGNUNG",
	"KAH": "MELBOURNE",
	"KAI": "KAIETEUR",
	"KAJ": "KAJAANI",
	"KAK": "KAR",
	"KAL": "KALTAG",
	"KAM": "KAMARAN IS.",
	"KAN": "KANO",
	"KAO": "KU",
	"KAP": "KAPANGA",
	"KAQ": "KAMULAI",
	"KAR": "KAMARANG",
	"KAS": "KARASBURG",
	"KAT": "KAITAIA",
	"KAU": "KAUHAVA",
	"KAV": "KAVANAYEN",
	"KAW": "KAWTHAUNG",
	"KAX": "KALBARRI",
	"KAY": "WAKAYA",
	"KAZ": "KAU",
	"KBA": "KABALA",
	"KBB": "KIRKIMBIE",
	"KBC": "BIRCH CREEK",
	"KBD": "KIMBERLY DOWNS",
	"KBE": "BELL ISLAND",
	"KBF": "KARUBAGA",
	"KBG": "KABALEGA FALLS",
	"KBH": "KALAT",
	"KBI": "KRIBI",
	"KBJ": "KINGS CANYON",
	"KBK": "KLAG BAY",
	"KBL": "KABUL",
	"KBM": "KABWUM",
	"KBN": "KABINDA",
	"KBO": "KABALO",
	"KBP": "KIEV",
	"KBQ": "KASUNGU",
	"KBR": "KOTA BHARU",
	"KBS": "BO",
	"KBT": "KABEN",
	"KBV": "KRABI",
	"KBW": "CHIGNIK",
	"KBX": "KAMBUAYA",
	"KBY": "STREAKY BAY",
	"KBZ": "KAIKOURA",
	"KCA": "KUQA",
	"KCB": "KASIKASIMA",
	"KCC": "COFFMAN COVE",
	"KCD": "KAMUR",
	"KCE": "COLLINSVILLE",
	"KCG": "CHIGNIK FISHERIES",
	"KCH": "KUCHING",
	"KCK": "KANSAS CITY",
	"KCL": "CHIGNIK LAGOON",
	"KCN": "CHERNOFSKI",
	"KCP": "KAMENETS-PODOLSKIY",
	"KCQ": "CHIGNIK",
	"KCR": "COLORADO CREEK",
	"KCS": "KINGS CREEK STATION",
	"KCU": "MASINDI",
	"KCZ": "KOCHI",
	"KDA": "KOLDA",
	"KDB": "KAMBALDA",
	"KDC": "KANDI",
	"KDD": "KHUZDAR",
	"KDE": "KOROBA",
	"KDG": "KARDJALI",
	"KDH": "KANDAHAR",
	"KDI": "KENDARI",
	"KDJ": "N DJOLE",
	"KDK": "KODIAK",
	"KDL": "KARDLA",
	"KDM": "KAADEDHDHOO",
	"KDN": "N'DENDE",
	"KDO": "KADHDHOO",
	"KDP": "KANDEP",
	"KDR": "KANDRIAN",
	"KDS": "KAMARAN DOWNS",
	"KDU": "SKARDU",
	"KDV": "KANDAVU",
	"KDX": "KOKSTAD",
	"KEA": "KAWERAU",
	"KEB": "ENGLISH BAY",
	"KEC": "KASENGA",
	"KED": "KAEDI",
	"KEE": "KELLE",
	"KEF": "REYKJAVIK",
	"KEG": "KEGLSUGL",
	"KEH": "KENMORE",
	"KEI": "KEPI",
	"KEJ": "KEMEROVO",
	"KEK": "EKWOK",
	"KEL": "KIEL",
	"KEM": "KEMI",
	"KEN": "KENEMA",
	"KEO": "ODIENNE",
	"KEP": "NEPALGANJ",
	"KEQ": "KEBAR",
	"KER": "KERMAN",
	"KES": "KELSEY",
	"KET": "KENGTUNG",
	"KEU": "KELLY BAR",
	"KEV": "KUOREVESI",
	"KEW": "MONKEY MIA",
	"KEX": "KANABEA",
	"KEY": "KERICHO",
	"KFA": "KIFFA",
	"KFF": "FARRANFORE",
	"KFG": "KALKURUNG",
	"KFP": "FALSE PASS",
	"KGA": "KANANGA",
	"KGB": "KONGE",
	"KGC": "KINGSCOTE",
	"KGD": "KALININGRAD",
	"KGE": "GREYMOUTH",
	"KGF": "KARAGANDA",
	"KGG": "KEDOUGOU",
	"KGH": "YONGAI",
	"KGI": "KALGOORLIE",
	"KGJ": "KARONGA",
	"KGK": "KOLIGANEK",
	"KGL": "KIGALI",
	"KGN": "KASONGO-LUNDA",
	"KGO": "KIROVOGRAD",
	"KGQ": "KASUNGU",
	"KGR": "KULGERA",
	"KGS": "KOS",
	"KGU": "KENINGAU",
	"KGW": "KAGI",
	"KGX": "GRAYLING",
	"KGY": "KINGAROY",
	"KGZ": "GLACIER CREEK",
	"KHA": "KHANEK",
	"KHC": "KERCH",
	"KHE": "KHERSON",
	"KHG": "KASHI",
	"KHH": "KAOHSIUNG",
	"KHI": "KARACHI",
	"KHJ": "KAUHAJOKI",
	"KHK": "KHARK",
	"KHL": "KHULNA",
	"KHM": "KHAMTI",
	"KHN": "NANCHANG",
	"KHO": "KHOKA MOYA",
	"KHR": "KHARKHORIN",
	"KHS": "KHASAB",
	"KHT": "KHOST",
	"KHU": "KREMENCHUG",
	"KHV": "KHABAROVSK",
	"KHW": "KHWAI RIVER LODGE",
	"KIA": "KAIAPIT",
	"KIB": "IVANOFF BAY",
	"KIC": "KING CITY",
	"KID": "KRISTIANSTAD",
	"KIE": "KIETA",
	"KIF": "KINGFISHER LAKE",
	"KIG": "KOINGNAAS",
	"KIH": "KISH ISLAND",
	"KIJ": "NIIGATA",
	"KIK": "KIRKUK",
	"KIL": "KILWA",
	"KIM": "KIMBERLEY",
	"KIN": "KINGSTON",
	"KIO": "KILI",
	"KIP": "WICHITA FALLS",
	"KIQ": "KIRA",
	"KIR": "KERRY COUNTY",
	"KIS": "KISUMU",
	"KIT": "KITHIRA",
	"KIU": "KIUNGA",
	"KIV": "KISHINEV",
	"KIW": "KITWE",
	"KIX": "OSAKA",
	"KIY": "KILWA",
	"KIZ": "KIKINONDA",
	"KJA": "KRASNOJARSK",
	"KJB": "KIMSQUIT",
	"KJK": "KORTRIJK",
	"KJP": "KERAMA",
	"KJU": "KAMIRABA",
	"KKA": "KOYUK",
	"KKB": "KITOI",
	"KKC": "KHON KAEN",
	"KKD": "KOKODA",
	"KKE": "KERIKERI",
	"KKF": "KAGVIK CREEK",
	"KKG": "KONAWARUK",
	"KKH": "KONGIGANAK",
	"KKI": "AKIACHAK",
	"KKJ": "KITA KYUSHU",
	"KKK": "KALAKAKET CREEK",
	"KKL": "KARLUK LAKE",
	"KKM": "LOP BURI",
	"KKN": "KIRKENES",
	"KKO": "KAIKOHE",
	"KKP": "KOOLBURRA",
	"KKR": "KAUKURA",
	"KKT": "KENTLAND",
	"KKU": "EKUK",
	"KKW": "KIKWIT",
	"KKX": "KIKAIGA SHIMA",
	"KKY": "KILKENNY",
	"KKZ": "KOH KONG",
	"KLB": "KALABO",
	"KLC": "KAOLACK",
	"KLD": "KALININ",
	"KLE": "KAELE",
	"KLF": "KALUGA",
	"KLG": "KALSKAG",
	"KLH": "KOLHAPUR",
	"KLI": "KOTAKOLI",
	"KLJ": "KLAIPEDA",
	"KLK": "KALOKOL",
	"KLL": "LEVELOCK",
	"KLN": "LARSEN BAY",
	"KLO": "KALIBO",
	"KLP": "KELP BAY",
	"KLQ": "KELUANG",
	"KLR": "KALMAR",
	"KLS": "KELSO",
	"KLT": "KAISERSLAUTER",
	"KLU": "KLAGENFURT",
	"KLV": "KARLOVY VARY",
	"KLW": "KLAWOCK",
	"KLX": "KALAMATA",
	"KLY": "KALIMA",
	"KLZ": "KLEINZEE",
	"KMA": "KEREMA",
	"KMB": "KOINAMBE",
	"KMC": "MC CORD",
	"KMD": "MANDJI",
	"KME": "KAMEMBE",
	"KMF": "KAMINA",
	"KMG": "KUNMING",
	"KMH": "KURUMAN",
	"KMI": "MIYAZAKI",
	"KMJ": "KUMAMOTO",
	"KMK": "MAKABANA",
	"KML": "KAMILEROI",
	"KMM": "KIMAM",
	"KMN": "KAMINA",
	"KMO": "MANOKOTAK",
	"KMP": "KEETMANSHOOP",
	"KMQ": "KOMATSU",
	"KMR": "KARIMUI",
	"KMS": "KUMASI",
	"KMT": "KAMPOT",
	"KMU": "KISMAYU",
	"KMV": "KALEMYO",
	"KMW": "KOSTROMA",
	"KMX": "KHAMIS MUSHAIT",
	"KMY": "MOSER BAY",
	"KMZ": "KAOMA",
	"KNA": "VINA DEL MAR",
	"KNB": "KANAB",
	"KND": "KINDU",
	"KNE": "KANAINJ",
	"KNG": "KAIMANA",
	"KNH": "KINMEN",
	"KNI": "KATANNING",
	"KNJ": "KINDANBA",
	"KNK": "KAKHONAK",
	"KNL": "KELANOA",
	"KNM": "KANIAMA",
	"KNN": "KANKAN",
	"KNO": "KNOCKE LE-ZOUTE",
	"KNP": "CAPANDA",
	"KNQ": "KONE",
	"KNR": "KANGAN",
	"KNS": "KING ISLAND",
	"KNT": "KENNETT",
	"KNU": "KANPUR",
	"KNV": "KNIGHTS INLET",
	"KNW": "NEW STUYAHOK",
	"KNX": "KUNUNURRA",
	"KNZ": "KENIEBA",
	"KOA": "KONA",
	"KOB": "KOUTABA",
	"KOC": "KOUMAC",
	"KOD": "KOTABANGUN",
	"KOE": "KUPANG",
	"KOF": "KOMATIPOORT",
	"KOG": "KHONG",
	"KOH": "KOOLATAH",
	"KOI": "KIRKWALL",
	"KOJ": "KAGOSHIMA",
	"KOK": "KOKKOLA",
	"KOL": "KOUMALA",
	"KOM": "KOMO-MANDA",
	"KON": "KONTUM",
	"KOO": "KONGOLO",
	"KOP": "NAKHON PHANOM",
	"KOQ": "KOETHEN",
	"KOR": "KOKORO",
	"KOS": "KOMPONG SOM",
	"KOT": "KOTLIK",
	"KOU": "KOULA MOUTOU",
	"KOV": "KOKCHETAV",
	"KOW": "GANZHOU",
	"KOX": "KOKONAO",
	"KOY": "OLGA BAY",
	"KOZ": "OUZINKIE",
	"KPA": "KOPIAGO",
	"KPB": "POINT BAKER",
	"KPC": "PORT CLARENCE",
	"KPD": "KING OF PRUSSIA",
	"KPE": "YAPSIEI",
	"KPG": "KURUPUNG",
	"KPH": "PAULOFF HARBOR",
	"KPI": "KAPIT",
	"KPK": "PARKS",
	"KPM": "KOMPIAM",
	"KPN": "KIPNUK",
	"KPO": "POHANG",
	"KPP": "KALPOWAR",
	"KPR": "PORT WILLIAMS",
	"KPS": "KEMPSEY",
	"KPT": "JACKPOT",
	"KPV": "PERRYVILLE",
	"KPY": "PORT BAILEY",
	"KQA": "AKUTAN",
	"KQB": "KOONIBBA",
	"KQL": "KOL",
	"KRA": "KERANG",
	"KRB": "KARUMBA",
	"KRC": "KERINCI",
	"KRD": "KURUNDI",
	"KRE": "KIRUNDO",
	"KRF": "KRAMFORS",
	"KRG": "KARASABAI",
	"KRI": "KIKORI",
	"KRJ": "KARAWARI",
	"KRK": "KRAKOW",
	"KRL": "KORLA",
	"KRM": "KARANAMBO",
	"KRN": "KIRUNA",
	"KRO": "KROONSTAD",
	"KRP": "KARUP",
	"KRQ": "KRAMATORSK",
	"KRR": "KRASNODAR",
	"KRS": "KRISTIANSAND",
	"KRT": "KHARTOUM",
	"KRU": "KERAU",
	"KRV": "KERIO VALLEY",
	"KRW": "KRASNOWODSK",
	"KRX": "KAR KAR",
	"KRY": "KARAMAY",
	"KRZ": "KIRI",
	"KSA": "KOSRAE",
	"KSB": "KASANOMBE",
	"KSC": "KOSICE",
	"KSD": "KARLSTAD",
	"KSE": "KASESE",
	"KSF": "KASSEL",
	"KSG": "KISENGAN",
	"KSH": "KERMANSHAH",
	"KSI": "KISSIDOUGOU",
	"KSJ": "KASOS ISLAND",
	"KSK": "KARLSKOGA",
	"KSL": "KASSALA",
	"KSM": "ST. MARY'S",
	"KSN": "KUSTANAY",
	"KSO": "KASTORIA",
	"KSP": "KOSIPE",
	"KSQ": "KARSHI",
	"KSS": "SIKASSO",
	"KST": "KOSTI",
	"KSU": "KRISTIANSUND",
	"KSV": "SPRINGVALE",
	"KSW": "KIRYAT SHMONA",
	"KSX": "YASURU",
	"KSY": "KARS",
	"KSZ": "KOTLAS",
	"KTA": "KARRATHA",
	"KTB": "THORNE BAY",
	"KTC": "KATIOLA",
	"KTD": "KITA- DAITO",
	"KTE": "KERTEH",
	"KTF": "TAKAKA",
	"KTG": "KETAPANG",
	"KTH": "TIKCHIK",
	"KTI": "KRATIE",
	"KTK": "KANUA",
	"KTL": "KITALE",
	"KTM": "KATHMANDU",
	"KTN": "KETCHIKAN",
	"KTO": "KATO",
	"KTP": "KINGSTON",
	"KTQ": "KITEE",
	"KTR": "KATHERINE",
	"KTS": "BREVIG MISSION",
	"KTT": "KITTILA",
	"KTU": "KOTA",
	"KTV": "KAMARATA",
	"KTW": "KATOWICE",
	"KTX": "KOUTIALA",
	"KTY": "TERROR BAY",
	"KTZ": "KWUN TONG",
	"KUA": "KUANTAN",
	"KUC": "KURIA",
	"KUD": "KUDAT",
	"KUE": "KUKUNDU",
	"KUF": "SAMARA (KUJBYSEV)",
	"KUG": "KUBIN ISLAND",
	"KUH": "KUSHIRO",
	"KUI": "KAWAU ISLAND",
	"KUJ": "KUSHIMOTO-KATSUURA",
	"KUK": "KASIGLUK",
	"KUL": "KUALA LUMPUR",
	"KUM": "YAKU SHIMA",
	"KUN": "KAUNAS",
	"KUO": "KUOPIO",
	"KUP": "KUPIANO",
	"KUQ": "KURI",
	"KUR": "KURAN-0-MUNJAN",
	"KUS": "KULUSUK",
	"KUT": "KUTAISI",
	"KUU": "KULU",
	"KUV": "KUNSAN",
	"KUW": "KAMUSI",
	"KUY": "UYAK",
	"KUZ": "KUSAN",
	"KVA": "KAVALLA",
	"KVB": "SKOVDE",
	"KVC": "KING COVE",
	"KVD": "GYANDZHA (KIROVOBAD)",
	"KVE": "KITAVA",
	"KVG": "KAVIENG",
	"KVK": "KIROVSK",
	"KVL": "KIVALINA",
	"KVU": "KOROLEVU",
	"KVX": "KIROV",
	"KWA": "KWAJALEIN",
	"KWB": "KARIMUNJAWA",
	"KWE": "GUIYANG",
	"KWF": "WATERFALL",
	"KWG": "KRIVOY ROG",
	"KWH": "KHWAHAN",
	"KWI": "KUWAIT",
	"KWJ": "KWANGJU",
	"KWK": "KWIGILLINGOK",
	"KWL": "GUILIN",
	"KWM": "KOWANYAMA",
	"KWN": "KWINHAGAK",
	"KWO": "KAWITO",
	"KWP": "WEST POINT",
	"KWR": "KWAI HARBOUR",
	"KWS": "KWAILABESI",
	"KWT": "KWETHLUK",
	"KWV": "KURWINA",
	"KWX": "KIWAI ISLAND",
	"KWY": "KIWAYU",
	"KWZ": "KOLWEZI",
	"KXA": "KASAAN",
	"KXE": "KLERKSDORP",
	"KXF": "KORO",
	"KXK": "KOMSOMOLXK NA AMURE",
	"KXR": "KAROOLA",
	"KYA": "KONYA",
	"KYD": "ORCHID ISLAND",
	"KYE": "TRIPOLI",
	"KYF": "YEELIRIE",
	"KYI": "YALATA MISSION",
	"KYK": "KARLUK",
	"KYL": "KEY LARGO",
	"KYN": "MILTON KEYNES",
	"KYP": "KYAUKPYU",
	"KYS": "KAYES",
	"KYT": "KYAUKTAW",
	"KYU": "KOYUKUK",
	"KYX": "YALUMET",
	"KYZ": "KYZYL",
	"KZB": "ZACHAR BAY",
	"KZC": "KOMPONG-CHHNANG",
	"KZD": "KRAKOR",
	"KZF": "KAINTIBA",
	"KZG": "KITZINGEN",
	"KZH": "KIZHUYAK",
	"KZI": "KOZANI",
	"KZK": "KOMPONG THOM",
	"KZN": "KAZAN",
	"KZS": "KASTELORIZO",
	"LAA": "LAMAR",
	"LAB": "LABLAB",
	"LAC": "LAE",
	"LAD": "LUANDA",
	"LAE": "LAE",
	"LAF": "LAFAYETTE",
	"LAG": "LA GUAIRA",
	"LAH": "LABUHA",
	"LAI": "LANNION",
	"LAJ": "LAGES",
	"LAK": "AKLAVIK",
	"LAL": "LAKELAND",
	"LAM": "LOS ALAMOS",
	"LAN": "LANSING",
	"LAO": "LAOAG",
	"LAP": "LA PAZ",
	"LAQ": "BEIDA",
	"LAR": "LARAMIE",
	"LAS": "LAS VEGAS",
	"LAU": "LAMU",
	"LAV": "LALOMALAVA",
	"LAW": "LAWTON",
	"LAX": "LOS ANGELES",
	"LAY": "LADYSMITH",
	"LAZ": "BOM JESUS DA LAPA",
	"LBA": "LEEDS/BRADFORD",
	"LBB": "LUBBOCK",
	"LBC": "LUEBECK",
	"LBD": "KHUDZHAND (LENINABAD)",
	"LBE": "LATROBE",
	"LBF": "NORTH PLATTE",
	"LBG": "LE BOURGET",
	"LBH": "SYDNEY",
	"LBI": "ALBI",
	"LBJ": "LABUAN BAJO",
	"LBK": "LIBOI",
	"LBL": "LIBERAL",
	"LBM": "LUABO",
	"LBN": "LAKE BARINGO",
	"LBO": "L",
	"LBP": "LONG BANGA",
	"LBQ": "LAMBARENE",
	"LBR": "LABREA",
	"LBS": "LABASA",
	"LBT": "LUMBERTON",
	"LBU": "LABUAN",
	"LBV": "LIBREVILLE",
	"LBW": "LONG BAWAN",
	"LBX": "LUBANG",
	"LBY": "LA BAULE",
	"LCA": "LARNACA",
	"LCB": "PONTES E LACERDA",
	"LCC": "LECCE",
	"LCD": "LOUIS TRICHARDT",
	"LCE": "LA CEIBA",
	"LCG": "LA CORUNA",
	"LCH": "LAKE CHARLES",
	"LCI": "LACONIA",
	"LCK": "COLUMBUS",
	"LCL": "LA COLOMA",
	"LCM": "LA CUMBRE",
	"LCN": "BALCANOONA",
	"LCO": "LAGUE",
	"LCR": "LA CHORRERA",
	"LCS": "LAS CANAS",
	"LCV": "LUCCA",
	"LCY": "LONDON",
	"LDA": "MALDA",
	"LDB": "LONDRINA",
	"LDC": "LINDEMAN ISLAND",
	"LDE": "LOURDES",
	"LDH": "LORD HOWE IS.",
	"LDI": "LINDI",
	"LDJ": "LINDEN",
	"LDK": "LIDKOPING",
	"LDM": "LUDINGTON",
	"LDN": "LAMIDANDA",
	"LDO": "LADOUANIE",
	"LDR": "LAWDAR",
	"LDU": "LAHAD DATU",
	"LDV": "LANDIVISIAU",
	"LDW": "LANSDOWNE",
	"LDX": "SAINT-LAURENT DU MARONI",
	"LDY": "LONDONDERRY",
	"LDZ": "LONDOLOZI",
	"LEA": "LEARMONTH",
	"LEB": "LEBANON",
	"LED": "ST. PETERSBURG",
	"LEE": "LEESBURG",
	"LEF": "LEBAKENG",
	"LEG": "ALEG",
	"LEH": "LE HAVRE",
	"LEI": "ALMERIA",
	"LEJ": "LEIPZIG",
	"LEK": "LABE",
	"LEL": "LAKE EVELLA",
	"LEM": "LEMMON",
	"LEO": "LECONI",
	"LEP": "LEOPOLDINA",
	"LEQ": "LANDS END",
	"LER": "LEINSTER",
	"LES": "LESOBENG",
	"LET": "LETICIA",
	"LEU": "SEO DE URGEL",
	"LEV": "LEVUKA",
	"LEW": "LEWISTON/AUBURN",
	"LEX": "LEXINGTON/FRANKFORT",
	"LEY": "LELYSTAD",
	"LEZ": "LA ESPERANZA",
	"LFB": "LAJES",
	"LFI": "NEWPORT NEWS/HAMPTON",
	"LFK": "LUFKIN",
	"LFN": "LOUISBURG",
	"LFO": "KELAFO",
	"LFP": "LAKEFIELD",
	"LFR": "LA FRIA",
	"LFT": "LAFAYETTE",
	"LFW": "LOME",
	"LGA": "NEW YORK",
	"LGB": "LONG BEACH",
	"LGC": "LA GRANGE",
	"LGD": "LA GRANDE",
	"LGE": "LENGEH",
	"LGG": "LIEGE",
	"LGH": "LEIGH CREEK",
	"LGI": "DEADMAN'S CAY",
	"LGK": "LANGKAWI",
	"LGL": "LONG LELLANG",
	"LGM": "LAIAGAM",
	"LGN": "LINGA LINGA",
	"LGO": "LANGEOOG",
	"LGP": "LEGASPI",
	"LGQ": "LAGO AGRIO",
	"LGR": "COCHRANE",
	"LGS": "MALARGUE",
	"LGT": "LAS GAVIOTAS",
	"LGU": "LOGAN",
	"LGW": "LONDON",
	"LGX": "LUGH GANANE",
	"LGY": "LAGUNILLAS",
	"LGZ": "LEGUIZAMO",
	"LHA": "LAHR",
	"LHB": "LOST HARBOR",
	"LHE": "LAHORE",
	"LHG": "LIGHTNING RIDGE",
	"LHI": "LEREH",
	"LHK": "GUANGHUA",
	"LHN": "LISHAN",
	"LHP": "LEHU",
	"LHR": "LONDON",
	"LHS": "LAS HERAS",
	"LHV": "LOCK HAVEN",
	"LHW": "LANZHOU",
	"LIA": "LIANGPING",
	"LIB": "LIMBUNYA",
	"LIC": "LIMON",
	"LID": "LEIDEN",
	"LIE": "LIBENGE",
	"LIF": "LIFU",
	"LIG": "LIMOGES",
	"LIH": "LIHUE",
	"LII": "MULIA",
	"LIJ": "LONG ISLAND",
	"LIK": "LIKIEP",
	"LIL": "LILLE",
	"LIM": "LIMA",
	"LIN": "MILAN",
	"LIO": "LIMON",
	"LIP": "LINS",
	"LIQ": "LISALA",
	"LIR": "LIBERIA",
	"LIS": "LISBON",
	"LIT": "LITTLE ROCK",
	"LIU": "LINOSA",
	"LIV": "LIVENGOOD",
	"LIW": "LOIKAW",
	"LIX": "LIKOMA ISLAND",
	"LIY": "HINESVILLE",
	"LIZ": "LIMESTONE",
	"LJA": "LODJA",
	"LJG": "LIJIANG CITY",
	"LJN": "LAKE JACKSON",
	"LJU": "LJUBLJANA",
	"LKA": "LARANTUKA",
	"LKB": "LAKEBA",
	"LKC": "LEKANA",
	"LKD": "LAKELAND DOWNS",
	"LKE": "SEATTLE",
	"LKK": "KULIK LAKE",
	"LKL": "LAKSELV",
	"LKN": "LEKNES",
	"LKO": "LUCKNOW",
	"LKR": "LAS KOREI",
	"LKS": "LAKESIDE",
	"LKT": "LAKOTA",
	"LKU": "LAKE RUDOLF",
	"LKV": "LAKEVIEW",
	"LKY": "LAKE MANYARA",
	"LKZ": "BRANDON",
	"LLA": "LULEA",
	"LLD": "LOVELAND",
	"LLE": "MALELANE",
	"LLG": "CHILLAGOE",
	"LLH": "LAS LIMAS",
	"LLI": "LALIBELLA",
	"LLL": "LISSADEL",
	"LLM": "LONG LAMA",
	"LLN": "KELILA",
	"LLP": "LINDA DOWNS",
	"LLS": "LAS LOMITAS",
	"LLW": "LILONGWE",
	"LLX": "LYNDONVILLE",
	"LLY": "MOUNT HOLLY",
	"LMA": "LAKE MINCHUMINA",
	"LMB": "MOZAMBIQUE",
	"LMC": "LAMACARENA",
	"LMD": "LOS MENUCOS",
	"LME": "LE MANS",
	"LMG": "LAMASSA",
	"LMH": "LIMON",
	"LMI": "LUMI",
	"LML": "LAE",
	"LMM": "LOS MOCHIS",
	"LMN": "LIMBANG",
	"LMP": "LAMPED",
	"LMQ": "MARSA EL BREGA",
	"LMR": "LIME ACRES",
	"LMS": "LOUISVILLE",
	"LMT": "KLAMATH FALLS",
	"LMX": "LOPEZ DE MICAY",
	"LMY": "LAKE MURRAY",
	"LMZ": "PALMA",
	"LNA": "WEST PALM BEACH",
	"LNB": "LAMEN BAY",
	"LNC": "LENGBATI",
	"LND": "LANDER",
	"LNE": "ONORORE",
	"LNF": "MUNBIL",
	"LNG": "LESE",
	"LNH": "LAKE NASH",
	"LNI": "LONELY",
	"LNK": "LINCOLN",
	"LNM": "LANGIMAR",
	"LNN": "WILLOUGHBY",
	"LNO": "LEONORA",
	"LNP": "WISE",
	"LNQ": "LOANI",
	"LNR": "LONE ROCK",
	"LNS": "LANCASTER",
	"LNV": "LONDOLOVIT",
	"LNX": "SMOLENSK",
	"LNY": "LANAI CITY",
	"LNZ": "LINZ",
	"LOA": "LORRAINE",
	"LOB": "LOBITO",
	"LOC": "LOCK",
	"LOD": "LONGANA",
	"LOE": "LOEI",
	"LOF": "LOEN",
	"LOG": "LONGVIEW",
	"LOH": "LOJA",
	"LOK": "LODWAR",
	"LOL": "LOVELOCK",
	"LOM": "LOERIESFONTEIN",
	"LON": "LONDON",
	"LOO": "LAGHOUAT",
	"LOQ": "LOBATSE",
	"LOS": "LAGOS",
	"LOT": "LOCKPORT",
	"LOV": "MONCLOVA",
	"LOW": "LOUISA",
	"LOY": "LOYANGALANI",
	"LOZ": "LONDON/CORBIN",
	"LPA": "LAS PALMAS",
	"LPB": "LA PAZ",
	"LPC": "LOMPOC",
	"LPD": "LA PEDRERA",
	"LPE": "LA PRIMAVERA",
	"LPG": "LA PLATA",
	"LPH": "LOCHGILPHEAD",
	"LPI": "LINKOEPING",
	"LPJ": "PIJIGUAOS",
	"LPK": "LIPETSK",
	"LPL": "LIVERPOOL",
	"LPM": "LAMAP",
	"LPO": "LA PORTE",
	"LPP": "LAPPEENRANTA",
	"LPQ": "LUANG PRABANG",
	"LPS": "LOPEZ ISLAND",
	"LPT": "LAMPANG",
	"LPU": "LONGAPUNG",
	"LPW": "LITTLE PORT WALTER",
	"LPX": "LIEPAYA",
	"LPY": "LE PUY",
	"LQK": "PICKENS",
	"LQM": "PUERTO LEGUIZAMO",
	"LQN": "QALA-NAU",
	"LRA": "LARISSA",
	"LRB": "LERIBE",
	"LRD": "LAREDO",
	"LRE": "LONGREACH",
	"LRF": "JACKSONVILLE",
	"LRG": "LORA LAI",
	"LRH": "LA ROCHELLE",
	"LRI": "LORICA",
	"LRJ": "LEMARS",
	"LRL": "LAMA-KARA",
	"LRM": "LA ROMANA",
	"LRQ": "LAURIE RIVER",
	"LRS": "LEROS",
	"LRT": "LORIENT",
	"LRU": "LAS CRUCES",
	"LRV": "LOS ROQUES",
	"LSA": "LOSUIA",
	"LSB": "LORDSBURG",
	"LSC": "LA SERENA",
	"LSE": "LA CROSSE",
	"LSF": "COLUMBUS",
	"LSH": "LASHIO",
	"LSI": "LERWICK",
	"LSJ": "LONG ISLAND",
	"LSK": "LUSK",
	"LSL": "LOS CHILES",
	"LSM": "LONG SEMADO",
	"LSN": "LOS BANOS",
	"LSO": "LES SABLES",
	"LSP": "LAS PIEDRAS",
	"LSQ": "LOS ANGELES",
	"LSR": "LOST RIVER",
	"LSS": "LES SAINTES",
	"LST": "LAUNCESTON",
	"LSU": "LONG SUKANG",
	"LSV": "LAS VEGAS",
	"LSW": "LHOKSEUMAWE",
	"LSX": "LHOK SUKON",
	"LSY": "LISMORE",
	"LSZ": "MALI LOSINJ",
	"LTA": "TZANEEN",
	"LTB": "LATROBE",
	"LTC": "LAI",
	"LTD": "GHADAMES",
	"LTF": "LEITRE",
	"LTG": "LANGTANG",
	"LTH": "LATHROP WELLS",
	"LTI": "ALTAI",
	"LTK": "LATTAKIA",
	"LTL": "LASTOURVILLE",
	"LTM": "LETHEM",
	"LTN": "LONDON",
	"LTO": "LORETO",
	"LTP": "LYNDHURST",
	"LTQ": "LE TOUQUET",
	"LTR": "LETTERKENNY",
	"LTS": "ALTUS",
	"LTT": "ST. TROPEZ",
	"LTV": "LOTUS VALE",
	"LTW": "PATUXENT/LEONARDTOWN",
	"LUA": "LUKLA",
	"LUB": "LUMID PAU",
	"LUC": "LAUCALA ISLAND",
	"LUD": "LUDERITZ",
	"LUE": "LUCENEC",
	"LUF": "PHOENIX",
	"LUG": "LUGANO",
	"LUH": "LUDHIANA",
	"LUI": "LA UNION",
	"LUJ": "LUSIKISIKI",
	"LUK": "CINCINNATI",
	"LUM": "LUXI",
	"LUN": "LUSAKA",
	"LUO": "LUENA",
	"LUP": "KALAUPAPA",
	"LUQ": "SAN LUIS",
	"LUR": "CAPE LISBURNE",
	"LUS": "LUSANGA",
	"LUT": "LAURA STATION",
	"LUU": "LAURA",
	"LUV": "LANGGUR",
	"LUW": "LUWUK",
	"LUX": "LUXEMBOURG",
	"LUY": "LUSHOTO",
	"LUZ": "LUSHAN",
	"LVA": "LAVAL",
	"LVB": "LIVRAMENTO",
	"LVD": "LIME VILLAGE",
	"LVI": "LIVINGSTONE",
	"LVK": "LIVERMORE",
	"LVL": "LAWRENCEVILLE",
	"LVM": "LIVINGSTON",
	"LVO": "LAVERTON",
	"LVP": "LAVAN",
	"LVS": "LAS VEGAS",
	"LWB": "GREENBRIER",
	"LWC": "LAWRENCE",
	"LWE": "LEWOLEBA",
	"LWH": "LAWN HILL",
	"LWI": "LOWAI",
	"LWK": "LERWICK",
	"LWL": "WELLS",
	"LWM": "LAWRENCE",
	"LWN": "LENINAKAN",
	"LWO": "LVOV",
	"LWR": "LEEUWARDEN",
	"LWS": "LEWISTON",
	"LWT": "LEWISTOWN",
	"LWV": "LAWRENCEVILLE",
	"LWY": "LAWAS",
	"LXA": "LHASA",
	"LXG": "LUONG-NAMTHA",
	"LXN": "LEXINGTON",
	"LXR": "LUXOR",
	"LXS": "LEMNOS",
	"LXU": "LUKULU",
	"LXV": "LEADVILLE",
	"LYA": "LUOYANG",
	"LYB": "LITTLE CAYMAN",
	"LYC": "LYCKSELE",
	"LYE": "LYNEHAM",
	"LYG": "LIANYUNGANG",
	"LYH": "LYNCHBURG",
	"LYK": "LUNYUK",
	"LYM": "ASHFORD",
	"LYN": "BRON AIRPORT",
	"LYO": "LYONS",
	"LYP": "FAISALABAD",
	"LYR": "LONGYEARBYEN",
	"LYS": "LYON",
	"LYT": "LADY ELLIOT ISLAND",
	"LYU": "ELY",
	"LYX": "LYDD",
	"LZA": "LUIZA",
	"LZC": "LAZARO CARDENAS",
	"LZD": "LANZHOU-LANZHOUDON",
	"LZH": "LIUZHOU",
	"LZI": "LUOZI",
	"LZO": "LUZHOU",
	"LZR": "LIZARD ISLAND",
	"MAA": "MADRAS",
	"MAB": "MARABA",
	"MAD": "MADRID",
	"MAE": "MADERA",
	"MAF": "MIDLAND/ODESSA",
	"MAG": "MADANG",
	"MAH": "MAHON",
	"MAI": "MANGOCHI",
	"MAJ": "MAJURO",
	"MAK": "MALAKAL",
	"MAL": "MANGOLE",
	"MAM": "MATAMOROS",
	"MAN": "MANCHESTER",
	"MAO": "MANAUS",
	"MAP": "MAMAI",
	"MAQ": "MAE SOT",
	"MAR": "MARACAIBO",
	"MAS": "MANUS IS.",
	"MAT": "MATADI",
	"MAU": "MAUPITI",
	"MAV": "MALOELAP",
	"MAW": "MALDEN",
	"MAX": "MATAM",
	"MAY": "MANGROVE CAY",
	"MAZ": "MAYAGUEZ",
	"MBA": "MOMBASA",
	"MBB": "MARBLE BAR",
	"MBC": "M'BIGOU",
	"MBD": "MIDDELBURG",
	"MBE": "MONBETSU",
	"MBF": "MOUNT BUFFALO",
	"MBG": "MOBRIDGE",
	"MBH": "MARYBOROUGH",
	"MBI": "MBEYA",
	"MBJ": "MONTEGO BAY",
	"MBK": "MATUPA",
	"MBL": "MANISTEE",
	"MBM": "MKAMBATI",
	"MBN": "MT. BARNETT",
	"MBO": "MAMBURAO",
	"MBP": "MOYOBAMBA",
	"MBQ": "MBARARA",
	"MBR": "MBOUT",
	"MBS": "SAGINAW",
	"MBT": "MASBATE",
	"MBU": "MBAMBANAKIRA",
	"MBV": "MASA",
	"MBW": "MOORABBIN",
	"MBX": "MARIBOR",
	"MBY": "MOBERLY",
	"MBZ": "MAUES",
	"MCA": "MACENTA",
	"MCB": "MCCOMB",
	"MCC": "SACRAMENTO",
	"MCD": "MACKINAC ISLAND",
	"MCE": "MERCED",
	"MCF": "TAMPA",
	"MCG": "MC GRATH",
	"MCH": "MACHALA",
	"MCI": "KANSAS CITY",
	"MCJ": "MAICAO",
	"MCK": "MC COOK",
	"MCL": "MCKINLEY NATIONAL PARK",
	"MCM": "MONTE CARLO",
	"MCN": "MACON",
	"MCO": "ORLANDO",
	"MCP": "MACAPA",
	"MCQ": "MISKOLC",
	"MCR": "MELCHOR DE MENCOS",
	"MCS": "MONTE CASEROS",
	"MCT": "MUSCAT",
	"MCU": "MONTLUCON",
	"MCV": "MAC ARTHUR RIVER",
	"MCW": "MASON CITY",
	"MCX": "MAKHACHKALA",
	"MCY": "MAROOCHYDORE",
	"MCZ": "MACEIO",
	"MDB": "MELINDA",
	"MDC": "MENADO",
	"MDD": "MIDLAND/ODESSA",
	"MDE": "MEDELLIN",
	"MDF": "MEDFORD",
	"MDG": "MUDANJIANG",
	"MDH": "CARBONDALE",
	"MDI": "MAKURDI",
	"MDJ": "MADRAS",
	"MDK": "MBANDAKA",
	"MDL": "MANDALAY",
	"MDM": "MUNDUKU",
	"MDN": "MADISON",
	"MDO": "MIDDLETON ISLAND",
	"MDP": "MINDIPTANA",
	"MDQ": "MAR DEL PLATA",
	"MDR": "MEDFRA",
	"MDS": "MIDDLE CAICOS",
	"MDT": "HARRISBURG",
	"MDU": "MENDI",
	"MDV": "MEDOUNEU",
	"MDW": "CHICAGO",
	"MDX": "MERCEDES",
	"MDY": "MIDWAY ISLAND",
	"MDZ": "MENDOZA",
	"MEA": "MACAE",
	"MEB": "MELBOURNE",
	"MEC": "MANTA",
	"MED": "MEDINA",
	"MEE": "MARE",
	"MEF": "MELFI",
	"MEG": "MALANGE",
	"MEH": "MEHAMN",
	"MEI": "MERIDIAN",
	"MEJ": "MEADVILLE",
	"MEK": "MEKNES",
	"MEL": "MELBOURNE",
	"MEM": "MEMPHIS",
	"MEN": "MENDE",
	"MEO": "MANTEO",
	"MEP": "MERSING",
	"MEQ": "MEULABOH",
	"MES": "MEDAN",
	"MET": "MORETON",
	"MEU": "MONTE DOURADO",
	"MEV": "MINDEN",
	"MEW": "MWEKA",
	"MEX": "MEXICO CITY",
	"MEY": "MEGHAULI",
	"MEZ": "MESSINA",
	"MFA": "MAFIA IS.",
	"MFB": "MONFORT",
	"MFC": "MAFETENG",
	"MFD": "MANSFIELD",
	"MFE": "MISSION/MCALLEN",
	"MFF": "MOANDA",
	"MFG": "MUZAFFARABAD",
	"MFH": "MESQUITE",
	"MFI": "WAUKESHA",
	"MFJ": "MOALA",
	"MFK": "MATSU",
	"MFL": "MT. FULL STOP",
	"MFM": "MACAU",
	"MFN": "MILFORD SOUND",
	"MFO": "RAS NASRANI",
	"MFP": "MANNERS CREEK",
	"MFQ": "MARADI",
	"MFR": "MEDFORD",
	"MFS": "MIRAFLORES",
	"MFT": "MACHU PICCHU",
	"MFU": "MFUWE",
	"MFV": "MELFA",
	"MFX": "MERIBEL",
	"MFY": "MAIFA",
	"MGA": "MANAGUA",
	"MGB": "MT. GAMBIER",
	"MGC": "MICHIGAN CITY",
	"MGD": "MAGDALENA",
	"MGE": "MARIETTA",
	"MGF": "MARINGA",
	"MGG": "MARGARIMA",
	"MGH": "MARGATE",
	"MGJ": "MONTGOMERY",
	"MGL": "MOENCHENGLADBACH",
	"MGM": "MONTGOMERY",
	"MGN": "MAGANGUE",
	"MGO": "MANEGA",
	"MGP": "MANGA",
	"MGQ": "MOGADISHU",
	"MGR": "MOULTRIE/THOMASVILLE",
	"MGS": "MANGAIA",
	"MGT": "MILLINGIMBIE",
	"MGU": "MANAUNG",
	"MGV": "MARGARET RIVER",
	"MGW": "MORGANTOWN",
	"MGX": "MOABI",
	"MGY": "DAYTON",
	"MGZ": "MERGUI",
	"MHA": "MAHDIA",
	"MHB": "AUCKLAND",
	"MHD": "MASHAD",
	"MHE": "MITCHELL",
	"MHF": "MORICHAL",
	"MHG": "MANNHEIM",
	"MHH": "MARSH HARBOUR",
	"MHI": "MUSHA",
	"MHJ": "MISRAK GASHAMO",
	"MHK": "MANHATTAN",
	"MHL": "MARSHALL",
	"MHM": "MINCHUMINA",
	"MHN": "MULLEN",
	"MHO": "MT. HOUSE",
	"MHQ": "MARIEHAMN",
	"MHR": "SACRAMENTO",
	"MHS": "MOUNT SHASTA",
	"MHT": "MANCHESTER",
	"MHU": "MOUNT HOTHAM",
	"MHV": "MOJAVE",
	"MHX": "MANIHIKI ISLAND",
	"MHY": "MOREHEAD",
	"MHZ": "MILDENHALL",
	"MIA": "MIAMI",
	"MIB": "MINOT",
	"MIC": "MINNEAPOLIS/ST PAUL",
	"MID": "MERIDA",
	"MIE": "MUNCIE",
	"MIF": "MONAHANS",
	"MIH": "MITCHELL PLATEAU",
	"MII": "MARILIA",
	"MIJ": "MILI",
	"MIK": "MIKKELI",
	"MIL": "MILAN",
	"MIM": "MERIMBULA",
	"MIN": "MINNIPA",
	"MIO": "MIAMI",
	"MIP": "MITSPEH RAMON",
	"MIQ": "OMAHA",
	"MIR": "MONASTIR",
	"MIS": "MISIMA",
	"MIT": "SHAFTER",
	"MIU": "MAIDUGURI",
	"MIV": "MILLVILLE",
	"MIW": "MARSHALLTOWN",
	"MIX": "MIRITI",
	"MIY": "MITTIEBAH",
	"MIZ": "MAINORU",
	"MJA": "MANJA",
	"MJB": "MEJIT",
	"MJC": "MAN",
	"MJD": "MOHENJO DARO",
	"MJE": "MAJKIN",
	"MJF": "MOSJOEN",
	"MJG": "MAYAJIGUA",
	"MJH": "MAJMA",
	"MJI": "MAJI",
	"MJJ": "MOKI",
	"MJK": "MONKEY MIA",
	"MJL": "MOUILA",
	"MJM": "MBUJI-MAYI",
	"MJN": "MAJUNGA",
	"MJO": "MOUNT ETJO LODGE",
	"MJP": "MANJIMUP",
	"MJQ": "JACKSON",
	"MJR": "MIRAMAR",
	"MJT": "MYTILENE",
	"MJU": "MAMUJU",
	"MJV": "MURCIA",
	"MJX": "TOMS RIVER",
	"MJY": "MANGUNJAYA",
	"MJZ": "MIRNYJ",
	"MKA": "MARIANSKE LAZNE",
	"MKB": "MEKAMBO",
	"MKC": "KANSAS CITY",
	"MKD": "METEKEL",
	"MKE": "MILWAUKEE",
	"MKG": "MUSKEGON",
	"MKH": "MOKHOTLONG",
	"MKI": "M'BOKI",
	"MKJ": "MAKOUA",
	"MKK": "MOLOKAI/KAUNAKAK",
	"MKL": "JACKSON",
	"MKM": "MUKAH",
	"MKN": "MALEKOLON",
	"MKO": "MUSKOGEE",
	"MKP": "MAKEMO",
	"MKQ": "MERAUKE",
	"MKR": "MEEKATHARRA",
	"MKS": "MEKANE SELAM",
	"MKT": "MANKATO",
	"MKU": "MAKOKOU",
	"MKV": "MT. CAVENAGH",
	"MKW": "MANOKWARI",
	"MKX": "AL-MUKALLA",
	"MKY": "MACKAY",
	"MKZ": "MALACCA",
	"MLA": "MALTA",
	"MLB": "MELBOURNE",
	"MLC": "MCALESTER",
	"MLD": "MALAD CITY",
	"MLE": "MALE",
	"MLF": "MILFORD",
	"MLG": "MALANG",
	"MLH": "MULHOUSE",
	"MLI": "MOLINE",
	"MLJ": "MILLEDGEVILLE",
	"MLK": "MALTA",
	"MLL": "MARSHALL",
	"MLM": "MORELIA",
	"MLN": "MELILLA",
	"MLO": "MILOS",
	"MLP": "MALABANG",
	"MLQ": "MALALAUA",
	"MLR": "MILLICENT",
	"MLS": "MILES CITY",
	"MLT": "MILLINOCKET",
	"MLU": "MONROE",
	"MLV": "MERLUNA",
	"MLW": "MONROVIA",
	"MLX": "MALATYA",
	"MLY": "MANLEY HOT SPRINGS",
	"MLZ": "MELO",
	"MMA": "MALMO",
	"MMB": "MEMANBETSU",
	"MMC": "CIUDAD MANTE",
	"MMD": "MINAMI DAITO JIMA",
	"MME": "TEES-SIDE",
	"MMF": "MAMFE",
	"MMG": "MT. MAGNET",
	"MMH": "MAMMOTH LAKES",
	"MMI": "ATHENS",
	"MMJ": "MATSUMOTO",
	"MMK": "MURMANSK",
	"MML": "MARSHALL",
	"MMM": "MIDDLEMOUNT",
	"MMN": "STOW",
	"MMO": "MAIO",
	"MMP": "MOMPOS",
	"MMQ": "MBALA",
	"MMS": "MARKS",
	"MMT": "COLUMBIA",
	"MMU": "MORRISTOWN",
	"MMW": "MOMA",
	"MMX": "MALMO-STURUP",
	"MMY": "MIYAKO JIMA",
	"MMZ": "MAIMANA",
	"MNA": "MELANGUANE",
	"MNB": "MOANDA",
	"MNC": "NACALA",
	"MND": "MEDINA",
	"MNE": "MUNGARANIE",
	"MNF": "MANA",
	"MNG": "MANINGRIDA",
	"MNH": "MINNERIYA",
	"MNI": "MONTSERRAT",
	"MNJ": "MANANJARY",
	"MNK": "MAIANA",
	"MNL": "MANILA",
	"MNM": "MENOMINEE",
	"MNN": "MARION",
	"MNO": "MANONO",
	"MNP": "MARON",
	"MNQ": "MONTO",
	"MNR": "MONGU",
	"MNS": "MANSA",
	"MNT": "MINTO",
	"MNU": "MAULMYINE",
	"MNV": "MOUNTAIN VALLEY",
	"MNW": "MACDONALD DOWNS",
	"MNX": "MANICORE",
	"MNY": "MONO IS.",
	"MNZ": "MANASSAS",
	"MOA": "MOA",
	"MOB": "MOBILE",
	"MOC": "MONTES CLAROS",
	"MOD": "MODESTO",
	"MOE": "MOMEIK",
	"MOF": "MAUMERE",
	"MOG": "MONGHSAT",
	"MOH": "MOHANBARI",
	"MOI": "MITIARO",
	"MOJ": "MOENGO",
	"MOK": "MANKONO",
	"MOL": "MOLDE",
	"MOM": "MOUDJERIA",
	"MON": "MOUNT COOK",
	"MOO": "MOOMBA",
	"MOP": "MT. PLEASANT",
	"MOQ": "MORONDAVA",
	"MOR": "MORRISTOWN",
	"MOS": "MOSES POINT",
	"MOT": "MINOT",
	"MOU": "MOUNTAIN VILLAGE",
	"MOV": "MORANBAH",
	"MOW": "MOSCOW",
	"MOX": "MORRIS",
	"MOY": "MONTERREY",
	"MOZ": "MOOREA IS.",
	"MPA": "MPACHA",
	"MPB": "MIAMI",
	"MPC": "MUKO-MUKO",
	"MPD": "MIRPUR KHAS",
	"MPE": "MADISON",
	"MPF": "MAPODA",
	"MPG": "MAKINI",
	"MPH": "MALAY",
	"MPI": "MAMITUPO",
	"MPJ": "MORRILTON",
	"MPK": "MOKPO",
	"MPL": "MONTPELLIER",
	"MPM": "MAPUTO",
	"MPN": "MOUNT PLEASANT",
	"MPO": "MT. POCONO",
	"MPP": "MULATUPO",
	"MPQ": "MAAN",
	"MPR": "MCPHERSON",
	"MPS": "MOUNT PLEASANT",
	"MPT": "MALIANA",
	"MPU": "MAPUA",
	"MPV": "MONTPELIER/BARRE",
	"MPW": "MARIUPOL",
	"MPX": "MIYANMIN",
	"MPY": "MARIPASOULA",
	"MPZ": "MT. PLEASANT",
	"MQA": "MANDORA",
	"MQB": "MACOMB",
	"MQC": "MIQUELON",
	"MQD": "MAQUINCHAO",
	"MQE": "MARQUA",
	"MQF": "MAGNITOGORSK",
	"MQH": "MINACU",
	"MQI": "QUINCY",
	"MQL": "MILDURA",
	"MQN": "MO-I-RANA",
	"MQQ": "MOUNDOU",
	"MQR": "MOSQUERA",
	"MQS": "MUSTIQUE",
	"MQT": "MARQUETTE",
	"MQU": "MARIQUITA",
	"MQV": "MAIQUETIA",
	"MQW": "MC RAE",
	"MQX": "MAKALE",
	"MQY": "SMYRNA",
	"MQZ": "MAQUELA",
	"MRA": "MISURATA",
	"MRB": "MARTINSBURG",
	"MRC": "COLUMBIA",
	"MRD": "MERIDA",
	"MRE": "MARA LODGES",
	"MRF": "MARFA",
	"MRG": "MAREEBA",
	"MRI": "ANCHORAGE",
	"MRJ": "MARCALA",
	"MRK": "MARCO ISLAND",
	"MRL": "MINERS LAKE",
	"MRM": "MANARI",
	"MRN": "MORGANTON",
	"MRO": "MASTERTON",
	"MRP": "MARLA",
	"MRQ": "MARINDUQUE",
	"MRR": "MACARA",
	"MRS": "MARSEILLE",
	"MRT": "MOROAK RIVER",
	"MRU": "MAURITIUS",
	"MRV": "MINERALNYE VODY",
	"MRW": "MORIOKA",
	"MRX": "MARGARITA ISLAND",
	"MRY": "MONTEREY",
	"MRZ": "MOREE",
	"MSA": "MUSKRAT DAM",
	"MSB": "MARIGOT",
	"MSC": "MESA",
	"MSD": "MT. PLEASANT",
	"MSE": "MANSTON",
	"MSF": "MT. SWAN",
	"MSG": "MATSAILE",
	"MSH": "MASIRAH",
	"MSJ": "MISAWA",
	"MSL": "MUSCLE SHOALS",
	"MSM": "MASI-MANIMBA",
	"MSN": "MADISON",
	"MSO": "MISSOULA",
	"MSP": "MINNEAPOLIS/ST.PAUL",
	"MSQ": "MINSK",
	"MSR": "MUS",
	"MSS": "MASSENA",
	"MST": "MAASTRICHT",
	"MSU": "MASERU",
	"MSV": "MONTICELLO",
	"MSW": "MASSAWA",
	"MSX": "MOSSENDJO",
	"MSY": "NEW ORLEANS",
	"MSZ": "MOSSAMEDES",
	"MTA": "MATAMATA",
	"MTB": "MONTE LIBANO",
	"MTC": "MOUNT CLEMENS",
	"MTD": "MT. SANDFORD",
	"MTE": "MONTE ALEGRE",
	"MTF": "MIZAN TEFERI",
	"MTG": "MATO GROSSO",
	"MTH": "MARATHON",
	"MTI": "MOSTEIROS",
	"MTJ": "MONTROSE",
	"MTK": "MAKIN",
	"MTL": "MAITLAND",
	"MTM": "METLAKATLA",
	"MTN": "BALTIMORE",
	"MTO": "MATTOON",
	"MTP": "MONTAUK POINT",
	"MTQ": "MITCHELL",
	"MTR": "MONTERIA",
	"MTS": "MANZINI",
	"MTT": "MINATITLAN",
	"MTU": "MONTEPUEZ",
	"MTV": "MOTA LAVA",
	"MTW": "MANITOWOC",
	"MTX": "FAIRBANKS",
	"MTY": "MONTERREY",
	"MTZ": "MASADA",
	"MUA": "MUNDA",
	"MUB": "MAUN",
	"MUC": "MUNICH",
	"MUD": "MUEDA",
	"MUE": "KAMUELA",
	"MUF": "MUTING",
	"MUG": "MULEGE",
	"MUH": "MERSA MATRUH",
	"MUJ": "MUI",
	"MUK": "MAUKE",
	"MUM": "MUMIAS",
	"MUN": "MATURIN",
	"MUO": "MOUNTAIN HOME",
	"MUP": "MULGA PARK",
	"MUQ": "MUCCAN",
	"MUR": "MARUDI",
	"MUT": "MUSCATINE",
	"MUU": "MOUNT UNION",
	"MUW": "MASCARA",
	"MUX": "MULTAN",
	"MUY": "MOUYONDZI",
	"MUZ": "MUSOMA",
	"MVA": "MYVATN",
	"MVB": "MVENGUE",
	"MVC": "MONROEVILLE",
	"MVD": "MONTEVIDEO",
	"MVE": "MONTEVIDEO",
	"MVF": "MOSSORO",
	"MVG": "MEVANG",
	"MVH": "MACKSVILLE",
	"MVI": "MANETAI",
	"MVJ": "MANDEVILLE",
	"MVK": "MULKA",
	"MVL": "MORRISVILLE",
	"MVM": "KAYENTA",
	"MVN": "MOUNT VERNON",
	"MVO": "MONGO",
	"MVP": "MITU",
	"MVQ": "MOGILEV",
	"MVR": "MAROUA",
	"MVS": "MUCURI",
	"MVT": "MATAIVA",
	"MVU": "MUSGRAVE",
	"MVV": "MEGEVE",
	"MVW": "MOUNT VERNON",
	"MVX": "MINVOUL",
	"MVY": "MARTHA'S VINEYARD",
	"MVZ": "MASVINGO",
	"MWA": "MARION",
	"MWB": "MORAWA",
	"MWC": "MILWAUKEE",
	"MWD": "MIANWALI",
	"MWE": "MEROWE",
	"MWF": "MAEWO",
	"MWG": "MARAWAKA",
	"MWH": "MOSES LAKE",
	"MWI": "MARAMUNI",
	"MWJ": "MATTHEW RIDGE",
	"MWK": "MATAK",
	"MWL": "MINERAL WELLS",
	"MWM": "WINDOM",
	"MWN": "MWADUI",
	"MWO": "MIDDLETOWN",
	"MWP": "MOUNTAIN",
	"MWQ": "MAGWE",
	"MWS": "MOUNT WILSON",
	"MWT": "MOOLAWATANA",
	"MWU": "MUSSAU",
	"MWX": "MOSS TOWN",
	"MWY": "MIRANDA DOWNS",
	"MWZ": "MWANZA",
	"MXA": "MANILA",
	"MXB": "MASAMBA",
	"MXC": "MONTICELLO",
	"MXD": "MARION DOWNS",
	"MXE": "MAXTON",
	"MXF": "MONTGOMERY",
	"MXG": "MARLBOROUGH",
	"MXH": "MORO",
	"MXI": "MATI",
	"MXJ": "MINNA",
	"MXK": "MINDIK",
	"MXL": "MEXICALI",
	"MXM": "MOROMBE",
	"MXN": "MORLAIX",
	"MXO": "MONTICELLO",
	"MXP": "MILAN",
	"MXQ": "MITCHELL RIVER",
	"MXR": "MIRGOROD",
	"MXS": "MAOTA",
	"MXT": "MAINTIRANO",
	"MXU": "MULLEWA",
	"MXV": "MORON",
	"MXW": "MANDALGOBI",
	"MXX": "MORA",
	"MXY": "MC CARTHY",
	"MXZ": "MEIXIAN",
	"MYA": "MORUYA",
	"MYB": "MAYOUMBA",
	"MYC": "MARACAY",
	"MYD": "MALINDI",
	"MYE": "MIYAKE JIMA",
	"MYF": "SAN DIEGO",
	"MYG": "MAYAGUANA",
	"MYH": "MARBLE CANYON",
	"MYI": "MURRAY ISLANDS",
	"MYJ": "MATSUYAMA",
	"MYK": "MAY CREEK",
	"MYL": "MCCALL",
	"MYM": "MONKEY MOUNTAIN",
	"MYN": "MAREB",
	"MYO": "MYROODAH",
	"MYP": "MARY",
	"MYQ": "MYSORE",
	"MYR": "MYRTLE BEACH",
	"MYS": "MOYALE",
	"MYT": "MYITKYINA",
	"MYU": "MEKORYUK",
	"MYV": "MARYSVILLE",
	"MYW": "MTWARA",
	"MYX": "MENYAMYA",
	"MYY": "MIRI",
	"MYZ": "MONKEY BAY",
	"MZA": "MUZAFFARNAGAR",
	"MZB": "MOCIMBOA DE PRAIA",
	"MZC": "MITZIC",
	"MZD": "MENDEZ",
	"MZE": "MANATEE",
	"MZF": "MZAMBA",
	"MZG": "MAKUNG",
	"MZH": "MERZIFON",
	"MZI": "MOPTI",
	"MZJ": "MARANA",
	"MZK": "MARAKEI",
	"MZL": "MANIZALES",
	"MZM": "METZ",
	"MZN": "MINJ",
	"MZO": "MANZANILLO",
	"MZP": "MOTUEKA",
	"MZQ": "CASTRES",
	"MZR": "MAZAR-I-SHERIF",
	"MZS": "MOSTYN",
	"MZT": "MAZATLAN",
	"MZU": "MUZAFFARPUR",
	"MZV": "MULU",
	"MZX": "MASSLO",
	"MZY": "MZIMBA",
	"MZZ": "MARION",
	"NAA": "NARRABRI",
	"NAC": "NARACOORTE",
	"NAD": "MACANAL",
	"NAE": "NATITINGOU",
	"NAF": "BANAINA",
	"NAG": "NAGPUR",
	"NAH": "NAHA",
	"NAI": "ANNAI",
	"NAK": "NAKHON RATCHASIMA",
	"NAL": "NALCHIK",
	"NAM": "NAMLEA",
	"NAN": "NADI",
	"NAO": "NANCHONG",
	"NAP": "NAPLES",
	"NAR": "NARE",
	"NAS": "NASSAU",
	"NAT": "NATAL",
	"NAU": "NAPUKA",
	"NAV": "NATIVIDADE",
	"NAW": "NARATHIWAT",
	"NAX": "BARBERS POINT",
	"NAY": "BEIJING-NANYUAN",
	"NBA": "NAMBAIYUFA",
	"NBB": "BARRANCOMINAS",
	"NBC": "NABEREVNYE CHELNY",
	"NBG": "NEW ORLEANS",
	"NBH": "NAMBUCCA HEADS",
	"NBL": "SAN BLAS",
	"NBO": "NAIROBI",
	"NBR": "NAMBOUR",
	"NBU": "GLENVIEW",
	"NBV": "CANA BRAVA",
	"NBW": "GUANTANAMO BAY NAS",
	"NBX": "NABIRE",
	"NCA": "NORTH CAICOS",
	"NCE": "NICE",
	"NCG": "NUEVA CASAS GRANDES",
	"NCH": "NACHINGWEA",
	"NCI": "NECOCLI",
	"NCL": "NEWCASTLE",
	"NCN": "NEW CHENEGAL",
	"NCO": "QUONSET POINT",
	"NCP": "LUZON ISLAND",
	"NCR": "SAN CARLOS",
	"NCS": "NEWCASTLE",
	"NCT": "NICOYA",
	"NCU": "NUKUS",
	"NCY": "ANNECY",
	"NDA": "BANDANAIRA",
	"NDB": "NOUADHIBOU",
	"NDC": "NANDED",
	"NDD": "N'GUNZA",
	"NDE": "MANDERA",
	"NDG": "QIQIHAR",
	"NDI": "NAMUDI",
	"NDJ": "N'DJAMENA",
	"NDK": "NAMORIK",
	"NDL": "NDELE",
	"NDM": "MENDI",
	"NDN": "NADUNUMU",
	"NDR": "NADOR",
	"NDS": "SANDSTONE",
	"NDU": "RUNDU",
	"NDY": "SANDAY",
	"NDZ": "NORDHOLZ-SPIEKA",
	"NEC": "NECOCHEA",
	"NEF": "NEFTEKAMSK",
	"NEG": "NEGRIL",
	"NEJ": "NEJJO",
	"NEK": "NEKEMT",
	"NEL": "LAKEHURST",
	"NEN": "WHITEHOUSE",
	"NER": "NERYUNGRI",
	"NES": "NEW YORK",
	"NEU": "SAM NEUA",
	"NEV": "NEVIS",
	"NEW": "NEW ORLEANS",
	"NFG": "NEFTEYUGANSK",
	"NFL": "FALLON",
	"NFO": "NIUAFO'OU",
	"NGA": "YOUNG",
	"NGB": "NINGBO",
	"NGC": "GRAND CANYON",
	"NGD": "ANEGADA",
	"NGE": "N'GAOUNDERE",
	"NGF": "KANEOHE BAY",
	"NGI": "NGAU",
	"NGL": "NGALA",
	"NGM": "AGANA NAS",
	"NGN": "NARGANA",
	"NGO": "NAGOYA",
	"NGP": "CORPUS CHRISTI",
	"NGR": "NIGERUM",
	"NGS": "NAGASAKI",
	"NGU": "NORFOLK",
	"NGW": "CORPUS CHRISTI",
	"NGX": "MANANG",
	"NGZ": "ALAMEDA",
	"NHA": "NHA-TRANG",
	"NHD": "MINHAD AB",
	"NHF": "NEW HALFA",
	"NHK": "PATUXENT RIVER",
	"NHS": "NUSHKI",
	"NHT": "NORTHOLT",
	"NHV": "NUKU HIVA",
	"NHX": "FOLEY",
	"NHZ": "BRUNSWICK",
	"NIA": "NIMBA",
	"NIB": "NIKOLAI",
	"NIE": "NIBLACK",
	"NIF": "NIFTY",
	"NIG": "NIKUNAU",
	"NIK": "NIOKOLO KOBA",
	"NIM": "NIAMEY",
	"NIN": "NINILCHIK",
	"NIO": "NIOKI",
	"NIP": "JACKSONVILLE",
	"NIT": "NIORT",
	"NIX": "NIORO",
	"NJC": "NIZHNEVARTOVSK",
	"NKA": "NKAN",
	"NKB": "NOONKANBAH",
	"NKC": "NOUAKCHOTT",
	"NKD": "SINAK",
	"NKG": "NANJING",
	"NKI": "NAUKITI",
	"NKL": "NKOLO",
	"NKN": "NANKINA",
	"NKS": "NKONGSAMBA",
	"NKT": "CHERRY POINT MCAS",
	"NKU": "NKAUS",
	"NKV": "NICHEN COVE",
	"NKW": "DIEGO GARCIA ATOLL",
	"NKX": "SAN DIEGO",
	"NKY": "NKAYI",
	"NLA": "NDOLA",
	"NLD": "NUEVO LAREDO",
	"NLE": "NILES",
	"NLF": "DARNLEY ISLAND",
	"NLG": "NELSON LAGOON",
	"NLK": "NORFOLK IS.",
	"NLL": "NULLAGINE",
	"NLO": "KINSHASA-N'DOLO",
	"NLP": "NELSPRUIT",
	"NLS": "NICHOLSON",
	"NLV": "NIKOLAEV",
	"NMA": "NAMANGAN",
	"NMB": "DAMAN",
	"NMC": "NORMAN'S CAY",
	"NME": "NIGHTMUTE",
	"NMG": "SAN MIGUEL",
	"NMN": "NOMANE",
	"NMP": "NEW MOON",
	"NMR": "NAPPAMERRIE",
	"NMS": "NAMSANG",
	"NMT": "NAMTU",
	"NMU": "NAMU",
	"NNB": "SANTA ANA",
	"NND": "NANGADE",
	"NNG": "NANNING",
	"NNI": "NAMUTONI",
	"NNK": "NAKNEK",
	"NNL": "NONDALTON",
	"NNM": "NARYAN-MAR",
	"NNR": "SPIDDAL",
	"NNT": "NAN",
	"NNU": "NANUQUE",
	"NNX": "NUNUKAN",
	"NNY": "NANYANG",
	"NOA": "NOWRA",
	"NOB": "NOSARA BEACH",
	"NOC": "CONNAUGHT",
	"NOD": "NORDEN",
	"NOE": "NORDDEICH",
	"NOG": "NOGALES",
	"NOI": "NOVOROSSIJSK",
	"NOJ": "NOJABRXSK",
	"NOK": "NOVA XAVANTINA",
	"NOL": "PORT NOLLOTH",
	"NOM": "NOMAD RIVER",
	"NON": "NONOUTI",
	"NOO": "NAORO",
	"NOP": "MACTAN ISLAND",
	"NOQ": "NORD",
	"NOR": "NORDFJORDUR",
	"NOS": "NOSSI-BE",
	"NOT": "NOVATO",
	"NOU": "NOUMEA",
	"NOV": "HUAMBO",
	"NOZ": "NOVOKUZNETSK",
	"NPA": "PENSACOLA",
	"NPE": "NAPIER",
	"NPG": "NIPA",
	"NPH": "NEPHI",
	"NPL": "NEW PLYMOUTH",
	"NPO": "NANGAPINOH",
	"NPP": "NAPPERBY",
	"NPT": "NEWPORT",
	"NQA": "MEMPHIS",
	"NQI": "KINGSVILLE",
	"NQL": "NIQUELANDIA",
	"NQN": "NEUQUEN",
	"NQU": "NUQUI",
	"NQX": "KEY WEST",
	"NQY": "NEWQUAY",
	"NRA": "NARRANDERA",
	"NRB": "MAYPORT",
	"NRC": "CROWS LANDING",
	"NRD": "NORDERNEY",
	"NRE": "NAMROLE",
	"NRG": "NARROGIN",
	"NRI": "SHANGRI-LA",
	"NRK": "NORRKOPING",
	"NRL": "NORTH RONALDSAY",
	"NRM": "NARA",
	"NRR": "ROOSEVELT ROADS",
	"NRT": "TOKYO",
	"NRY": "NEWRY",
	"NSA": "NOOSA",
	"NSB": "BIMINI",
	"NSE": "MILTON",
	"NSH": "NOWSHAHR",
	"NSI": "YAOUNDE-NSIMALEN",
	"NSK": "NORIL'SK",
	"NSM": "NORSEMAN",
	"NSN": "NELSON",
	"NSO": "SCONE",
	"NST": "NAKHON SI THAMMARAT",
	"NSV": "NOOSAVILLE",
	"NSX": "NORTH SOUND VIRGIN GORDA",
	"NSY": "SIGONELLA",
	"NTA": "NATADOLA",
	"NTB": "NOTODDEN",
	"NTD": "PORT HUENEME",
	"NTE": "NANTES",
	"NTG": "NANTONG",
	"NTI": "BINTUNI",
	"NTJ": "MANTI",
	"NTL": "NEWCASTLE",
	"NTM": "MIRACEMA DO NORTE",
	"NTN": "NORMANTON",
	"NTO": "SANTO ANTAO",
	"NTR": "MONTERREY",
	"NTT": "NUIATOPUTAPU",
	"NTU": "OCEANA",
	"NTX": "NATUNA RANAI",
	"NTY": "SUN CITY",
	"NUB": "NUMBULWAR",
	"NUD": "EN NAHUD",
	"NUE": "NUREMBERG",
	"NUG": "NUGURIA",
	"NUH": "NUNCHIA",
	"NUI": "NUIQSUT",
	"NUK": "NUKUTAVAKE",
	"NUL": "NULATO",
	"NUP": "NUNAPITCHUK",
	"NUQ": "MOUNTAIN VIEW",
	"NUR": "NULLARBOR",
	"NUS": "NORSUP",
	"NUT": "NUTUVE",
	"NUU": "NAKURU",
	"NUW": "WHIDBEY ISLAND",
	"NUX": "NOVYJ URENGOJ",
	"NVA": "NEIVA",
	"NVD": "NEVADA",
	"NVG": "NUEVA GUINEA",
	"NVK": "NARVIK",
	"NVP": "NOVO ARIPUANA",
	"NVR": "NOVGOROD",
	"NVS": "NEVERS",
	"NVT": "NAVEGANTES",
	"NVY": "NEYVELI",
	"NWA": "MOHELI",
	"NWH": "NEWPORT",
	"NWI": "NORWICH",
	"NWS": "NEW YORK",
	"NWT": "NOWATA",
	"NWU": "BERMUDA NAS",
	"NXX": "WILLOW GROVE",
	"NYC": "NEW YORK",
	"NYE": "NYERI",
	"NYI": "SUNYANI",
	"NYK": "NANYUKI",
	"NYM": "NADYM",
	"NYN": "NYNGAN",
	"NYO": "NYKOPING",
	"NYU": "NYAUNG-U",
	"NZC": "JACKSONVILLE",
	"NZE": "NZEREKORE",
	"NZJ": "SANTA ANA",
	"NZO": "NZOIA",
	"NZW": "SOUTH WEYMOUTH",
	"NZY": "SAN DIEGO",
	"OAG": "ORANGE",
	"OAJ": "JACKSONVILLE",
	"OAK": "OAKLAND",
	"OAL": "CACOAL",
	"OAM": "OAMARU",
	"OAN": "OLANCHITO",
	"OAX": "OAXACA",
	"OBA": "OBAN",
	"OBC": "OBOCK",
	"OBD": "OBANO",
	"OBE": "OKEECHOBEE",
	"OBF": "OBERPFAFFENHOFEN",
	"OBI": "OBIDOS",
	"OBK": "NORTHBROOK",
	"OBM": "MOROBE",
	"OBN": "OBAN",
	"OBO": "OBIHIRO",
	"OBS": "AUBENAS",
	"OBT": "OAKLAND",
	"OBU": "KOBUK",
	"OBX": "OBO",
	"OBY": "SCORESBYSUND",
	"OCA": "OCEAN REEF CLUB",
	"OCC": "COCA",
	"OCE": "OCEAN CITY",
	"OCF": "OCALA",
	"OCH": "NACOGDOCHES",
	"OCI": "OCEANIC",
	"OCJ": "OCHO RIOS",
	"OCN": "OCEANSIDE",
	"OCV": "OCANA",
	"OCW": "WASHINGTON",
	"ODA": "OUADDA",
	"ODB": "CORDOBA",
	"ODD": "OODNADATTA",
	"ODE": "ODENSE",
	"ODJ": "OUANDA DJALLE",
	"ODL": "CORDILLO DOWNS",
	"ODM": "OAKLAND",
	"ODN": "LONG SERIDAN",
	"ODR": "ORD RIVER",
	"ODS": "ODESSA",
	"ODW": "OAK HARBOR",
	"ODY": "OUDOMXAY",
	"OEA": "VINCENNES",
	"OEC": "OCUSSI",
	"OEL": "OREL",
	"OEM": "PALOEMEU",
	"OEO": "OSCEOLA",
	"OER": "ORNSKOLDSVIK",
	"OES": "SAN ANTONIO OESTE",
	"OFF": "OMAHA",
	"OFI": "OUANGO FITINI",
	"OFJ": "OLAFSFJORDUR",
	"OFK": "NORFOLK",
	"OFU": "OFU ISLAND",
	"OGA": "OGALLALA",
	"OGB": "ORANGEBURG",
	"OGD": "OGDEN",
	"OGE": "OGERANANG",
	"OGG": "KAHULUI",
	"OGL": "OGLE",
	"OGN": "YONAGUNI",
	"OGO": "ABENGOUROU",
	"OGR": "BONGOR",
	"OGS": "OGDENSBURG",
	"OGX": "OUARGLA",
	"OGZ": "VLADIKAVKAZ (ORDZHONIKIDZE)",
	"OHA": "OHAKEA",
	"OHC": "NORTH EAST CAPE",
	"OHD": "OHRID",
	"OHG": "OGHSENOKALE",
	"OHI": "OSHAKATI",
	"OHO": "OHOTSK",
	"OHP": "OBAN-HLPT",
	"OHR": "WYK AUF FOEHR",
	"OHT": "KOHAT",
	"OIM": "OSHIMA IS.",
	"OIR": "OKUSHIRI",
	"OIT": "OITA",
	"OJC": "KANSAS CITY",
	"OKA": "OKINAWA",
	"OKB": "ORCHID BEACH",
	"OKC": "OKLAHOMA CITY",
	"OKD": "SAPPORO",
	"OKE": "OKINO ERABU",
	"OKF": "OKAUKUEJO",
	"OKG": "OKOYO",
	"OKI": "OKI ISLAND",
	"OKJ": "OKAYAMA",
	"OKK": "KOKOMO",
	"OKL": "OKSIBIL",
	"OKM": "OKMULGEE",
	"OKN": "OKONDJA",
	"OKO": "YOKOTA AFB",
	"OKP": "OKSAPMIN",
	"OKQ": "OKABA",
	"OKR": "YORKE ISLANDS",
	"OKS": "OSHKOSH",
	"OKT": "OKTIABRSKIJ",
	"OKU": "MOKUTI LODGE",
	"OKY": "OAKEY",
	"OLA": "ORLAND",
	"OLB": "OLBIA",
	"OLD": "OLD TOWN",
	"OLE": "OLEAN",
	"OLF": "WOLF POINT",
	"OLH": "OLD HARBOR",
	"OLI": "OLAFSVIK",
	"OLJ": "OLPOI",
	"OLM": "OLYMPIA",
	"OLN": "COLONIA SARMIENTO",
	"OLO": "OLOMOUC",
	"OLP": "OLYMPIC DAM",
	"OLQ": "OLSOBIP",
	"OLS": "NOGALES",
	"OLU": "COLUMBUS",
	"OLV": "OLIVE BRANCH",
	"OLY": "OLNEY",
	"OMA": "OMAHA",
	"OMB": "OMBOUE",
	"OMC": "ORMOC",
	"OMD": "ORANJEMUND",
	"OME": "NOME",
	"OMF": "MAFRAQ",
	"OMG": "OMEGA",
	"OMH": "ORUMIEH",
	"OMK": "OMAK",
	"OML": "OMKALAI",
	"OMM": "MARMUL",
	"OMN": "OSMANABAD",
	"OMO": "MOSTAR",
	"OMR": "ORADEA MARE",
	"OMS": "OMSK",
	"OMY": "PHNOM THBENG MEANCHEY",
	"ONA": "WINONA",
	"ONB": "ONONGE",
	"OND": "ONDANGWA",
	"ONE": "ONEPUSU",
	"ONG": "MORNINGTON IS.",
	"ONH": "ONEONTA",
	"ONI": "MOANAMANI",
	"ONL": "ONEILL",
	"ONM": "SOCORRO",
	"ONN": "ONION BAY",
	"ONO": "ONTARIO",
	"ONP": "NEWPORT",
	"ONR": "MONKIRA",
	"ONS": "ONSLOW",
	"ONT": "ONTARIO",
	"ONU": "ONO-I-LAU",
	"ONX": "COLON",
	"ONY": "OLNEY",
	"OOA": "OSKALOOSA",
	"OOK": "TOKSOOK",
	"OOL": "GOLD COAST",
	"OOM": "COOMA",
	"OOR": "MOORABERRIE",
	"OOT": "ONOTOA",
	"OPA": "KOPASKER",
	"OPB": "OPEN BAY",
	"OPF": "MIAMI",
	"OPI": "OENPELLI",
	"OPL": "OPELOUSAS",
	"OPO": "OPORTO",
	"OPS": "SINOP",
	"OPU": "BALIMO",
	"OPW": "OPUWA",
	"ORA": "ORAN",
	"ORB": "OREBRO",
	"ORC": "OROCUE",
	"ORD": "CHICAGO",
	"ORE": "ORLEANS",
	"ORF": "NORFOLK",
	"ORG": "PARAMARIBO",
	"ORH": "WORCESTER",
	"ORI": "PORT LIONS",
	"ORJ": "ORINDUIK",
	"ORK": "CORK",
	"ORL": "ORLANDO",
	"ORM": "NORTHAMPTON",
	"ORN": "ORAN",
	"ORO": "YORO",
	"ORP": "ORAPA",
	"ORQ": "NORWALK",
	"ORR": "YORKETOWN",
	"ORS": "ORPHEUS IS. RESORT",
	"ORT": "NORTHWAY",
	"ORU": "ORURO",
	"ORV": "NOORVIK",
	"ORW": "ORMARA",
	"ORX": "ORIXIMINA",
	"ORY": "PARIS",
	"ORZ": "ORANGE WALK",
	"OSA": "OSAKA",
	"OSB": "OSAGE BEACH",
	"OSC": "OSCODA",
	"OSD": "OSTERSUND",
	"OSE": "OMORA",
	"OSG": "OSSIMA",
	"OSH": "OSHKOSH",
	"OSI": "OSIJEK",
	"OSK": "OSKARSHAMN",
	"OSL": "OSLO",
	"OSM": "MOSUL",
	"OSN": "OSAN AB",
	"OSP": "SLUPSK",
	"OSR": "OSTRAVA",
	"OSS": "OSH",
	"OST": "OSTEND",
	"OSU": "COLUMBUS",
	"OSW": "ORSK",
	"OSX": "KOSCIUSKO",
	"OSY": "OSYSOS",
	"OSZ": "KOSZALIN",
	"OTA": "MOTA",
	"OTC": "BOL",
	"OTD": "CONTADORA",
	"OTG": "WORTHINGTON",
	"OTH": "NORTH BEND",
	"OTI": "MOROTAI ISLAND",
	"OTL": "BOUTILIMIT",
	"OTM": "OTTUMWA",
	"OTN": "OAKTOWN",
	"OTO": "OTTO",
	"OTP": "BUCHAREST-OTOPENI",
	"OTR": "COTO 47",
	"OTS": "ANACORTES",
	"OTU": "OTU",
	"OTY": "ORIA",
	"OTZ": "KOTZEBUE",
	"OUA": "OUAGADOUGOU",
	"OUD": "OUJDA",
	"OUE": "OUESSO",
	"OUG": "OUAHIGOUYA",
	"OUH": "OUDTSHOORN",
	"OUI": "BAN HOUEI SAI",
	"OUK": "OUTER SKERRIES",
	"OUL": "OULU",
	"OUM": "OUM-HADJER",
	"OUN": "NORMAN",
	"OUR": "BATOURI",
	"OUS": "OURINHOS",
	"OUT": "BOUSSO",
	"OUU": "OUANGA",
	"OUZ": "ZOUERATE",
	"OVA": "BEKILY",
	"OVB": "NOVOSIBIRSK",
	"OVD": "OVIEDO",
	"OVE": "OROVILLE",
	"OVL": "OVALLE",
	"OWA": "OWATONNA",
	"OWB": "OWENSBORO",
	"OWD": "NORWOOD",
	"OWE": "OWENDO",
	"OWK": "NORRIDGEWOCK",
	"OXB": "BISSAU",
	"OXC": "WATERBURY",
	"OXF": "OXFORD",
	"OXO": "ORIENTOS",
	"OXR": "OXNARD",
	"OXY": "MORNEY",
	"OYA": "GOYA",
	"OYE": "OYEM",
	"OYG": "MOYO",
	"OYK": "OIAPOQUE",
	"OYL": "MOYALE",
	"OYN": "OUYEN",
	"OYO": "TRES ARROYOS",
	"OYP": "ST-GEORGES DE L/OYAPOCK",
	"OYS": "YOSEMITE NATL PARK",
	"OZA": "OZONA",
	"OZC": "OZAMIS CITY",
	"OZH": "ZAPOROZH'YE",
	"OZP": "MORON",
	"OZR": "OZARK",
	"OZZ": "OUARZAZATE",
	"PAA": "PA-AN",
	"PAB": "PEDRO AFONSO",
	"PAC": "PANAMA CITY",
	"PAD": "PADERBORN",
	"PAE": "EVERETT",
	"PAF": "PAKUBA",
	"PAG": "PAGADIAN",
	"PAH": "PADUCAH",
	"PAI": "PAILIN",
	"PAJ": "PARA CHINAR",
	"PAK": "HANAPEPE",
	"PAL": "PALANQUERO",
	"PAM": "PANAMA CITY",
	"PAN": "PATTANI",
	"PAO": "PALO ALTO",
	"PAP": "PORT AU PRINCE",
	"PAQ": "PALMER",
	"PAR": "PARIS",
	"PAS": "PAROS",
	"PAT": "PATNA",
	"PAU": "PAUK",
	"PAV": "PAULO AFONSO",
	"PAW": "PAMBWA",
	"PAX": "PORT-DE-PAIX",
	"PAY": "PAMOL",
	"PAZ": "POZA RICA",
	"PBA": "POINT BARROW",
	"PBB": "PARANAIBA",
	"PBC": "PUEBLA",
	"PBD": "PORBANDAR",
	"PBE": "PUERTO BERRIO",
	"PBF": "PINE BLUFF",
	"PBG": "PLATTSBURGH",
	"PBH": "PARO",
	"PBI": "WEST PALM BEACH",
	"PBJ": "PAAMA",
	"PBK": "PACK CREEK",
	"PBL": "PUERTO CABELLO",
	"PBM": "PARAMARIBO",
	"PBN": "PORTO AMBOIN",
	"PBO": "PARABURDOO",
	"PBP": "PUNTA ISLITA",
	"PBQ": "PIMENTA BUENO",
	"PBR": "PUERTO BARRIOS",
	"PBS": "PATONG BEACH",
	"PBU": "PUTAO",
	"PBZ": "PLETTENBERG",
	"PCA": "PORTAGE CREEK",
	"PCB": "PONDOK CABE",
	"PCC": "PUERTO RICO",
	"PCD": "PRAIRIE DU CHIEN",
	"PCE": "PAINTER CREEK",
	"PCG": "PASO CABALLOS",
	"PCH": "PALACIOS",
	"PCK": "PORCUPINE CREEK",
	"PCL": "PUCALLPA",
	"PCM": "PLAYA DEL CARMEN",
	"PCN": "PICTON",
	"PCO": "PUNTA COLORADO",
	"PCP": "PRINCIPE IS.",
	"PCR": "PUERTO CARRENO",
	"PCS": "PICOS",
	"PCT": "PRINCETON",
	"PCU": "PICAYUNE",
	"PCV": "PUNTA CHIVATO",
	"PDA": "PUERTO INIRIDA",
	"PDB": "PEDRO BAY",
	"PDC": "MUEO",
	"PDE": "PANDIE PANDIE",
	"PDF": "PRADO",
	"PDG": "PADANG",
	"PDI": "PINDIU",
	"PDK": "ATLANTA",
	"PDL": "PONTA DELGADA",
	"PDM": "PATOS DE MINAS",
	"PDN": "PARNDANA",
	"PDO": "PENDOPO",
	"PDP": "PUNTA DEL ESTE",
	"PDR": "POFADDER",
	"PDS": "PIEDRAS NEGRAS",
	"PDT": "PENDLETON",
	"PDU": "PAYSANDU",
	"PDV": "PLOVDIV",
	"PDX": "PORTLAND",
	"PDZ": "PEDERNALES",
	"PEA": "PENNESHAW",
	"PEB": "PEBANE",
	"PEC": "PELICAN",
	"PEE": "PERM",
	"PEG": "PERUGIA",
	"PEH": "PEHUAJO",
	"PEI": "PEREIRA",
	"PEK": "BEIJING",
	"PEL": "PELANENG",
	"PEM": "PUERTO MALDONADO",
	"PEN": "PENANG",
	"PEP": "PEPPIMENARTI",
	"PEQ": "PECOS",
	"PER": "PERTH",
	"PES": "PETROZAVODSK",
	"PET": "PELOTAS",
	"PEU": "PUERTO LEMPIRA",
	"PEW": "PESHAWAR",
	"PEX": "PECHORA",
	"PEY": "PENONG",
	"PEZ": "PENZA",
	"PFA": "PAF WARREN",
	"PFB": "PASSO FUNDO",
	"PFC": "PACIFIC CITY",
	"PFD": "PORT FREDERICK",
	"PFJ": "PATREKSFJORDUR",
	"PFN": "PANAMA CITY",
	"PFO": "PAPHOS",
	"PFR": "ILEBO",
	"PGA": "PAGE",
	"PGB": "PANGOA",
	"PGC": "PETERSBURG",
	"PGD": "PUNTA GORDA",
	"PGE": "YEGEPA",
	"PGF": "PERPIGNAN",
	"PGG": "PROGRESSO",
	"PGH": "PANTNAGAR",
	"PGI": "CHITATO",
	"PGK": "PANGKALPINANG",
	"PGL": "PASCAGOULA",
	"PGM": "PORT GRAHAM",
	"PGN": "PANGIA",
	"PGO": "PAGOSA SPRINGS",
	"PGP": "PORTO ALEGRE",
	"PGR": "PARAGOULD",
	"PGS": "PEACH SPRINGS",
	"PGT": "PORANGATU",
	"PGV": "GREENVILLE",
	"PGX": "PERIGUEUX",
	"PGZ": "PONTA GROSSA",
	"PHA": "PHAN RANG",
	"PHB": "PARNAIBA",
	"PHC": "PORT HARCOURT",
	"PHD": "NEW PHILADELPHIA",
	"PHE": "PORT HEDLAND",
	"PHF": "NEWPORT NEWS/WMSBURG",
	"PHH": "PHAN THIET",
	"PHI": "PINHEIRO",
	"PHJ": "PORT HUNTER",
	"PHK": "PAHOKEE",
	"PHL": "PHILADELPHIA",
	"PHM": "BOEBLINGEN",
	"PHN": "PORT HURON",
	"PHO": "POINT HOPE",
	"PHP": "PHILIP",
	"PHR": "PACIFIC HARBOUR",
	"PHS": "PHITSANULOKE",
	"PHT": "PARIS",
	"PHU": "PHU-VINH",
	"PHW": "PHALABORWA",
	"PHX": "PHOENIX",
	"PHY": "PINHEIRO",
	"PHZ": "PHI PHI ISLAND",
	"PIA": "PEORIA",
	"PIB": "LAUREL/HATTIESBURG",
	"PIC": "PINE CAY",
	"PID": "NASSAU-PARADISE I",
	"PIE": "ST. PETERSBURG",
	"PIF": "PINGTUNG",
	"PIG": "PITINGA",
	"PIH": "POCATELLO",
	"PII": "FAIRBANKS",
	"PIK": "GLASGOW",
	"PIL": "PILAR",
	"PIM": "PINE MOUNTAIN",
	"PIN": "PARINTINS",
	"PIO": "PISCO",
	"PIP": "PILOT POINT",
	"PIQ": "PIPILLIPAI",
	"PIR": "PIERRE",
	"PIS": "POITIERS",
	"PIT": "PITTSBURGH",
	"PIU": "PIURA",
	"PIV": "PIRAPORA",
	"PIW": "PIKWITONEI",
	"PIX": "PICO ISLAND",
	"PIZ": "POINT LAY",
	"PJB": "PAYSON",
	"PJC": "PEDRO JUAN CABALLERO",
	"PJG": "PANJGUR",
	"PJM": "PUERTO JIMENEZ",
	"PJS": "PORT SAN JUAN",
	"PJZ": "PUERTO JUAREZ",
	"PKA": "NAPASKIAK",
	"PKB": "PARKERSBURG",
	"PKC": "PETROPAVLOVSK-KAMCHATSKY",
	"PKD": "PARK RAPIDS",
	"PKE": "PARKES",
	"PKF": "PARK FALLS",
	"PKG": "PANGKOR",
	"PKH": "PORTO-KHELI",
	"PKK": "PAKOKKU",
	"PKL": "PAKATOA",
	"PKM": "PORT KAITUMA",
	"PKN": "PANGKALANBUUN",
	"PKO": "PARAKOU",
	"PKP": "PUKAPUKA",
	"PKR": "POKHARA",
	"PKS": "PAKSANE",
	"PKT": "PORT KEATS",
	"PKU": "PAKANBARU",
	"PKV": "PSKOV",
	"PKW": "SELEBI-PHIKWE",
	"PKY": "PALANGKARAYA",
	"PKZ": "PAKSE",
	"PLA": "PLANADAS",
	"PLB": "PLATTSBURGH",
	"PLC": "PLANETA RICA",
	"PLD": "PLAYA SAMARA",
	"PLE": "PAIELA",
	"PLF": "PALA",
	"PLH": "PLYMOUTH",
	"PLI": "PALM ISLAND",
	"PLJ": "PLACENCIA",
	"PLK": "POINT LOOKOUT",
	"PLL": "PONTA PELADA",
	"PLM": "PALEMBANG",
	"PLN": "PELLSTON",
	"PLO": "PORT LINCOLN",
	"PLP": "LA PALMA",
	"PLQ": "PALANGA",
	"PLR": "PELL CITY",
	"PLS": "PROVIDENCIALES",
	"PLT": "PLATO",
	"PLU": "BELO HORIZONTE",
	"PLV": "POLTAVA",
	"PLW": "PALU",
	"PLX": "SEMIPALATINSK",
	"PLY": "PLYMOUTH",
	"PLZ": "PORT ELIZABETH",
	"PMA": "PEMBA IS.",
	"PMB": "PEMBINA",
	"PMC": "PUERTO MONTT",
	"PMD": "PALMDALE/LANCASTER",
	"PME": "PORTSMOUTH",
	"PMF": "PARMA",
	"PMG": "PONTA PORA",
	"PMH": "PORTSMOUTH",
	"PMI": "PALMA",
	"PMK": "PALM ISLAND",
	"PML": "PORT MOLLER",
	"PMM": "PHANOM SARAKHAM",
	"PMN": "PUMANI",
	"PMO": "PALERMO",
	"PMP": "PIMAGA",
	"PMQ": "PERITO MORENO",
	"PMR": "PALMERSTON N.",
	"PMS": "PALMYRA",
	"PMT": "PARAMAKATOI",
	"PMU": "PAIMIUT",
	"PMV": "PORLAMAR",
	"PMW": "PALMAS",
	"PMX": "PALMER",
	"PMY": "PUERTO MADRYN",
	"PMZ": "PALMAR",
	"PNA": "PAMPLONA",
	"PNB": "PORTO NACIONAL",
	"PNC": "PONCA CITY",
	"PND": "PUNTA GORDA",
	"PNE": "PHILADELPHIA",
	"PNF": "PETERSON'S POINT",
	"PNG": "PARANAGUA",
	"PNH": "PHNOM PENH",
	"PNI": "PONAPE",
	"PNK": "PONTIANAK",
	"PNL": "PANTELLERIA",
	"PNN": "PRINCETON",
	"PNO": "PINOTEPA",
	"PNP": "POPONDETTA",
	"PNQ": "POONA",
	"PNR": "POINTE NOIRE",
	"PNS": "PENSACOLA",
	"PNT": "PUERTO NATALES",
	"PNU": "PANGUITCH",
	"PNV": "PANEVEZYS",
	"PNW": "PENEDO",
	"PNX": "SHERMAN",
	"PNY": "PONDICHERRY",
	"PNZ": "PETROLINA",
	"POA": "PORTO ALEGRE",
	"POB": "FAYETTEVILLE",
	"POC": "LA VERNE",
	"POD": "PODOR",
	"POE": "FT. POLK",
	"POF": "POPLAR BLUFF",
	"POG": "PORT GENTIL",
	"POH": "POCAHONTAS",
	"POI": "POTOSI",
	"POL": "PEMBA",
	"POM": "PORT MORESBY",
	"PON": "POPTUN",
	"POO": "POCOS DE CALDAS",
	"POP": "PUERTO PLATA",
	"POQ": "POLK INLET",
	"POR": "PORI",
	"POS": "PORT OF SPAIN",
	"POT": "PORT ANTONIO",
	"POU": "POUGHKEEPSIE",
	"POV": "PRESOV",
	"POW": "PORTOROZ",
	"POX": "PONTOISE",
	"POY": "POWELL",
	"POZ": "POZNAN",
	"PPA": "PAMPA",
	"PPB": "PRES PRUDENTE",
	"PPC": "PROSPECT CREEK",
	"PPD": "HUMACAO",
	"PPE": "PUERTO PENASCO",
	"PPF": "PARSONS",
	"PPG": "PAGO PAGO",
	"PPH": "PERAITEPUY",
	"PPI": "PORT PIRIE",
	"PPJ": "PULAU PANJANG",
	"PPK": "PETROPAVLOVSK",
	"PPL": "PHAPLU",
	"PPM": "POMPANO BEACH",
	"PPN": "POPAYAN",
	"PPO": "POWELL POINT",
	"PPP": "PROSERPINE",
	"PPQ": "PARAPARAUMU",
	"PPR": "PASIR PANGARAYAN",
	"PPS": "PUERTO PRINCESA",
	"PPT": "PAPEETE",
	"PPU": "PAPUN",
	"PPV": "PORT PROTECTION",
	"PPW": "PAPA WESTRAY",
	"PPX": "PARAM",
	"PPY": "POUSO ALEGRE",
	"PPZ": "PUERTO PAEZ",
	"PQC": "PHUQUOC",
	"PQI": "PRESQUE ISLE",
	"PQM": "PALENQUE",
	"PQQ": "PORT MACQUARIE",
	"PQS": "PILOT STATION",
	"PRA": "PARANA",
	"PRB": "PASO ROBLES",
	"PRC": "PRESCOTT",
	"PRD": "PARDOO",
	"PRE": "PORE",
	"PRF": "PORT JOHNSON",
	"PRG": "PRAGUE",
	"PRH": "PHRAE",
	"PRI": "PRASLIN IS.",
	"PRJ": "CAPRI",
	"PRK": "PRIESKA",
	"PRL": "PORT OCEANIC",
	"PRM": "PORTIMAO",
	"PRN": "PRISTINA",
	"PRP": "PROPRIANO",
	"PRQ": "PRES. ROQUE SAENZ PENA",
	"PRR": "POTGIETERSRUS",
	"PRS": "PARASI",
	"PRU": "PROME",
	"PRV": "PREROV",
	"PRW": "PRENTICE",
	"PRX": "PARIS",
	"PRY": "PRETORIA",
	"PRZ": "PRINEVILLE",
	"PSA": "PISA",
	"PSB": "PHILIPSBURG",
	"PSC": "PASCO",
	"PSD": "PORT SAID",
	"PSE": "PONCE",
	"PSF": "PITTSFIELD",
	"PSG": "PETERSBURG",
	"PSH": "ST. PETER-ORDING",
	"PSI": "PASNI",
	"PSJ": "POSO",
	"PSK": "PULASKI",
	"PSL": "PERTH",
	"PSM": "PORTSMOUTH",
	"PSN": "PALESTINE",
	"PSO": "PASTO",
	"PSP": "INDIO/PALM SPRINGS",
	"PSR": "PESCARA",
	"PSS": "POSADAS",
	"PST": "PRESTON",
	"PSU": "PUTUSSIBAU",
	"PSV": "PAPA STOUR",
	"PSW": "PASSOS",
	"PSX": "PALACIOS",
	"PSY": "PORT STANLEY",
	"PSZ": "PUERTO SUAREZ",
	"PTA": "PORT ALSWORTH",
	"PTB": "PETERSBURG",
	"PTC": "PORT ALICE",
	"PTD": "PORT ALEXANDER",
	"PTE": "PORT STEPHENS",
	"PTF": "MALOLOLAILAI",
	"PTG": "PIETERSBURG",
	"PTH": "PORT HEIDEN",
	"PTI": "PORT DOUGLAS",
	"PTJ": "PORTLAND",
	"PTK": "PONTIAC",
	"PTL": "PORT ARMSTRONG",
	"PTM": "PALMARITO",
	"PTN": "PATTERSON/MORGAN CITY",
	"PTO": "PATO BRANCO",
	"PTP": "POINTE A PITRE",
	"PTQ": "PORTO DE MOZ",
	"PTR": "PLEASANT HARBOR",
	"PTS": "PITTSBURG",
	"PTT": "PRATT",
	"PTU": "PLATINUM",
	"PTV": "PORTERVILLE",
	"PTW": "POTTSTOWN",
	"PTX": "PITALITO",
	"PTY": "PANAMA CITY",
	"PTZ": "PASTAZA",
	"PUA": "PUAS",
	"PUB": "PUEBLO",
	"PUC": "PRICE",
	"PUD": "PUERTO DESEADO",
	"PUE": "PUERTO OBALDIA",
	"PUF": "PAU",
	"PUG": "PORT AUGUSTA",
	"PUH": "POCHUTLA",
	"PUI": "PURENI",
	"PUJ": "PUNTA CANA",
	"PUK": "PUKARUA",
	"PUL": "POULSBO",
	"PUM": "POMALAA",
	"PUN": "PUNIA",
	"PUO": "PRUDHOE BAY",
	"PUP": "PO",
	"PUQ": "PUNTA ARENAS",
	"PUR": "PUERTO RICO",
	"PUS": "PUSAN",
	"PUT": "PUTTAPARTHI",
	"PUU": "PUERTO ASIS",
	"PUV": "POUM",
	"PUW": "PULLMAN",
	"PUX": "PUERTO VARAS",
	"PUY": "PULA",
	"PUZ": "PUERTO CABEZAS",
	"PVA": "PROVIDENCIA",
	"PVC": "PROVINCETOWN",
	"PVD": "PROVIDENCE",
	"PVE": "PORVENIR",
	"PVF": "PLACERVILLE",
	"PVH": "PORTO VELHO",
	"PVI": "PARANAVAI",
	"PVK": "PREVEZA",
	"PVN": "PLEVEN",
	"PVO": "PORTOVIEJO",
	"PVR": "PUERTO VALLARTA",
	"PVS": "PROVIDENIYA",
	"PVU": "PROVO",
	"PVW": "PLAINVIEW",
	"PVX": "PROVEDENIA",
	"PVY": "POPE VANOY",
	"PVZ": "PAINESVILLE",
	"PWA": "OKLAHOMA CITY",
	"PWD": "PLENTYWOOD",
	"PWE": "PEWEK",
	"PWI": "PAWI",
	"PWK": "CHICAGO",
	"PWL": "PURWOKERTO",
	"PWM": "PORTLAND",
	"PWN": "PITTS TOWN",
	"PWO": "PWETO",
	"PWQ": "PAVLODAR",
	"PWR": "PORT WALTER",
	"PWT": "BREMERTON",
	"PXA": "PARANA",
	"PXL": "POLACCA",
	"PXM": "PUERTO ESCONDIDO",
	"PXO": "PORTO SANTO",
	"PXU": "PLEIKU",
	"PXX": "PORTO AFONSO",
	"PYA": "PUERTO BOYACA",
	"PYB": "JEYPORE",
	"PYC": "PLAYON CHICO",
	"PYE": "PENRHYN ISLAND",
	"PYH": "PUERTO AYACUCHO",
	"PYJ": "POLYARNYJ",
	"PYL": "PERRY ISLAND",
	"PYM": "PLYMOUTH",
	"PYN": "PAYAN",
	"PYO": "PUTUMAYO",
	"PYR": "PYRGOS",
	"PYV": "YAVIZA",
	"PYX": "PATTAYA",
	"PZA": "PAZ DE ARIPORO",
	"PZB": "PIETERMARITZBURG",
	"PZE": "PENZANCE",
	"PZH": "ZHOB",
	"PZK": "PUKA PUKA ISLAND",
	"PZL": "PHINDA",
	"PZO": "PUERTO ORDAZ",
	"PZU": "PORT SUDAN",
	"PZY": "PIESTANY",
	"QAH": "ALCANTARA",
	"QAR": "ARNHEM",
	"QBC": "BELLA COOLA",
	"QBE": "BEGA",
	"QBF": "BRACK",
	"QBW": "BATEMANS BAY",
	"QCE": "COPPER MOUNTAIN",
	"QCU": "AKUNNAAQ",
	"QDM": "DAMMAM",
	"QDN": "EDEN",
	"QDU": "DUSSELDORF",
	"QEY": "QEQERTARSUATSIAAT",
	"QFE": "COLUMBUS",
	"QFG": "EQALUGAARSUIT",
	"QFI": "IGINNIARFIK",
	"QFJ": "ALLUITSUP PAA",
	"QFK": "SELJE",
	"QFN": "NARSAQ KUJALLEQ",
	"QFP": "KJELLERUP",
	"QFQ": "MALOY",
	"QFT": "QASSIARSUK",
	"QFV": "BERGEN OLP HARBOUR",
	"QFX": "IGALIKU",
	"QGQ": "ATTU",
	"QGR": "KANGERLUK",
	"QGV": "FRANKFURT-ISENBURG",
	"QJB": "JUBAIL",
	"QJD": "JINDABYNE",
	"QJE": "KITSISSUARSUIT",
	"QJF": "ATAMMIK",
	"QJG": "ITILLEQ",
	"QJH": "QASSIMIUT",
	"QJI": "IKAMIUT",
	"QJT": "NAPASOQ",
	"QJV": "SKAGEN",
	"QKB": "BRECKENRIDGE",
	"QKL": "COLOGNE/BONN-MAIN",
	"QKS": "KEYSTONE",
	"QKT": "KANGAAMIUT",
	"QKU": "COLOGNE-DEUTZ",
	"QLE": "LEETON",
	"QLX": "LAUTERACH",
	"QMK": "NIAQORNAARSUK",
	"QMM": "MARINA MASSA",
	"QMV": "MONTVALE",
	"QMX": "FICTITIOUS CITY ONE",
	"QMY": "FICTITIOUS CITY TWO",
	"QNA": "BALLINA",
	"QND": "NOVI SAD",
	"QOF": "KIEV-DARNTS BUS",
	"QOH": "KIEV-RUS BUS",
	"QOQ": "SAARLOQ",
	"QPG": "SINGAPORE",
	"QPV": "PERISHER VALLEY",
	"QPW": "KANGAATSIAQ",
	"QPX": "FICTITIOUS CITY THREE",
	"QPY": "FICTITIOUS CITY FOUR",
	"QQA": "CENTER ISLAND",
	"QQB": "KEYES POINT",
	"QQD": "BOSTON",
	"QQI": "SUNNYSIDE",
	"QQJ": "STEVENSVILLE",
	"QQK": "GARBERVILLE",
	"QQL": "BONNERS FERRY",
	"QQM": "SHINGLETOWN",
	"QQN": "WOODLAKE",
	"QQO": "BREWSTER",
	"QQP": "SANGER",
	"QQQ": "GRASS VALLEY",
	"QQR": "HORSHAM",
	"QQS": "WALDRONAIRE",
	"QQU": "WINDOW ROCK",
	"QQV": "LUPIN",
	"QQW": "SANTA ROSA",
	"QQX": "HEALDSBURG",
	"QQY": "SAN MARCOS",
	"QQZ": "PARAGUANA",
	"QRA": "JOHANNESBURG",
	"QRM": "NARROMINE",
	"QRO": "QUERETARO",
	"QRR": "WARREN",
	"QRX": "NAROOMA",
	"QRY": "IKERASAARSUK",
	"QTH": "THREDBO",
	"QTO": "SKITUBE",
	"QTZ": "COATZACOALCOS",
	"QUB": "UBARI",
	"QUE": "QULLISSAT",
	"QUF": "TALLINN-PIRITA HRB",
	"QUL": "ULM",
	"QUP": "SAQQAQ",
	"QUV": "AAPPILATTOQ",
	"QUW": "AMMASSIVIK",
	"QVU": "VIRU",
	"QWC": "BERLIN ZOO",
	"QWF": "FT. COLLINS/LOVELAND",
	"QWH": "FT. COLLINS/LOVELAND",
	"QWL": "CRACKENBACK VILLAGE",
	"QWM": "LONGMONT",
	"QWP": "WINTER PARK",
	"QWV": "VALJEVO",
	"QXD": "CACHOEIRO ITAPEMIRIM",
	"QXE": "SORA",
	"QYE": "ENSCHEDE-RR STN",
	"QYH": "HENGELO",
	"QYI": "HILVERSUM",
	"QYK": "KOYANG",
	"QYL": "ALMELO",
	"QYM": "AMERSFOORT",
	"QYN": "BYRON BAY",
	"QYO": "OLSZTYN",
	"QYP": "APELDOORN",
	"QYV": "DEVENTER",
	"QYZ": "HEERENVEEN",
	"QZC": "SMIGGIN HOLES",
	"QZM": "BULLOCKS FLAT",
	"QZU": "RAUMA",
	"QZX": "FICTITIOUS CITY FIVE",
	"QZY": "FICTITIOUS POINT/AF",
	"RAA": "RAKANDA",
	"RAB": "RABAUL",
	"RAC": "RACINE",
	"RAD": "TORTOLA",
	"RAE": "ARAR",
	"RAF": "RAS AN NAQB",
	"RAG": "RAGLAN",
	"RAH": "RAFHA",
	"RAI": "PRAIA",
	"RAJ": "RAJKOT",
	"RAK": "MARRAKECH",
	"RAL": "RIVERSIDE",
	"RAM": "RAMINGINING",
	"RAN": "RAVENNA",
	"RAO": "RIBEIRAO PRETO",
	"RAP": "RAPID CITY",
	"RAQ": "RAHA",
	"RAR": "RAROTONGA",
	"RAS": "RASHT",
	"RAT": "RADUZHNYI",
	"RAU": "RANGPUR",
	"RAV": "CRAVO NORTE",
	"RAW": "ARAWA",
	"RAX": "ORAM",
	"RAY": "ROTHESAY",
	"RAZ": "RAWALA KOT",
	"RBA": "RABAT",
	"RBB": "BORBA",
	"RBC": "ROBINVALE",
	"RBE": "RATANANKIRI",
	"RBF": "BIG BEAR",
	"RBG": "ROSEBURG",
	"RBH": "BROOKS LODGE",
	"RBI": "RABI",
	"RBJ": "REBUN",
	"RBK": "RANCHO CALIFORNIA",
	"RBL": "RED BLUFF",
	"RBM": "STRAUBING",
	"RBN": "FT. JEFFERSON NATL PARK",
	"RBO": "ROBORE",
	"RBP": "RABA RABA",
	"RBQ": "RURRENABAQUE",
	"RBR": "RIO BRANCO",
	"RBS": "ORBOST",
	"RBT": "MARSABIT",
	"RBU": "ROEBOURNE",
	"RBV": "RANCHO BUENA VISTA",
	"RBW": "WALTERBORO",
	"RBY": "RUBY",
	"RCA": "RAPID CITY",
	"RCB": "RICHARDS BAY",
	"RCE": "ROCHE HARBOR",
	"RCH": "RIOHACHA",
	"RCK": "ROCKDALE",
	"RCL": "REDCLIFFE",
	"RCM": "RICHMOND",
	"RCN": "AMERICAN RIVER",
	"RCO": "ROCHEFORT",
	"RCQ": "RECONQUISTA",
	"RCS": "ROCHESTER",
	"RCT": "REED CITY",
	"RCU": "RIO CUARTO",
	"RCY": "RUM CAY",
	"RDA": "ROCKHAMPTON DOWNS",
	"RDB": "RED DOG",
	"RDC": "REDENCAO",
	"RDD": "REDDING",
	"RDE": "MERDEY",
	"RDG": "READING",
	"RDM": "REDMOND",
	"RDN": "RONDONIA",
	"RDR": "RED RIVER",
	"RDS": "RINCON DE LOS SAUCES",
	"RDT": "RICHARD-TOLL",
	"RDU": "RALEIGH/DURHAM",
	"RDV": "RED DEVIL",
	"RDZ": "RODEZ",
	"REA": "REAO",
	"REB": "RECHLIN",
	"REC": "RECIFE",
	"RED": "REEDSVILLE",
	"REE": "LUBBOCK",
	"REG": "REGGIO CALABRIA",
	"REH": "REHOBOTH BEACH",
	"REI": "REGINA",
	"REK": "REYKJAVIK",
	"REL": "TRELEW",
	"REN": "ORENBURG",
	"REO": "ROME",
	"REP": "SIEM REAP",
	"RES": "RESISTENCIA",
	"RET": "ROST",
	"REU": "REUS",
	"REW": "REWA",
	"REX": "REYNOSA",
	"REY": "REYES",
	"RFA": "RAFAI",
	"RFD": "ROCKFORD",
	"RFK": "ANGUILLA",
	"RFN": "RAUFARHOFN",
	"RFP": "RAIATEA",
	"RFR": "RIO FRIO",
	"RFS": "ROSITA",
	"RGA": "RIO GRANDE",
	"RGE": "PORGERA",
	"RGH": "BALURGHAT",
	"RGI": "RANGIROA",
	"RGL": "RIO GALLEGOS",
	"RGN": "YANGON",
	"RGR": "RANGER",
	"RGT": "RENGAT",
	"RHA": "REYKHOLAR",
	"RHD": "RIO HONDO",
	"RHE": "REIMS",
	"RHG": "RUHENGERI",
	"RHI": "RHINELANDER",
	"RHL": "ROY HILL",
	"RHO": "RHODES ISLAND",
	"RHP": "RAMECHAP",
	"RHV": "SAN JOSE",
	"RIA": "SANTA MARIA",
	"RIB": "RIBERALTA",
	"RIC": "RICHMOND",
	"RID": "RICHMOND",
	"RIE": "RICE LAKE",
	"RIF": "RICHFIELD",
	"RIG": "RIO GRANDE",
	"RIJ": "RIOJA",
	"RIK": "CARRILLO",
	"RIL": "RIFLE",
	"RIM": "RODRIGUEZ DE MENDOZA",
	"RIN": "RINGI COVE",
	"RIO": "RIO DE JANEIRO",
	"RIS": "RISHIRI",
	"RIT": "RIO TIGRE",
	"RIV": "RIVERSIDE",
	"RIW": "RIVERTON",
	"RIX": "RIGA",
	"RIY": "RIYAN",
	"RIZ": "RIO ALZUCAR",
	"RJA": "RAJAHMUNDRY",
	"RJB": "RAJBIRAJ",
	"RJH": "RAJSHAHI",
	"RJI": "RAJOURI",
	"RJK": "RIJEKA",
	"RKC": "YREKA",
	"RKD": "ROCKLAND",
	"RKE": "ROSKILDE",
	"RKH": "ROCK HILL",
	"RKI": "ROKOT",
	"RKO": "SIPORA",
	"RKP": "ROCKPORT",
	"RKR": "POTEAU",
	"RKS": "ROCK SPRINGS",
	"RKT": "RAS AL KHAIMAH",
	"RKU": "KAIRUKU",
	"RKV": "REYKJAVIK-DOMESTIC",
	"RKW": "ROCKWOOD",
	"RKY": "ROKEBY",
	"RLA": "ROLLA",
	"RLD": "RICHLAND",
	"RLG": "ROSTOCK-LAAGE",
	"RLP": "ROSELLA PLAINS",
	"RLT": "ARLIT",
	"RLU": "BORNITE",
	"RMA": "ROMA",
	"RMB": "BURAIMI",
	"RMD": "RAMAGUNDAM",
	"RME": "ROME",
	"RMG": "ROME",
	"RMI": "RIMINI",
	"RMK": "RENMARK",
	"RML": "COLOMBO",
	"RMN": "RUMGINAE",
	"RMP": "RAMPART",
	"RMS": "RAMSTEIN",
	"RMT": "ROCKY MOUNT-WILSON",
	"RNB": "RONNEBY",
	"RNC": "MCMINNVILLE",
	"RND": "SAN ANTONIO",
	"RNE": "ROANNE",
	"RNG": "RANGELY",
	"RNH": "NEW RICHMOND",
	"RNI": "CORN ISLAND",
	"RNJ": "YORON-JIMA",
	"RNL": "RENNELL IS.",
	"RNN": "RONNE",
	"RNO": "RENO",
	"RNP": "RONGELAP",
	"RNR": "ROBINSON RIVER",
	"RNS": "RENNES",
	"RNT": "RENTON",
	"RNU": "RANAU",
	"RNZ": "RENSSELAER",
	"ROA": "ROANOKE",
	"ROB": "MONROVIA",
	"ROC": "ROCHESTER",
	"ROD": "ROBERTSON",
	"ROG": "ROGERS",
	"ROH": "ROBINHOOD",
	"ROK": "ROCKHAMPTON",
	"ROL": "ROOSEVELT",
	"ROM": "ROME",
	"RON": "RONDON",
	"ROO": "RONDONOPOLIS",
	"ROP": "ROTA",
	"ROR": "KOROR",
	"ROS": "ROSARIO",
	"ROT": "ROTORUA",
	"ROU": "RUSSE",
	"ROV": "ROSTOV",
	"ROW": "ROSWELL",
	"ROX": "ROSEAU",
	"ROY": "RIO MAYO",
	"RPA": "ROLPA",
	"RPB": "ROPER BAR",
	"RPM": "ROPER R. MISSION",
	"RPN": "ROSH-PINA",
	"RPR": "RAIPUR",
	"RPV": "ROPER VALLEY",
	"RPX": "ROUNDUP",
	"RQA": "LONE PINE",
	"RQB": "ATOMIC CITY",
	"RQC": "HARLOWTON",
	"RQD": "MENDOTA",
	"RQE": "PROSSER",
	"RQF": "LINCOLNTON",
	"RQG": "COVINGTON",
	"RQH": "PERKASIE",
	"RQI": "NIXON FORK MINE",
	"RQJ": "AMMAN",
	"RQK": "CHINA LAKE",
	"RQL": "LOWELL",
	"RQM": "STERLING",
	"RQN": "CAVE JUNCTION",
	"RQO": "SENECA",
	"RQP": "LOVINGTON",
	"RQQ": "WOODLAND",
	"RQR": "HARBOR SPRINGS",
	"RQS": "BARRIE",
	"RQT": "LUBBOCK",
	"RQU": "WASCO",
	"RQV": "SALT LAKE CITY",
	"RQW": "PAROWAN",
	"RQX": "BURLINGTON",
	"RQY": "COMANCHE",
	"RQZ": "MARY ESTHER",
	"RRE": "MARREE",
	"RRG": "RODRIGUES IS.",
	"RRI": "BARORA",
	"RRK": "ROURKELA",
	"RRL": "MERRILL",
	"RRM": "MARROMEU",
	"RRN": "SERRA NORTE",
	"RRO": "SORRENTO",
	"RRS": "ROROS",
	"RRT": "WARROAD",
	"RRV": "ROBINSON RIVER",
	"RSA": "SANTA ROSA",
	"RSB": "ROSEBERTH",
	"RSD": "ROCK SOUND",
	"RSE": "SYDNEY",
	"RSG": "SERRA PELADA",
	"RSH": "RUSSIAN MISSION",
	"RSI": "RIO SIDRA",
	"RSJ": "ROSARIO",
	"RSK": "RANSIKI",
	"RSL": "RUSSELL",
	"RSN": "RUSTON",
	"RSO": "REMANSO",
	"RSP": "RASPBERRY STRAIT",
	"RSS": "ROSEIRES",
	"RST": "ROCHESTER",
	"RSU": "YOSU",
	"RSW": "FT. MYERS",
	"RSX": "ROUSES POINT",
	"RTA": "ROTUMA",
	"RTB": "ROATAN",
	"RTC": "RATNAGIRI",
	"RTD": "ROTUNDA",
	"RTE": "MARGUERITE BAY",
	"RTG": "RUTENG",
	"RTI": "ROTI",
	"RTK": "TOKUA",
	"RTL": "SPIRIT LAKE",
	"RTM": "ROTTERDAM",
	"RTN": "RATON",
	"RTO": "COLTON",
	"RTP": "RUTLAND PLAINS",
	"RTS": "ROTTNEST IS.",
	"RTW": "SARATOV",
	"RTY": "MERTY",
	"RUA": "ARUA",
	"RUF": "YURUF",
	"RUG": "RUGAO",
	"RUH": "RIYADH",
	"RUI": "RUIDOSO",
	"RUK": "RUKUMKOT",
	"RUM": "RUMJARTAR",
	"RUN": "REUNION IS.",
	"RUP": "RUPSI",
	"RUR": "RURUTU IS.",
	"RUS": "MARAU",
	"RUT": "RUTLAND",
	"RUU": "RUTI",
	"RUV": "RUBELSANTO",
	"RUY": "RUINAS DE COPAN",
	"RVA": "FARAFANGANA",
	"RVC": "RIVERCESS",
	"RVD": "RIO VERDE",
	"RVE": "SARAVENA",
	"RVH": "ST. PETERSBURG-RZHEVKA",
	"RVK": "RORVIK",
	"RVN": "ROVANIEMI",
	"RVO": "REIVILO",
	"RVR": "GREEN RIVER",
	"RVS": "TULSA",
	"RVY": "RIVERA",
	"RWB": "ROWAN BAY",
	"RWF": "REDWOOD FALLS",
	"RWI": "ROCKY MOUNT",
	"RWL": "RAWLINS",
	"RWN": "ROVNO",
	"RWP": "RAWALPINDI/ISLAMABAD",
	"RXA": "RAUDHA",
	"RXS": "ROXAS CITY",
	"RYB": "RYBINSK",
	"RYK": "RAHIMYAR KHAN",
	"RYN": "ROYAN",
	"RYO": "RIO TURBIO",
	"RZA": "SANTA CRUZ",
	"RZE": "RZESZOW",
	"RZN": "RYAZAN",
	"RZR": "RAMSAR",
	"RZY": "REZAYIEH",
	"RZZ": "ROANOKE RAPIDS",
	"SAA": "SARATOGA",
	"SAB": "SABA",
	"SAC": "SACRAMENTO",
	"SAD": "SAFFORD",
	"SAE": "SANGI",
	"SAF": "SANTA FE",
	"SAG": "SAGWON",
	"SAH": "SANA'A",
	"SAI": "SAN MARINO",
	"SAJ": "SIRAJGANJ",
	"SAK": "SAUDARKROKUR",
	"SAL": "SAN SALVADOR",
	"SAM": "SALAMO",
	"SAN": "SAN DIEGO",
	"SAO": "SAO PAULO",
	"SAP": "SAN PEDRO SULA",
	"SAQ": "SAN ANDROS",
	"SAR": "SPARTA",
	"SAS": "SALTON CITY",
	"SAT": "SAN ANTONIO",
	"SAU": "SAWU",
	"SAV": "SAVANNAH",
	"SAW": "MARQUETTE",
	"SAX": "SAMBU",
	"SAY": "SIENA",
	"SAZ": "SASSTOWN",
	"SBA": "SANTA BARBARA",
	"SBB": "SANTA BARBARA",
	"SBC": "SIBASA",
	"SBD": "SAN BERNARDINO",
	"SBE": "SEEB",
	"SBG": "SABANG",
	"SBH": "ST. BARTHELEMY",
	"SBI": "KOUNDARA",
	"SBJ": "SAO MATEUS",
	"SBK": "ST BRIEUC",
	"SBL": "SANTA ANA",
	"SBM": "SHEBOYGAN",
	"SBN": "SOUTH BEND",
	"SBO": "SALINA",
	"SBP": "SAN LUIS OBISPO",
	"SBQ": "SIBI",
	"SBR": "SAIBAI ISLAND",
	"SBS": "STEAMBOAT SPRINGS",
	"SBT": "SAN BERNARDINO",
	"SBU": "SPRINGBOK",
	"SBV": "SABAH",
	"SBW": "SIBU",
	"SBX": "SHELBY",
	"SBY": "SALISBURY",
	"SBZ": "SIBIU",
	"SCA": "SANTA CATALINA",
	"SCB": "SCRIBNER",
	"SCC": "DEADHORSE",
	"SCD": "SULACO",
	"SCE": "STATE COLLEGE",
	"SCF": "PHOENIX",
	"SCG": "SPRING CREEK",
	"SCH": "SCHENECTADY",
	"SCI": "SAN CRISTOBAL",
	"SCJ": "SMITH COVE",
	"SCK": "STOCKTON",
	"SCL": "SANTIAGO",
	"SCM": "SCAMMON BAY",
	"SCN": "SAARBRUCKEN",
	"SCO": "SHEVCHENKO",
	"SCP": "ST. CREPIN/VARS",
	"SCQ": "SANTIAGO DE COMPOSTELA",
	"SCS": "LERWICK",
	"SCT": "SOCOTRA",
	"SCU": "SANTIAGO",
	"SCV": "SUCEAVA",
	"SCW": "SYKTYVKAR",
	"SCX": "SALINA CRUZ",
	"SCY": "GALAPAGOS ISLANDS-S CRISTOB",
	"SCZ": "SANTA CRUZ",
	"SDA": "BAGHDAD-SADDAM",
	"SDB": "SALDANHA BAY",
	"SDC": "SANDCREEK",
	"SDD": "LUBANGO",
	"SDE": "SANTIAGO DEL ESTERO",
	"SDF": "LOUISVILLE",
	"SDG": "SANANDAJ",
	"SDH": "SANTA ROSA DE COPAN",
	"SDI": "SAIDOR",
	"SDJ": "SENDAI",
	"SDK": "SANDAKAN",
	"SDL": "SUNDSVALL",
	"SDM": "SAN DIEGO",
	"SDN": "SANDANE",
	"SDO": "RYOTSU SADO ISLAND",
	"SDP": "SAND POINT",
	"SDQ": "SANTO DOMINGO",
	"SDR": "SANTANDER",
	"SDS": "SADO SHIMA",
	"SDT": "SAIDU SHARIF",
	"SDU": "RIO DE JANEIRO",
	"SDV": "TEL AVIV.",
	"SDW": "SANDWIP",
	"SDX": "SEDONA",
	"SDY": "SIDNEY",
	"SEA": "SEATTLE",
	"SEB": "SEBHA",
	"SEC": "SERRE CHEVALIER",
	"SED": "SEDOM",
	"SEE": "SAN DIEGO",
	"SEF": "SEBRING",
	"SEG": "SELINSGROVE",
	"SEH": "SENGGEH",
	"SEI": "SENHOR DO BONFIM",
	"SEJ": "SEYDISFJORDUR",
	"SEK": "KSAR ES SOUK",
	"SEL": "SEOUL",
	"SEM": "SELMA",
	"SEN": "SOUTHEND",
	"SEO": "SEGUELA",
	"SEP": "STEPHENVILLE",
	"SEQ": "SUNGAI PAKNING",
	"SER": "SEYMOUR",
	"SES": "SELMA",
	"SET": "SAN ESTEBAN",
	"SEU": "SERONERA",
	"SEV": "SEVERODONECK",
	"SEW": "SIWA",
	"SEX": "SEMBACH",
	"SEY": "SELIBABY",
	"SEZ": "MAHE IS.",
	"SFA": "SFAX",
	"SFB": "SANFORD",
	"SFC": "ST. FRANCOIS",
	"SFD": "SAN FERNANDO",
	"SFE": "SAN FERNANDO",
	"SFF": "SPOKANE",
	"SFG": "ST. MARTIN",
	"SFH": "SAN FELIPE",
	"SFI": "SAFI",
	"SFJ": "SONDRE STROMFJORD",
	"SFK": "SOURE",
	"SFL": "SAO FILIPE",
	"SFM": "SANFORD",
	"SFN": "SANTA FE",
	"SFO": "SAN FRANCISCO",
	"SFP": "SURFERS PARADISE",
	"SFQ": "SAN FRANCISQUITO",
	"SFS": "SUBIC BAY",
	"SFT": "SKELLEFTEA",
	"SFU": "SAFIA",
	"SFV": "SANTA FE DO SUL",
	"SFW": "SANTA FE",
	"SFX": "SAN FELIX",
	"SGA": "SHEGHNAN",
	"SGB": "SINGAUA",
	"SGC": "SURGUT",
	"SGD": "SONDERBORG",
	"SGE": "SIEGEN",
	"SGF": "SPRINGFIELD",
	"SGG": "SIMANGGANG",
	"SGH": "SPRINGFIELD",
	"SGI": "SARGODHA",
	"SGJ": "SAGARAI",
	"SGK": "SANGAPI",
	"SGM": "SAN IGNACIO",
	"SGN": "HO CHI MINH",
	"SGO": "ST. GEORGE",
	"SGP": "SHAY GAP",
	"SGQ": "SANGGATA",
	"SGR": "SUGAR LAND",
	"SGS": "SANGA SANGA",
	"SGT": "STUTTGART",
	"SGU": "ST. GEORGE",
	"SGV": "SIERRA GRANDE",
	"SGW": "SAGINAW",
	"SGX": "SONGEA",
	"SGY": "SKAGWAY",
	"SGZ": "SINGORA",
	"SHA": "SHANGHAI",
	"SHB": "NAKASHIBETSU",
	"SHC": "INDASELASSIE",
	"SHD": "STAUNTON",
	"SHE": "SHENYANG",
	"SHF": "SHANHAIGUAN",
	"SHG": "SHUNGNAK",
	"SHH": "SHISHMAREF",
	"SHI": "SHIMOJISHIMA",
	"SHJ": "SHARJAH",
	"SHK": "SEHONGHONG",
	"SHL": "SHILLONG",
	"SHM": "NANKI SHIRAHAMA",
	"SHN": "SHELTON",
	"SHO": "SOKCHO",
	"SHP": "QINHUANGDAO",
	"SHQ": "SOUTHPORT",
	"SHR": "SHERIDAN",
	"SHS": "SHASHI",
	"SHT": "SHEPPARTON",
	"SHU": "SMITH POINT",
	"SHV": "SHREVEPORT",
	"SHW": "SHARAWRAH",
	"SHX": "SHAGELUK",
	"SHY": "SHINYANGA",
	"SHZ": "SESHUTE'S",
	"SIA": "XIAN",
	"SIB": "SIBITI",
	"SIC": "SANTO ANTONIO DO ICA",
	"SID": "SAL ISLAND",
	"SIE": "SINES",
	"SIF": "SIMRA",
	"SIG": "SAN JUAN",
	"SIH": "SILGARHI DOTI",
	"SII": "SIDI IFNI",
	"SIJ": "SIGLUFJORDUR",
	"SIK": "SIKESTON",
	"SIL": "SILA",
	"SIM": "SIMBAI",
	"SIN": "SINGAPORE",
	"SIO": "SMITHTON",
	"SIP": "SIMFEROPOL",
	"SIQ": "SINGKEP",
	"SIR": "SION",
	"SIS": "SISHEN",
	"SIT": "SITKA",
	"SIU": "SIUNA",
	"SIV": "SULLIVAN",
	"SIX": "SINGLETON",
	"SIY": "MONTAGUE",
	"SIZ": "SISSANO",
	"SJA": "SAN JUAN",
	"SJB": "SAN JOAQUIN",
	"SJC": "SAN JOSE",
	"SJD": "SAN JOSE DEL CABO",
	"SJE": "SAN JOSE DEL GUAVIARO",
	"SJF": "ST. JOHN",
	"SJG": "SAN PEDRO JAGUA",
	"SJH": "SAN JUAN DEL CESAR",
	"SJI": "SAN JOSE",
	"SJJ": "SARAJEVO",
	"SJK": "SAO JOSE DOS CAMPOS",
	"SJL": "SAO GABRIEL",
	"SJM": "SAN JUAN DE LA M.",
	"SJN": "ST. JOHNS",
	"SJO": "SAN JOSE",
	"SJP": "SAO JOSE DO RIO PRETO",
	"SJQ": "SESHEKE",
	"SJR": "SAN JUAN DE URABA",
	"SJS": "SAN JOSE",
	"SJT": "SAN ANGELO",
	"SJU": "SAN JUAN",
	"SJV": "SAN JAVIER",
	"SJW": "SHIJIAZHUANG",
	"SJX": "SARTANEJA",
	"SJY": "SEINAJOKI",
	"SJZ": "SAN JORGE ISLAND",
	"SKA": "SPOKANE",
	"SKB": "ST. KITTS",
	"SKC": "SUKI",
	"SKD": "SAMARKAND",
	"SKE": "SKIEN",
	"SKF": "SAN ANTONIO",
	"SKG": "THESSALONIKI",
	"SKH": "SURKHET",
	"SKI": "SKIKDA",
	"SKJ": "SITKINAK",
	"SKK": "SHAKTOOLIK",
	"SKL": "ISLE OF SKYE",
	"SKM": "SKELDON",
	"SKN": "STOKMARKNES",
	"SKO": "SOKOTO",
	"SKP": "SKOPJE",
	"SKQ": "SEKAKES",
	"SKR": "SHAKISO",
	"SKS": "SKRYDSTRUP",
	"SKT": "SIALKOT",
	"SKU": "SKIROS",
	"SKV": "SANTA KATARINA",
	"SKW": "SKWENTNA",
	"SKX": "SARANSK",
	"SKY": "SANDUSKY",
	"SKZ": "SUKKUR",
	"SLA": "SALTA",
	"SLB": "STORM LAKE",
	"SLC": "SALT LAKE CITY",
	"SLD": "SLIAC",
	"SLE": "SALEM",
	"SLF": "SULAYEL",
	"SLG": "SILOAM SPRINGS",
	"SLH": "OLA",
	"SLI": "SOLWEZI",
	"SLJ": "CHANDLER",
	"SLK": "SARANAC LAKE",
	"SLL": "SALALAH",
	"SLM": "SALAMANCA",
	"SLN": "SALINA",
	"SLO": "SALEM",
	"SLP": "SAN LUIS POTOSI",
	"SLQ": "SLEETMUTE",
	"SLR": "SULPHUR SPRINGS",
	"SLS": "SILISTRA",
	"SLT": "SALIDA",
	"SLU": "ST. LUCIA",
	"SLV": "SIMLA",
	"SLW": "SALTILLO",
	"SLX": "SALT CAY",
	"SLY": "SALEHARD",
	"SLZ": "SAO LUIZ",
	"SMA": "SANTA MARIA",
	"SMB": "CERRO EL SOMBRERO",
	"SMC": "SANTA MARIA",
	"SME": "SOMERSET",
	"SMF": "SACRAMENTO",
	"SMG": "SANTA MARIA",
	"SMH": "SAPMANGA",
	"SMI": "SAMOS ISLAND",
	"SMJ": "SIM",
	"SMK": "ST. MICHAEL",
	"SML": "STELLA MARIS",
	"SMM": "SEMPORNA",
	"SMN": "SALMON",
	"SMO": "LOS ANGELES",
	"SMP": "STOCKHOLM",
	"SMQ": "SAMPIT",
	"SMR": "SANTA MARTA",
	"SMS": "ST. MARIE",
	"SMT": "SUN MOON LAKE",
	"SMU": "SHEEP MOUNTAIN",
	"SMV": "ST. MORITZ",
	"SMW": "SMARA",
	"SMX": "SANTA MARIA",
	"SMY": "SIMENTI",
	"SMZ": "STOELMANSEILAND",
	"SNA": "SANTA ANA",
	"SNB": "SNAKE BAY",
	"SNC": "SALINAS",
	"SND": "SENO",
	"SNE": "SAO NICOLAU",
	"SNF": "SAN FELIPE",
	"SNG": "SAN IGNACIO DE VELASCO",
	"SNH": "STANTHORPE",
	"SNI": "SINOE/GREENVILLE",
	"SNJ": "SAN JULIAN",
	"SNK": "SNYDER",
	"SNL": "SHAWNEE",
	"SNM": "SAN IGNACIO DE MOXOS",
	"SNN": "SHANNON",
	"SNO": "SAKON NAKHON",
	"SNP": "ST. PAUL ISLAND",
	"SNQ": "SAN QUINTIN",
	"SNR": "ST. NAZAIRE",
	"SNS": "SALINAS",
	"SNT": "SABANA DE TORRES",
	"SNU": "SANTA CLARA",
	"SNV": "SANTA ELENA",
	"SNW": "SANDOWAY",
	"SNX": "SABANA DE LA MAR",
	"SNY": "SIDNEY",
	"SOB": "SOBRAL",
	"SOC": "SOLO",
	"SOD": "SOROCABA",
	"SOE": "SOUANKE",
	"SOF": "SOFIA",
	"SOG": "SOGNDAL",
	"SOH": "SOLITA",
	"SOI": "SOUTH MOLLE IS.",
	"SOJ": "SORKJOSEN",
	"SOK": "SEMONGKONG",
	"SOL": "SOLOMON",
	"SOM": "SAN TOME",
	"SON": "SPIRITU SANTO",
	"SOO": "SODERHAMN",
	"SOP": "PINEHURST/SOUTHERN PINES",
	"SOQ": "SORONG",
	"SOR": "AL THAURAH",
	"SOT": "SODANKYLA",
	"SOU": "SOUTHAMPTON",
	"SOV": "SELDOVIA",
	"SOW": "SHOW LOW",
	"SOX": "SOGAMOSO",
	"SOY": "STRONSAY",
	"SOZ": "SOLENZARA",
	"SPA": "GREENVILLE/SPARTANBURG",
	"SPB": "ST.THOMAS",
	"SPC": "SANTA CRUZ LA PALMA",
	"SPD": "SAIDPUR",
	"SPE": "SEPULOT",
	"SPF": "SPEARFISH",
	"SPG": "TAMPA",
	"SPH": "SOPU",
	"SPI": "SPRINGFIELD",
	"SPJ": "SPARTA",
	"SPK": "SAKURAMORI AIRPORT(SAPPORO",
	"SPN": "SAIPAN",
	"SPP": "MENONGUE",
	"SPQ": "SAN PEDRO",
	"SPR": "SAN PEDRO",
	"SPS": "WICHITA FALLS",
	"SPT": "SIPITANG",
	"SPU": "SPLIT",
	"SPV": "SEPIK PLAINS",
	"SPW": "SPENCER",
	"SPY": "SAN PEDRO",
	"SPZ": "SPRINGDALE",
	"SQA": "SANTA YNEZ",
	"SQB": "SANTA ANA",
	"SQC": "SOUTHERN CROSS",
	"SQE": "SAN LUIS DE PALENQUE",
	"SQF": "SOLANO",
	"SQG": "SINTANG",
	"SQH": "SON LA",
	"SQI": "STERLING ROCK FALLS",
	"SQJ": "SHEHDI",
	"SQK": "SIDI BARANI",
	"SQL": "SAN CARLOS",
	"SQM": "SAO MIGUEL DO ARAGUAIA",
	"SQN": "SANANA",
	"SQO": "STORUMAN",
	"SQP": "STARCKE",
	"SQQ": "SIAULIAI",
	"SQR": "SOROAKO",
	"SQS": "SAN IGNACIO",
	"SQT": "SAMARAI ISLAND",
	"SQU": "SAPOSOA",
	"SQV": "PORT ANGELES",
	"SQW": "SKIVE",
	"SRA": "SANTA ROSA",
	"SRB": "SANTA ROSA",
	"SRC": "SEARCY",
	"SRD": "SAN RAMON",
	"SRE": "SUCRE",
	"SRG": "SEMARANG",
	"SRH": "SARH",
	"SRI": "SAMARINDA",
	"SRJ": "SAN BORJA",
	"SRK": "SIERRA LEONE",
	"SRL": "SANTA ROSALIA",
	"SRM": "SANDRINGHAM",
	"SRN": "STRAHAN",
	"SRO": "SANTANA RAMOS",
	"SRP": "SAGVAAG",
	"SRQ": "SARASOTA/BRADENTON",
	"SRS": "SAN MARCOS",
	"SRT": "SOROTI",
	"SRV": "STONY RIVER",
	"SRW": "SALISBURY",
	"SRX": "SERT",
	"SRY": "SARY",
	"SRZ": "SANTA CRUZ",
	"SSA": "SALVADOR",
	"SSB": "ST. CROIX",
	"SSC": "SUMTER",
	"SSD": "SAN FELIPE",
	"SSE": "SHOLAPUR",
	"SSG": "MALABO",
	"SSH": "SHARM EL SHEIKH",
	"SSI": "ST. SIMONS ISLAND",
	"SSJ": "SANDNESSJOEN",
	"SSK": "STURT CREEK",
	"SSL": "SANTA ROSALIA",
	"SSM": "SAULT STE. MARIE",
	"SSN": "SEOUL",
	"SSO": "SAO SIMAO",
	"SSP": "SILVER PLAINS",
	"SSQ": "LA SARRE",
	"SSR": "SARA",
	"SSS": "SIASSI",
	"SST": "SANTA TERESITA",
	"SSV": "SIASI",
	"SSW": "STUART ISLAND",
	"SSX": "SAMSUN",
	"SSY": "M'BANZA CONGO",
	"SSZ": "SANTOS",
	"STA": "STAUNING",
	"STB": "SANTA BARBARA",
	"STC": "ST. CLOUD",
	"STD": "SANTO DOMINGO",
	"STE": "STEVENS POINT",
	"STF": "SANTA FE DO SUL",
	"STG": "ST. GEORGE ISLAND",
	"STH": "STRATHMORE",
	"STI": "SANTIAGO",
	"STJ": "ST. JOSEPH",
	"STK": "STERLING",
	"STL": "ST. LOUIS",
	"STM": "SANTAREM",
	"STN": "LONDON",
	"STO": "STOCKHOLM",
	"STP": "MINNEAPOLIS/ST.PAUL",
	"STQ": "ST. MARYS",
	"STR": "STUTTGART",
	"STS": "SANTA ROSA",
	"STT": "CHARLOTTE AMALIE",
	"STU": "SANTA CRUZ",
	"STV": "SURAT",
	"STW": "STAVROPOL",
	"STX": "CHRISTIANSTED",
	"STY": "SALTO",
	"STZ": "SANTA TEREZINHA",
	"SUA": "STUART",
	"SUB": "SURABAYA",
	"SUC": "SUNDANCE",
	"SUD": "STROUD",
	"SUE": "STURGEON BAY",
	"SUF": "LAMETIA-TERME",
	"SUG": "SURIGAO",
	"SUH": "SUR",
	"SUI": "SUKHUMI",
	"SUJ": "SATU MARE",
	"SUK": "SAMCHUK",
	"SUL": "SUI",
	"SUM": "SUMTER",
	"SUN": "SUN VALLEY",
	"SUO": "SUN RIVER",
	"SUP": "SUMENEP",
	"SUQ": "SUCUA",
	"SUR": "SUMMER BEAVER",
	"SUS": "ST. LOUIS",
	"SUT": "SUMBAWANGA",
	"SUU": "FAIRFIELD",
	"SUV": "SUVA",
	"SUW": "DULUTH MN/SUPERIOR",
	"SUX": "SIOUX CITY",
	"SUY": "SUDUREYRI",
	"SUZ": "SURIA",
	"SVA": "SAVOONGA",
	"SVB": "SAMBAVA",
	"SVC": "SILVER CITY",
	"SVD": "ST. VINCENT",
	"SVE": "SUSANVILLE",
	"SVF": "SAVE",
	"SVG": "STAVANGER",
	"SVH": "STATESVILLE",
	"SVI": "SAN VINCENTE DEL CAGUAN",
	"SVJ": "SVOLVAER",
	"SVK": "SILVER CREEK",
	"SVL": "SAVONLINNA",
	"SVM": "ST. PAUL'S MISSION",
	"SVN": "SAVANNAH",
	"SVO": "MOSCOW",
	"SVP": "BIE",
	"SVQ": "SEVILLE",
	"SVR": "SVAY RIENG",
	"SVS": "STEVENS VILLAGE",
	"SVU": "SAVUSAVU",
	"SVV": "SAN SALVADOR DE PAUL",
	"SVW": "SPARREVOHN",
	"SVX": "EKATERINBURG (SVERDLOVSK)",
	"SVY": "SAVO",
	"SVZ": "SAN ANTONIO",
	"SWA": "SHANTOU",
	"SWB": "SHAW RIVER",
	"SWC": "STAWELL",
	"SWD": "SEWARD",
	"SWE": "SIWEA",
	"SWF": "NEWBURGH",
	"SWG": "SATWAG",
	"SWH": "SWAN HILL",
	"SWI": "SWINDON",
	"SWJ": "SOUTH WEST BAY",
	"SWM": "SUIA-MISSU",
	"SWN": "SAHIWAL",
	"SWO": "STILLWATER",
	"SWP": "SWAKOPMUND",
	"SWQ": "SUMBAWA IS.",
	"SWR": "SILUR",
	"SWS": "SWANSEA",
	"SWT": "STRZHEWOI",
	"SWU": "SU WON CITY",
	"SWV": "SHIKARPUR",
	"SWW": "SWEETWATER",
	"SWX": "SHAKAWE",
	"SWY": "SITIAWAN",
	"SWZ": "SYDNEY",
	"SXA": "SIALUM",
	"SXB": "STRASBOURG",
	"SXC": "CATALINA ISLAND",
	"SXD": "SOPHIA ANTIPOLIS",
	"SXE": "SALE",
	"SXF": "BERLIN",
	"SXG": "SENANGA",
	"SXH": "SEHULEA",
	"SXI": "SIRRI ISLAND",
	"SXJ": "SHANSHAN",
	"SXK": "SAUMLAKI",
	"SXL": "SLIGO",
	"SXM": "ST. MAARTEN",
	"SXN": "SAO JOSE DO XINGU",
	"SXO": "SAO FELIX ARAGUAIA",
	"SXP": "SHELDON POINT",
	"SXQ": "SOLDOTNA",
	"SXR": "SRINAGAR",
	"SXS": "SAHABAT 16",
	"SXT": "TAMAN NEGARA",
	"SXU": "SODDU",
	"SXV": "SALEM",
	"SXW": "SAUREN",
	"SXX": "SAO FELIX DO XINGU",
	"SXY": "SIDNEY",
	"SXZ": "SIIRT",
	"SYA": "SHEMYA ISLAND",
	"SYB": "SEAL BAY",
	"SYD": "SYDNEY",
	"SYE": "SA'DAH",
	"SYF": "SILVA BAY",
	"SYI": "SHELBYVILLE",
	"SYK": "STYKKISHOLMUR",
	"SYM": "SIMAO",
	"SYN": "STANTON",
	"SYO": "SHONAI",
	"SYP": "SANTIAGO",
	"SYR": "SYRACUSE",
	"SYS": "SUNCHON",
	"SYT": "SAINT-YAN",
	"SYU": "SUE ISLAND",
	"SYV": "SYLVESTER",
	"SYX": "SANYA",
	"SYY": "STORNOWAY",
	"SYZ": "SHIRAZ",
	"SZA": "SOYO",
	"SZC": "SANTA CRUZ",
	"SZG": "SALZBURG",
	"SZH": "SENIPAH",
	"SZK": "SKUKUZA",
	"SZL": "WARRENSBURG",
	"SZO": "SHANZHOU",
	"SZP": "SANTA PAULA",
	"SZQ": "SAENZ PENA",
	"SZR": "STARA ZAGORA",
	"SZS": "STEWART ISLAND",
	"SZU": "SEGOU",
	"SZV": "SUZHOU",
	"SZX": "SHENZHEN",
	"SZZ": "SZCZECIN",
	"TAA": "TARAPINA",
	"TAB": "TOBAGO",
	"TAC": "TACLOBAN",
	"TAD": "TRINIDAD",
	"TAE": "TAEGU",
	"TAF": "ORAN",
	"TAG": "TAGBILARAN",
	"TAH": "ANNA ISLAND",
	"TAI": "TAIZ",
	"TAJ": "TADJI",
	"TAK": "TAKAMATSU",
	"TAL": "TANANA",
	"TAM": "TAMPICO",
	"TAN": "TANGALOOMA",
	"TAO": "QINGDAO",
	"TAP": "TAPACHULA",
	"TAQ": "TARCOOLA",
	"TAR": "TARANTO",
	"TAS": "TASHKENT",
	"TAT": "TATRY/POPRAD",
	"TAU": "TAURAMENA",
	"TAV": "TAU",
	"TAW": "TACUAREMBO",
	"TAX": "TALIABU",
	"TAY": "TARTU",
	"TAZ": "TAKURONG",
	"TBA": "TABIBUGA",
	"TBB": "TUY HOA",
	"TBC": "TUBA CITY",
	"TBD": "TIMBIQUI",
	"TBE": "TIMBUNKE",
	"TBF": "TABITEUEA NORTH",
	"TBG": "TABUBIL",
	"TBH": "TABLAS",
	"TBI": "THE BIGHT",
	"TBJ": "TABARKA",
	"TBK": "TIMBER CREEK",
	"TBL": "TABLELAND",
	"TBM": "TUMBANG SAMBA",
	"TBN": "FT. LEONARD WOOD",
	"TBO": "TABORA",
	"TBP": "TUMBES",
	"TBR": "STATESBORO",
	"TBS": "TBILISI",
	"TBT": "TABATINGA",
	"TBU": "TONGATAPU",
	"TBV": "TABAL",
	"TBW": "TAMBOW",
	"TBY": "TSABONG",
	"TBZ": "TABRIZ",
	"TCA": "TENNANT CREEK",
	"TCB": "TREASURE CAY",
	"TCC": "TUCUMCARI",
	"TCD": "TARAPACA",
	"TCE": "TULCEA",
	"TCF": "TOCOA",
	"TCG": "TOCACHE",
	"TCH": "TCHIBANGA",
	"TCI": "TENERIFE",
	"TCJ": "TOREMBI",
	"TCK": "TINBOLI",
	"TCL": "TUSCALOOSA",
	"TCM": "TACOMA",
	"TCN": "TEHUACAN",
	"TCO": "TUMACO",
	"TCP": "TABA",
	"TCQ": "TACNA",
	"TCS": "TRUTH OR CONSEQUENCES",
	"TCT": "TAKOTNA",
	"TCU": "TACUANE",
	"TCV": "TETE",
	"TCW": "TOCUMWAL",
	"TDA": "TRINIDAD",
	"TDB": "TETEBEDI",
	"TDD": "TRINIDAD",
	"TDG": "TANDAG",
	"TDJ": "TADJOURA",
	"TDK": "TALDY-KURGAN",
	"TDL": "TANDIL",
	"TDR": "THEODORE",
	"TDT": "TANDA TULA",
	"TDV": "TANANDAVA",
	"TDW": "AMARILLO",
	"TEA": "TELA",
	"TEB": "TETERBORO",
	"TEC": "TELEMACO BORBA",
	"TED": "THISTED",
	"TEE": "TEBESSA",
	"TEF": "TELFER",
	"TEG": "TENKODOGO",
	"TEH": "TETLIN",
	"TEI": "TEZU",
	"TEK": "TATITLEK",
	"TEL": "TELUPID",
	"TEM": "TEMORA",
	"TEO": "TERAPO",
	"TEP": "TEPTEP",
	"TER": "TERCEIRA",
	"TES": "TESSENEI",
	"TET": "TETE",
	"TEU": "TE ANAU",
	"TEX": "TELLURIDE",
	"TEY": "THINGEYRI",
	"TEZ": "TEZPUR",
	"TFA": "TILFALMIN",
	"TFF": "TEFE",
	"TFI": "TUFI",
	"TFL": "TEOFILO OTONI",
	"TFM": "TELEFOMIN",
	"TFN": "TENERIFE-N LOSRODEO",
	"TFO": "TEOFILO OTONI",
	"TFR": "TARBES",
	"TFS": "TENERIFE.",
	"TFT": "TAFTAN",
	"TFY": "TARFAYA",
	"TGD": "PODGORICA",
	"TGE": "TUSKEGEE",
	"TGF": "TIGNES",
	"TGG": "KUALA TRENGGANU",
	"TGH": "ONGOA",
	"TGI": "TINGO MARIA",
	"TGJ": "TIGA",
	"TGL": "TAGULA",
	"TGM": "TIRGU MURES",
	"TGN": "TRARALGON",
	"TGO": "TONGLIAO",
	"TGR": "TOUGGOURT",
	"TGS": "TRIGO DE MORAIS",
	"TGT": "TANGA",
	"TGU": "TEGUCIGALPA",
	"TGV": "TARGOVISHTE",
	"TGX": "TINGRELA",
	"TGZ": "TUXTLA GUTIERREZ",
	"THA": "TULLAHOMA",
	"THB": "THABA TSEKA",
	"THC": "TCHIEN",
	"THE": "TERESINA",
	"THF": "BERLIN",
	"THG": "THANGOOL",
	"THH": "TAHAROA",
	"THI": "TICHITT",
	"THK": "THAKHEK",
	"THL": "TACHILEK",
	"THN": "TROLLHATTAN",
	"THO": "THORSHOFN",
	"THP": "THERMOPOLIS",
	"THR": "TEHRAN",
	"THS": "SUKHOTHAI",
	"THT": "TAMCHACKETT",
	"THU": "THULE",
	"THY": "THOYANDOU VENDA",
	"THZ": "TAHOUA",
	"TIA": "TIRANA",
	"TIB": "TIBU",
	"TIC": "TINAK",
	"TID": "TIARET",
	"TIE": "TIPPI",
	"TIF": "TAIF",
	"TIG": "TINGWON",
	"TIH": "TIKEHAU",
	"TII": "TIRINKOT",
	"TIJ": "TIJUANA",
	"TIK": "OKLAHOMA CITY",
	"TIM": "TIMIKA",
	"TIN": "TINDOUF",
	"TIO": "TILIN",
	"TIP": "TRIPOLI",
	"TIQ": "TINIAN",
	"TIR": "TIRUPATI",
	"TIS": "THURSDAY IS.",
	"TIU": "TIMARU",
	"TIV": "TIVAT",
	"TIW": "TACOMA",
	"TIX": "TITUSVILLE",
	"TIY": "TIDJIKJA",
	"TIZ": "TARI",
	"TJA": "TARIJA",
	"TJB": "TANJUNG BALAI",
	"TJG": "TANJUNG WARUKIN",
	"TJH": "TOYOOKA",
	"TJI": "TRUJILLO",
	"TJK": "TOKAT",
	"TJM": "TYUMEN",
	"TJQ": "TANDJUNG PANDAN",
	"TJS": "TANJUNG SELOR",
	"TJV": "THANJAVUR",
	"TKA": "TALKEETNA",
	"TKB": "TEKADU",
	"TKC": "TIKO",
	"TKD": "TAKORADI",
	"TKE": "TENAKEE",
	"TKF": "TRUCKEE",
	"TKG": "BANDAR LAMPUNG",
	"TKH": "TAKHLI",
	"TKI": "TOKEEN",
	"TKJ": "TOK",
	"TKK": "TRUK",
	"TKM": "TIKAL",
	"TKN": "TOKUNO SHIMA",
	"TKO": "TLOKOENG",
	"TKP": "TAKAPOTO IS.",
	"TKQ": "KIGOMA",
	"TKR": "THAKURGAON",
	"TKS": "TOKUSHIMA",
	"TKT": "TAK",
	"TKU": "TURKU",
	"TKV": "TATAKOTO",
	"TKW": "TEKIN",
	"TKX": "TAKAROA",
	"TKY": "TURKEY CREEK",
	"TKZ": "TOKOROA",
	"TLA": "TELLER",
	"TLB": "TARBELA",
	"TLC": "TOLUCA",
	"TLD": "TULI LODGE",
	"TLE": "TULEAR",
	"TLF": "TELIDA",
	"TLG": "TULAGI",
	"TLH": "TALLAHASSEE",
	"TLI": "TOLITOLI",
	"TLJ": "TATALINA",
	"TLK": "TALKNAFJORDUR",
	"TLL": "TALLINN",
	"TLM": "TLEMCEN",
	"TLN": "TOULON/HYERES",
	"TLO": "TOL",
	"TLP": "TUMOLBIL",
	"TLR": "TULARE",
	"TLS": "TOULOUSE",
	"TLT": "TULUKSAK",
	"TLU": "TOLU",
	"TLV": "TEL AVIV-YAFO",
	"TLW": "TALASEA",
	"TLX": "TALCA",
	"TLZ": "CATALAO",
	"TMA": "TIFTON",
	"TMC": "TAMBOLAKA",
	"TMD": "TIMBEDRA",
	"TME": "TAME",
	"TMG": "TOMANGGONG",
	"TMH": "TANAHMERAH",
	"TMI": "TUMLINGTAR",
	"TMJ": "TERMEZ",
	"TMK": "TAM-KY",
	"TML": "TAMALE",
	"TMM": "TAMATAVE",
	"TMN": "TAMANA",
	"TMO": "TUMEREMO",
	"TMP": "TAMPERE",
	"TMQ": "TAMBAO",
	"TMR": "TAMANRASSET",
	"TMS": "SAO TOME IS.",
	"TMT": "TROMBETAS",
	"TMU": "TAMBOR",
	"TMW": "TAMWORTH",
	"TMX": "TIMIMOUN",
	"TMY": "TIOM",
	"TMZ": "THAMES",
	"TNA": "JINAN",
	"TNB": "TANAHGROGOT",
	"TNC": "TIN CITY",
	"TND": "TRINIDAD",
	"TNE": "TANEGASHIMA",
	"TNF": "TOUSSUS-LE-NOBLE",
	"TNG": "TANGIER",
	"TNH": "TONGHUA",
	"TNI": "SATNA",
	"TNJ": "TANJUNG PINANG",
	"TNK": "TUNUNAK",
	"TNL": "TERNOPOL",
	"TNN": "TAINAN",
	"TNO": "TAMARINDO",
	"TNP": "TWENTY NINE PALMS",
	"TNQ": "TERAINA",
	"TNR": "TANANARIVE",
	"TNS": "TUNGSTEN",
	"TNT": "MIAMI",
	"TNU": "NEWTON",
	"TNV": "TABUAERAN",
	"TNX": "STUNG TRENG",
	"TOA": "TORRANCE",
	"TOB": "TOBRUK",
	"TOC": "TOCCOA",
	"TOD": "TIOMAN",
	"TOE": "TOZEUR",
	"TOF": "TOMSK",
	"TOG": "TOGIAK",
	"TOH": "TORRES",
	"TOI": "TROY",
	"TOJ": "MADRID",
	"TOK": "TOROKINA",
	"TOL": "TOLEDO",
	"TOM": "TOMBOUCTOU",
	"TON": "TONU",
	"TOO": "TOLEDO",
	"TOP": "TOPEKA",
	"TOQ": "TOCOPILLA",
	"TOR": "TORRINGTON",
	"TOS": "TROMSO",
	"TOT": "TOTNESS",
	"TOU": "TOUHO",
	"TOV": "TORTOLA",
	"TOW": "TOLEDO",
	"TOX": "TOCANTINIA",
	"TOY": "TOYAMA",
	"TOZ": "TOUBA",
	"TPA": "TAMPA",
	"TPC": "TARAPOA",
	"TPE": "TAIPEI",
	"TPG": "TAIPING",
	"TPH": "TONOPAH",
	"TPI": "TAPINI",
	"TPJ": "TAPLEJUNG",
	"TPK": "TAPAKTUAN",
	"TPL": "TEMPLE",
	"TPN": "TIPUTINI",
	"TPO": "TANALIAN POINT",
	"TPP": "TARAPOTO",
	"TPQ": "TEPIC",
	"TPR": "TOM PRICE",
	"TPS": "TRAPANI",
	"TPT": "TAPPITA",
	"TPU": "TAPURUCUARA",
	"TPY": "TOCANTINOPOLIS",
	"TQN": "TALUQAN",
	"TQS": "TRES ESQUINAS",
	"TRA": "TARAMAJIMA",
	"TRB": "TURBO",
	"TRC": "TORREON",
	"TRD": "TRONDHEIM",
	"TRE": "TIREE ISLAND",
	"TRF": "SANDEFJORD",
	"TRG": "TAURANGA",
	"TRH": "TRONA",
	"TRI": "BRISTOL",
	"TRJ": "TARAKBITS",
	"TRK": "TARAKAN",
	"TRM": "THERMAL",
	"TRN": "TURIN",
	"TRO": "TAREE",
	"TRQ": "TARAUACA",
	"TRR": "TRINCOMALEE",
	"TRS": "TRIESTE",
	"TRT": "TREMONTON",
	"TRU": "TRUJILLO",
	"TRV": "TRIVANDRUM",
	"TRW": "TARAWA",
	"TRX": "TRENTON",
	"TRY": "TORORO",
	"TRZ": "TRICHINOPOLY",
	"TSA": "TAIPEI",
	"TSB": "TSUMEB",
	"TSC": "TAISHA",
	"TSD": "TSHIPISE",
	"TSE": "TSELINOGRAD",
	"TSF": "VENICE",
	"TSG": "TANACROSS",
	"TSH": "TSHIKAPA",
	"TSI": "TSILI TSILI",
	"TSJ": "TSUSHIMA",
	"TSK": "TASKUL",
	"TSL": "TAMUIN",
	"TSM": "TAOS",
	"TSN": "TIANJIN",
	"TSO": "ISLES OF SCILLY-TRESCO",
	"TSP": "TEHACHAPI",
	"TSR": "TIMISOARA",
	"TSS": "NEW YORK",
	"TST": "TRANG",
	"TSU": "TABITEUEA SOUTH",
	"TSV": "TOWNSVILLE",
	"TSW": "TSEWI",
	"TSX": "TANJUNG SANTAN",
	"TSY": "TASIKMALAYA",
	"TSZ": "TSETSERLEG",
	"TTA": "TAN TAN",
	"TTB": "TORTOLI",
	"TTC": "TALTAL",
	"TTD": "TROUTDALE",
	"TTE": "TERNATE",
	"TTG": "TARTAGAL",
	"TTH": "THUMRAIT",
	"TTI": "TETIAROA IS.",
	"TTJ": "TOTTORI",
	"TTK": "TOTTENHAM HALE STATION",
	"TTL": "TURTLE ISLAND",
	"TTM": "TABLON DE TAMARA",
	"TTN": "TRENTON",
	"TTO": "BRITTON",
	"TTQ": "TORTUQUERO",
	"TTR": "TANA TORAJA",
	"TTS": "TSARATANANA",
	"TTT": "TAITUNG",
	"TTU": "TETUAN",
	"TUA": "TULCAN",
	"TUB": "TUBUAI IS.",
	"TUC": "TUCUMAN",
	"TUD": "TAMBACOUNDA",
	"TUE": "TUPILE",
	"TUF": "TOURS",
	"TUG": "TUGUEGARAO",
	"TUH": "TULLAHOMA",
	"TUI": "TURAIF",
	"TUJ": "TUM",
	"TUK": "TURBAT",
	"TUL": "TULSA",
	"TUM": "TUMUT",
	"TUN": "TUNIS",
	"TUO": "TAUPO",
	"TUP": "TUPELO",
	"TUQ": "TOUGAN",
	"TUR": "TUCURUI",
	"TUS": "TUCSON",
	"TUT": "TAUTA",
	"TUU": "TABUK",
	"TUV": "TUCUPITA",
	"TUW": "TUBALA",
	"TUX": "TUMBLER RIDGE",
	"TUY": "TULUM",
	"TUZ": "TUPA",
	"TVA": "MORAFENOBE",
	"TVC": "TRAVERSE CITY",
	"TVF": "THIEF RIVER FALLS",
	"TVI": "THOMASVILLE",
	"TVL": "LAKE TAHOE",
	"TVU": "TAVEUNI",
	"TVY": "TAVOY",
	"TWA": "TWIN HILLS",
	"TWB": "TOOWOOMBA",
	"TWD": "PORT TOWNSEND",
	"TWE": "TAYLOR",
	"TWF": "TWIN FALLS",
	"TWH": "CATALINA ISLAND",
	"TWN": "TEWANTIN",
	"TWP": "TORWOOD",
	"TWT": "TAWI- TAWI",
	"TWU": "TAWAU",
	"TWY": "TAWA",
	"TWZ": "MOUNT COOK-PUKA/TWIZL",
	"TXA": "DILOLO",
	"TXF": "TEIXEIRA DE FREITAS",
	"TXG": "TAICHUNG",
	"TXK": "TEXARKANA",
	"TXL": "BERLIN",
	"TXM": "TEMINABUAN",
	"TXN": "TUNXI",
	"TXR": "TANBAR",
	"TXU": "TABOU",
	"TYA": "TULA",
	"TYB": "TIBOOBURRA",
	"TYD": "TYNDA",
	"TYE": "TYONEK",
	"TYF": "TORSBY",
	"TYG": "THYLUNGRA",
	"TYL": "TALARA",
	"TYM": "STANIEL CAY",
	"TYN": "TAIYUAN",
	"TYO": "TOKYO",
	"TYP": "TOBERMOREY",
	"TYR": "TYLER",
	"TYS": "KNOXVILLE",
	"TYT": "TREINTA Y TRES",
	"TYZ": "TAYLOR",
	"TZA": "BELIZE",
	"TZM": "TIZIMIN",
	"TZN": "SOUTH ANDROS",
	"TZX": "TRABZON",
	"UAC": "SAN LUIS RIO COLORADO",
	"UAE": "MOUNT AUE",
	"UAH": "UA HUKA",
	"UAI": "SUAI",
	"UAK": "NARSSARSSUAQ",
	"UAL": "LUAU",
	"UAM": "ANDERSON AFB",
	"UAP": "UA POU",
	"UAQ": "SAN JUAN",
	"UAS": "SAMBURU",
	"UAX": "UAXACTUN",
	"UBA": "UBERABA",
	"UBB": "MABUIAG ISLAND",
	"UBI": "BUIN",
	"UBJ": "UBE",
	"UBP": "UBON",
	"UBR": "UBRUB",
	"UBT": "UBATUBA",
	"UBU": "KALUMBURU",
	"UCA": "UTICA",
	"UCE": "EUNICE",
	"UCK": "LUTSK",
	"UCN": "BUCHANAN",
	"UCT": "UKHTA",
	"UCY": "UNION CITY",
	"UCZ": "UCHIZA",
	"UDA": "UNDARRA",
	"UDD": "BERMUDA DUNES",
	"UDE": "UDEN",
	"UDI": "UBERLANDIA",
	"UDJ": "UZHGOROD",
	"UDN": "UDINE",
	"UDO": "UDOMXAY",
	"UDR": "UDAIPUR",
	"UEE": "QUEENSTOWN",
	"UEL": "QUELIMANE",
	"UEO": "KUME JIMA",
	"UES": "WAUKESHA",
	"UET": "QUETTA",
	"UFA": "UFA",
	"UGA": "BULGAN",
	"UGB": "PILOT POINT",
	"UGC": "URGENCH",
	"UGI": "UGANIK",
	"UGN": "WAUKEGAN",
	"UGO": "UIGE",
	"UGS": "UGASHIK",
	"UGU": "ZUGAPA",
	"UHE": "UHERSKE HRADISTE",
	"UHF": "UPPER HEYFORD",
	"UIB": "QUIBDO",
	"UIH": "QUINHON",
	"UII": "UTILA IS.",
	"UIK": "UST-ILIMSK",
	"UIL": "QUILLAYUTE",
	"UIN": "QUINCY",
	"UIO": "QUITO",
	"UIP": "QUIMPER",
	"UIQ": "QUINE HILL",
	"UIR": "QUIRINDI",
	"UIT": "JALUIT",
	"UJE": "UJAE ISLAND",
	"UKB": "KOBE",
	"UKI": "UKIAH",
	"UKK": "UST-KAMENOGORSK",
	"UKN": "WAUKON",
	"UKR": "MUKEIRAS",
	"UKT": "QUAKERTOWN",
	"UKU": "NUKU",
	"UKX": "UST-KUT",
	"ULA": "SAN JULIAN",
	"ULB": "ULEI",
	"ULC": "LOS CERILLOS AIRPORT-SANIAGO CHILE",
	"ULD": "ULUNDI",
	"ULE": "SULE",
	"ULG": "ULGII",
	"ULI": "ULITHI",
	"ULL": "MULL",
	"ULM": "NEW ULM",
	"ULN": "ULAN BATOR",
	"ULO": "ULAANGOM",
	"ULP": "QUILPIE",
	"ULQ": "TULUA",
	"ULS": "MULATOS",
	"ULU": "GULU",
	"ULX": "ULUSABA",
	"ULY": "ULYANOVSK",
	"ULZ": "ULIASTAI",
	"UMA": "PUNTA DE MAISI",
	"UMB": "UMNAK",
	"UMC": "UMBA",
	"UMD": "UMANAK",
	"UME": "UMEA",
	"UMI": "QUINCEMIL",
	"UMM": "SUMMIT",
	"UMR": "WOOMERA",
	"UMT": "UMIAT",
	"UMU": "UMUARAMA",
	"UMY": "SUMY",
	"UNC": "UNGUIA",
	"UND": "KUNDUZ",
	"UNE": "QACHA'S NEK",
	"UNG": "KIUNGA",
	"UNI": "UNION ISLAND",
	"UNK": "UNALAKLEET",
	"UNN": "RANONG",
	"UNR": "UNDERKHAAN",
	"UNS": "UMNAK",
	"UNT": "UNST. SHETLAND IS.",
	"UNU": "JUNEAU",
	"UOL": "BUOL",
	"UON": "MUONG SAI",
	"UOS": "SEWANEE",
	"UOX": "UNIVERSITY",
	"UPA": "PUNTA ALEGRE",
	"UPC": "PUERTO LA CRUZ",
	"UPF": "PFORHEIM",
	"UPG": "UJUNG PANDANG",
	"UPL": "UPALA",
	"UPN": "URUAPAN",
	"UPP": "UPOLU POINT",
	"UPR": "UPIARA",
	"UQE": "QUEEN",
	"URA": "URALSK",
	"URB": "URUBUPUNGA",
	"URC": "URUMQI",
	"URD": "BURG FEUERSTEIN",
	"URE": "KURESSAARE",
	"URG": "URUGUAIANA",
	"URI": "URIBE",
	"URJ": "URAJ",
	"URM": "URIMAN",
	"URO": "ROUEN",
	"URR": "URRAO",
	"URS": "KURSK",
	"URT": "SURAT THANI",
	"URY": "GURAYAT",
	"URZ": "UROOZGAN",
	"USH": "USHUAIA",
	"USI": "MABARUMA",
	"USK": "USINSK",
	"USL": "USELESS LOOP",
	"USM": "KOH SAMUI",
	"USN": "ULSAN",
	"USO": "USINO",
	"USS": "SANCTI SPIRITUS",
	"UST": "ST. AUGUSTINE",
	"USU": "BUSUANGA",
	"UTA": "UMTALI",
	"UTB": "MUTTABURRA",
	"UTC": "UTRECHT",
	"UTD": "NUTWOOD DOWNS",
	"UTE": "BUTTERWORTH",
	"UTG": "QUTHING",
	"UTH": "UDON THANI",
	"UTI": "KOUVOLA",
	"UTK": "UTIRIK",
	"UTL": "TORREMOLINOS",
	"UTN": "UPINGTON",
	"UTO": "UTOPIA CREEK",
	"UTP": "UTAPAO",
	"UTR": "UTTARADIT",
	"UTT": "UMTATA",
	"UTU": "USTUPO",
	"UTW": "QUEENSTOWN",
	"UUA": "BUGULMA",
	"UUD": "ULAN-UDE",
	"UUK": "KUPARUK",
	"UUN": "BARUUN-URT",
	"UUP": "UAUPES",
	"UUS": "YUZHNO-SAKHALINSK",
	"UUU": "MANUMU",
	"UVA": "UVALDE",
	"UVE": "OUVEA",
	"UVF": "ST. LUCIA",
	"UVL": "NEW VALLEY",
	"UVO": "UVOL",
	"UWA": "WARE",
	"UXA": "LIK CAMP",
	"UXB": "FASSBERG",
	"UXC": "HAWLEY",
	"UXD": "CHOWCHILLA",
	"UXE": "FAULKTON",
	"UXF": "ST. ANTHONY",
	"UXG": "DUTCH JOHN",
	"UXH": "KIEV",
	"UXI": "CALDER BAY",
	"UXJ": "RICHMOND",
	"UXK": "PETALUMA",
	"UXL": "CHARIOT",
	"UXM": "47-MILE MINE",
	"UXN": "PRESTONSBURG",
	"UXO": "FITCHBURG",
	"UXP": "BAY ST. LOUIS",
	"UXQ": "GILZE-RIJEN",
	"UXR": "MONUMENT VALLEY",
	"UXS": "CIMARRON",
	"UXT": "LORAIN/ELYRIA",
	"UXU": "CHEBOYGAN",
	"UXV": "ENTERPRISE",
	"UXW": "COOKEVILLE",
	"UXX": "HOT SPRINGS",
	"UXY": "CIRCLE",
	"UXZ": "GARRISON",
	"UYL": "NYALA",
	"UYN": "YULIN",
	"UZC": "UZICE",
	"UZH": "UNAYZAH",
	"UZU": "CURUZU CUATIA",
	"VAA": "VAASA",
	"VAB": "YAVARATE",
	"VAC": "VARRELBUSCH",
	"VAD": "VALDOSTA",
	"VAE": "VALLES",
	"VAF": "VALENCE",
	"VAG": "VARGINHA",
	"VAH": "VALLEGRANDE",
	"VAI": "VANIMO",
	"VAK": "CHEVAK",
	"VAL": "VALENCA",
	"VAN": "VAN",
	"VAO": "SUAVANAO",
	"VAP": "VALPARAISO",
	"VAR": "VARNA",
	"VAS": "SIVAS",
	"VAT": "VATOMANDRY",
	"VAU": "VATUKOULA",
	"VAV": "VAVA'U",
	"VAW": "VARDOE",
	"VAY": "VAN RHYNSDORP",
	"VAZ": "VAL D'ISERE",
	"VBG": "LOMPOC",
	"VBV": "VANUABALAVU",
	"VBY": "VISBY",
	"VCA": "CAN THO",
	"VCB": "VIEW COVE",
	"VCC": "VICTORIA",
	"VCD": "VICTORIA R. DOWNS",
	"VCE": "VENICE",
	"VCF": "VALCHETA",
	"VCH": "VICHADERO",
	"VCP": "SAO PAULO",
	"VCR": "CARORA",
	"VCT": "VICTORIA",
	"VCV": "VICTORVILLE",
	"VCW": "VICTORIA WEST",
	"VDA": "OVDA",
	"VDB": "FAGERNES",
	"VDC": "VITORIA DA CONQUISTA",
	"VDE": "VALVERDE",
	"VDI": "VIDALIA",
	"VDM": "VIEDMA",
	"VDP": "VALLE DE LA PASCUA",
	"VDR": "VILLA DOLORES",
	"VDS": "VADSO",
	"VDZ": "VALDEZ",
	"VEE": "VENETIE",
	"VEG": "MAIKWAK",
	"VEJ": "VEJLE",
	"VEL": "VERNAL",
	"VER": "VERACRUZ",
	"VEV": "BARAKOMA",
	"VEX": "TIOGA",
	"VEY": "VESTMANNAEYJAR",
	"VFA": "VICTORIA FALLS",
	"VGA": "VIJAYAWADA",
	"VGD": "VOLOGDA",
	"VGN": "VIGAN",
	"VGO": "VIGO",
	"VGS": "GENERAL VILLEGAS",
	"VGT": "LAS VEGAS",
	"VGV": "VILA GOUVEIA",
	"VGZ": "VILLAGARZON",
	"VHC": "SAURIMO",
	"VHM": "VILHELMINA",
	"VHN": "VAN HORN",
	"VHO": "VILA COUTINHO",
	"VHY": "VICHY",
	"VHZ": "VAHITAHI",
	"VIA": "VIANA",
	"VIB": "VILLA CONSTITUCION",
	"VIC": "VICENZA",
	"VID": "VIDIN",
	"VIE": "VIENNA",
	"VIG": "EL VIGIA",
	"VIH": "VICHY",
	"VII": "VINH CITY",
	"VIJ": "VIRGIN GORDA",
	"VIL": "DAKHLA",
	"VIN": "VINNICA",
	"VIQ": "VIQUEQUE",
	"VIS": "VISALIA",
	"VIT": "VITORIA",
	"VIU": "VIRU",
	"VIV": "VIVIGANI",
	"VIX": "VITORIA",
	"VJB": "XAI XAI",
	"VJI": "ABINGDON",
	"VJQ": "VILA JUNQUEIRO",
	"VKG": "RACH GIA",
	"VKO": "MOSCOW",
	"VKT": "VORKUTA",
	"VKW": "WEST KAVIK",
	"VLA": "VANDALIA",
	"VLC": "VALENCIA",
	"VLD": "VALDOSTA",
	"VLE": "VALLE",
	"VLG": "VILLA GESELL",
	"VLI": "PORT VILA",
	"VLK": "VOLGODONSK",
	"VLL": "VALLADOLID",
	"VLM": "VILLA MONTES",
	"VLN": "VALENCIA",
	"VLO": "VALLEJO",
	"VLP": "VILA RICA",
	"VLR": "VALLENAR",
	"VLS": "VALESDIR",
	"VLT": "VALETTA",
	"VLU": "VELIKIYE LUKI",
	"VLV": "VALERA",
	"VME": "VILLA MERCEDES",
	"VMU": "BAIMURU",
	"VNA": "SARAVANE",
	"VNC": "VENICE",
	"VND": "VANGAINDRANO",
	"VNE": "VANNES",
	"VNG": "VIENGXAY",
	"VNO": "VILNIUS",
	"VNR": "VANROOK",
	"VNS": "VARANASI",
	"VNX": "VILANCULOS",
	"VNY": "LOS ANGELES",
	"VOG": "VOLGOGRAD",
	"VOH": "VOHEMAR",
	"VOI": "VOINJAMA",
	"VOK": "CAMP DOUGLAS",
	"VOL": "VOLOS",
	"VOT": "VOTUPORANGA",
	"VOZ": "VORONEZH",
	"VPE": "ONGIVA",
	"VPN": "VOPNAFJORDUR",
	"VPS": "VALPARAISO",
	"VPY": "VILA PERY",
	"VPZ": "VALPARAISO",
	"VQS": "VIEQUES",
	"VRA": "VARAPERO",
	"VRB": "VERO BEACH",
	"VRC": "VIRAC",
	"VRE": "VREDENDAL",
	"VRK": "VARKAUS",
	"VRL": "VILA REAL",
	"VRN": "VERONA",
	"VRS": "VERSAILLES",
	"VRU": "VRYBURG",
	"VRY": "VAEROY",
	"VSA": "VILLAHERMOSA",
	"VSE": "VISEU",
	"VSF": "SPRINGFIELD",
	"VSG": "LUGANSK (VOROSHILOVGRAD)",
	"VSO": "PHUOC LONG",
	"VST": "VASTERAS",
	"VTA": "VICTORIA",
	"VTB": "VITEBSK",
	"VTE": "VIENTIANE",
	"VTF": "VATULELE",
	"VTG": "VUNG TAU",
	"VTL": "VITTEL",
	"VTN": "VALENTINE",
	"VTU": "LAS TUNAS",
	"VTZ": "VISHAKHAPATNAM",
	"VUP": "VALLEDUPAR",
	"VUS": "VELIKIJ USTYUG",
	"VVB": "MAHANORO",
	"VVC": "VILLAVICENCIO",
	"VVI": "SANTA CRUZ-VIRU VIRU",
	"VVK": "VASTERVIK",
	"VVO": "VLADIVOSTOK",
	"VVZ": "ILLIZI",
	"VWA": "HEADLAND",
	"VWB": "MYRTLE CREEK",
	"VWC": "GALLATIN",
	"VWD": "HANFORD",
	"VWE": "CHELAN",
	"VWF": "CUSTER",
	"VWG": "GARIBALDI",
	"VWH": "MIDLAND/ODESSA",
	"VWI": "CALIFORNIA CITY",
	"VWJ": "CAMERON",
	"VWK": "SARATOGA SPRINGS",
	"VWL": "BEMIDJI",
	"VWM": "WEED",
	"VWN": "PARKER",
	"VWO": "ARLINGTON",
	"VWP": "BERNARD",
	"VWQ": "LODI",
	"VWR": "INDEPENDENCE",
	"VWS": "WICHITA",
	"VWT": "AMERICUS",
	"VWU": "GLEN ULLIN",
	"VWV": "MOUNTAIN VIEW",
	"VWW": "DEER LODGE",
	"VWX": "HONDO",
	"VWY": "LANDSBERG",
	"VWZ": "GRANITE CREEK MINE",
	"VXC": "LICHINGA",
	"VXE": "SAO VICENTE",
	"VXO": "VAXJO",
	"VYD": "VRYHEID",
	"VYS": "PERU",
	"VZA": "MARVEL CREEK MINE",
	"VZB": "MITROFANIA",
	"VZC": "MUSH BAY",
	"VZD": "CAMPBELL LAGOON",
	"VZE": "FRAZER LAKE",
	"VZF": "AFOGNAK STRAITS",
	"VZG": "MOFFET BAY",
	"VZH": "MINEOLA",
	"VZI": "BRUNDIDGE",
	"VZJ": "PAYETTE",
	"VZL": "PETER PAN SEAFOODS",
	"VZM": "JENSENS STRIP",
	"VZN": "WILDMAN CREEK",
	"VZO": "STEPOVAK",
	"VZP": "ANIAKCHAK",
	"VZQ": "PUALE BAY",
	"VZR": "KATMAI BAY",
	"VZS": "KASHVIK BAY",
	"VZT": "CINDER MOUNTAIN",
	"VZU": "HAGEMEISTER ISLAND",
	"VZV": "CAPE PEIRCE",
	"VZW": "CAPE GREIG",
	"VZX": "KASBA LAKE",
	"VZY": "KATMAI LODGE",
	"VZZ": "SOUTH ST. PAUL",
	"WAA": "WALES",
	"WAB": "WABAG",
	"WAC": "WACA",
	"WAD": "ANDRIAMENA",
	"WAE": "WADI-AD-DAWASIR",
	"WAF": "WANA",
	"WAG": "WANGANUI",
	"WAH": "WAHPETON",
	"WAI": "ANTSOHIHY",
	"WAJ": "ALMA",
	"WAK": "ANKAZOABO",
	"WAL": "CHINCOTEAGUE",
	"WAM": "AMBATONDRAZAKA",
	"WAN": "WAVERNEY",
	"WAO": "WABO",
	"WAP": "ALTO PALENA",
	"WAQ": "ANTSALOVA",
	"WAR": "WARIS",
	"WAS": "WASHINGTON",
	"WAT": "WATERFORD",
	"WAU": "WAUCHOPE",
	"WAV": "WAVE HILL",
	"WAW": "WARSAW",
	"WAX": "WARROAD",
	"WAZ": "WARWICK",
	"WBA": "WARRABER ISLAND",
	"WBB": "STEBBINS",
	"WBD": "BEFANDRIANA",
	"WBE": "BEALANANA",
	"WBI": "BOULDER",
	"WBM": "WAPENAMANDA",
	"WBN": "WOBURN",
	"WBO": "BEROROHA",
	"WBQ": "BEAVER",
	"WBR": "BIG RAPIDS",
	"WBU": "BOULDER",
	"WCA": "CASTRO",
	"WCH": "CHAITEN",
	"WCR": "CHANDALAR",
	"WDA": "WADI AIN",
	"WDB": "DEEP BAY",
	"WDG": "ENID",
	"WDH": "WINDHOEK",
	"WDI": "WONDAI",
	"WDN": "WALDRON ISLAND",
	"WDR": "WINDER",
	"WEA": "WEATHERFORD",
	"WEB": "WEBEQUIE",
	"WED": "WEDAU",
	"WEF": "WEIFANG",
	"WEH": "WEIHAI",
	"WEI": "WEIPA",
	"WEL": "WELKOM",
	"WEM": "WEST MALLING",
	"WEP": "WEAM",
	"WES": "WEASUA",
	"WET": "WAGETHE",
	"WEW": "WEE WAA",
	"WEX": "WEXFORD",
	"WFB": "KETCHIKAN",
	"WFI": "FIANARANTSOA",
	"WFK": "MADAWASKA/FT.KENT",
	"WGA": "WAGGA-WAGGA",
	"WGB": "BAHAWALNAGAR",
	"WGC": "WARANGAL",
	"WGE": "WALGETT",
	"WGN": "WAITANGI",
	"WGO": "WINCHESTER",
	"WGP": "WAINGAPU",
	"WGT": "WANGARATTA",
	"WGU": "WAGAU",
	"WGY": "WAGNY",
	"WHD": "HYDER",
	"WHF": "WADI HALFA",
	"WHH": "BOULDER",
	"WHK": "WHAKATANE",
	"WHL": "WELSHPOOL",
	"WHO": "FRANZ JOSEF GLACIER",
	"WHR": "VAIL",
	"WHS": "WHALSAY",
	"WHT": "WHARTON",
	"WHU": "WUHU",
	"WIC": "WICK",
	"WID": "WILDENRATH",
	"WIK": "SURFDALE",
	"WIL": "NAIROBI",
	"WIN": "WINTON",
	"WIO": "WILCANNIA",
	"WIR": "WAIROA",
	"WIT": "WITTENOOM GORGE",
	"WIU": "WITU",
	"WJA": "WOJA",
	"WJF": "PALMDALE/LANCASTER",
	"WJR": "WAJIR",
	"WKA": "WANAKA",
	"WKB": "WARRACKNABEAL",
	"WKI": "HWANGE",
	"WKJ": "WAKKANAI",
	"WKK": "ALEKNAGIK",
	"WKL": "WAIKOLOA",
	"WKN": "WAKUNAI",
	"WKR": "WALKER'S CAY",
	"WLA": "WALLAL",
	"WLB": "LABOUCHERE BAY",
	"WLC": "WALCHA",
	"WLD": "WINFIELD",
	"WLG": "WELLINGTON",
	"WLH": "WALAHA",
	"WLK": "SELAWIK",
	"WLL": "WOLLOGORANG",
	"WLM": "WALTHAM",
	"WLN": "LITTLE NAUKATI",
	"WLO": "WATERLOO",
	"WLR": "LORING",
	"WLS": "ALLIS IS.",
	"WLW": "WILLOWS",
	"WMA": "MANDRITSARA",
	"WMB": "WARRNAMBOOL",
	"WMC": "WINNEMUCCA",
	"WMD": "MANDABE",
	"WME": "MOUNT KEITH",
	"WMH": "MOUNTAIN HOME",
	"WMK": "MEYERS CHUCK",
	"WML": "MALAIMBANDY",
	"WMN": "MAROANTSETRA",
	"WMO": "WHITE MOUNTAIN",
	"WMR": "MANANARA",
	"WMV": "MADIROVALO",
	"WMX": "WAMENA",
	"WNA": "NAPAKIAK",
	"WNC": "TUXEKAN ISLAND",
	"WND": "WINDARRA",
	"WNE": "WORA NA YE",
	"WNN": "WUNNUMMIN LAKE",
	"WNP": "NAGA",
	"WNR": "WINDORAH",
	"WNS": "NAWAB SHAH",
	"WNU": "WANUMA",
	"WNY": "WYNYARD",
	"WNZ": "WENZHOU",
	"WOA": "WONENARA",
	"WOD": "WOOD RIVER",
	"WOE": "WOENSDRECHT AB",
	"WOG": "WOODGREEN",
	"WOI": "WOLOGISSI",
	"WOK": "WONKEN",
	"WOL": "WOLLONGONG",
	"WON": "WONDOOLA",
	"WOO": "WOODCHOPPER",
	"WOT": "WONAN",
	"WOW": "WILLOW",
	"WPA": "PUERTO AYSEN",
	"WPB": "PORT BERGE",
	"WPC": "PINCHER CREEK",
	"WPK": "WROTHAM PARK",
	"WPL": "POWELL LAKE",
	"WPM": "WIPIM",
	"WPO": "PAONIA",
	"WPR": "PORVENIR",
	"WPU": "PUERTO WILLIAMS",
	"WQA": "OVID",
	"WQB": "MANDAN",
	"WQC": "HAMILTON",
	"WQD": "TULELAKE",
	"WQE": "DINSMORE",
	"WQF": "RIGBY",
	"WQG": "HESPERIA",
	"WQH": "DORRIS",
	"WQI": "CONNELL",
	"WQJ": "PEARD BAY",
	"WQK": "TOWNSEND",
	"WQL": "BLUFF",
	"WQM": "LAKE CHARLES",
	"WQN": "CAMPBELLSVILLE",
	"WQO": "ROSAMOND",
	"WQP": "OCEANO",
	"WQQ": "FAIRBURN",
	"WQS": "POPLAR",
	"WQT": "BRIDGER",
	"WQU": "PORTALES",
	"WQV": "ROUND ISLAND",
	"WQW": "DAVID RIVER",
	"WQX": "KIAVAK",
	"WQY": "PORT WAKEFIELD",
	"WQZ": "CHIGNIK LAKE",
	"WRA": "WARDER",
	"WRB": "MACON",
	"WRE": "WHANGAREI",
	"WRG": "WRANGELL",
	"WRH": "WRENCH CREEK",
	"WRI": "FT. DIX",
	"WRL": "WORLAND",
	"WRO": "WROCLAW",
	"WRW": "WARRAWAGINE",
	"WRY": "WESTRAY",
	"WSA": "WASUA",
	"WSB": "STEAMBOAT BAY",
	"WSG": "WASHINGTON",
	"WSH": "SHIRLEY",
	"WSJ": "SAN JUAN",
	"WSM": "WISEMAN",
	"WSN": "SOUTH NAKNEK",
	"WSO": "WASHABO",
	"WSP": "WASPAM",
	"WSR": "WASIOR",
	"WST": "WESTERLY",
	"WSU": "WASU",
	"WSX": "WESTSOUND",
	"WSY": "SHUTE HARBOUR/WHITSUNDAY",
	"WSZ": "WESTPORT",
	"WTA": "TAMBOHORANO",
	"WTC": "NEW YORK",
	"WTD": "WEST END",
	"WTE": "WOTJE",
	"WTK": "NOATAK",
	"WTL": "TUNTATULIAK",
	"WTO": "WOTHO",
	"WTP": "WOITAPE",
	"WTR": "WHITERIVER",
	"WTS": "TSIROANOMANDIDY",
	"WTT": "WANTOAT",
	"WTZ": "WHITIANGA",
	"WUD": "WUDINNA",
	"WUG": "WAU",
	"WUH": "WUHAN",
	"WUN": "WILUNA",
	"WUS": "WUYISHAN",
	"WUU": "WAU",
	"WUV": "WUVULU ISLAND",
	"WUX": "WUXI",
	"WUZ": "WUZHOU",
	"WVB": "WALVIS BAY",
	"WVI": "WATSONVILLE",
	"WVK": "MANAKARA",
	"WVL": "WATERVILLE",
	"WVN": "WILHELMSHAVEN",
	"WWA": "WASILLA",
	"WWD": "CAPE MAY",
	"WWI": "WOODIE WOODIE",
	"WWK": "WEWAK",
	"WWP": "WHALE PASS",
	"WWR": "WOODWARD",
	"WWT": "NEWTOK",
	"WWY": "WEST WYALONG",
	"WXN": "WANXIAN",
	"WYA": "WHYALLA",
	"WYB": "YES BAY",
	"WYE": "YENGEMA",
	"WYN": "WYNDHAM",
	"WYS": "WEST YELLOWSTONE",
	"WYU": "HERRERA AIRPORT- SANTO DOM. DOM REP",
	"WZY": "NASSAU",
	"XAL": "ALAMOS",
	"XAP": "CHAPECO",
	"XAR": "ARIBINDA",
	"XAU": "SAUL",
	"XAV": "XAVANTINA",
	"XAW": "CAPREOL",
	"XAY": "XAYABURY",
	"XAZ": "CAMPBELLTON",
	"XBB": "BLUBBER BAY",
	"XBE": "BEARSKIN LAKE",
	"XBG": "BOGANDE",
	"XBJ": "BIRJAND",
	"XBL": "BUNO BEDELLE",
	"XBN": "BINIGUNI",
	"XBO": "BOULSA",
	"XBR": "BROCKVILLE",
	"XBW": "KILLINEQ",
	"XCA": "CANARANA",
	"XCC": "COCHRANE",
	"XCH": "CHRISTMAS ISLAND",
	"XCI": "CHAMBORD",
	"XCK": "ST. DIE",
	"XCL": "CLUFF LAKE",
	"XCM": "CHATHAM",
	"XCN": "CORON",
	"XCO": "COLAC",
	"XDB": "LILLE-EUROPE RR",
	"XDE": "DIEBOUGOU",
	"XDJ": "DJIBO",
	"XDL": "CHANDLER",
	"XDM": "DRUMMONDVILLE",
	"XDO": "GRANDE-RIVIERE",
	"XDT": "PARIS-D GAULE RR",
	"XDU": "HERVEY",
	"XEE": "LAC EDOUARD",
	"XEH": "LADYSMITH",
	"XEJ": "LANGFORD",
	"XEK": "MELVILLE",
	"XEL": "NEW CARLISLE",
	"XEM": "NEW RICHMOND",
	"XEN": "XINGCHENG",
	"XES": "LAKE GENEVA",
	"XEY": "NEWCASTLE",
	"XFA": "LILLE-FLANDRE RR",
	"XFC": "FREDERICTON JUNCTION",
	"XFD": "STRATFORD",
	"XFE": "PARENT",
	"XFG": "PERCE",
	"XFI": "PORT-DANIEL",
	"XFK": "SENNETERRE",
	"XFL": "SHAWINIGAN",
	"XFM": "SHAWNIGAN",
	"XFN": "XIANGFAN",
	"XFO": "TASCHEREAU",
	"XFQ": "WEYMONT",
	"XFS": "ALEXANDRIA",
	"XFV": "BRANTFORD",
	"XFW": "FINKENWERDER",
	"XFY": "SAINTE-FOY",
	"XFZ": "CHARNY",
	"XGA": "GAOUA",
	"XGG": "GOROM-GOROM",
	"XGJ": "COBOURG",
	"XGK": "COTEAU",
	"XGL": "GRANVILLE LAKE",
	"XGN": "XANGONGO",
	"XGR": "KANGIQSUALUJJUAQ",
	"XGW": "GANANOQUE",
	"XGY": "GRIMSBY",
	"XHM": "GEORGETOWN",
	"XHS": "CHEMAINUS",
	"XIA": "GUELPH",
	"XIB": "INGERSOLL",
	"XIC": "XICHANG",
	"XID": "MAXVILLE",
	"XIE": "XIENG LOM",
	"XIF": "NAPANEE",
	"XIG": "XINGUARA",
	"XII": "PRESCOTT",
	"XIL": "XILINHOT",
	"XIM": "SAINT-HYACINTHE",
	"XIN": "XINGNING",
	"XIO": "ST. MARYS",
	"XIP": "WOODSTOCK",
	"XIQ": "XIQUE XIQUE",
	"XIY": "XI AN-XIANYANG",
	"XJL": "JOLIETTE",
	"XJQ": "JONQUIERE",
	"XKA": "KANTCHARI",
	"XKH": "XIENG KHOUANG",
	"XKO": "KEMANO",
	"XKS": "KASABONIKA",
	"XKV": "SACKVILLE",
	"XKY": "KAYA",
	"XLB": "LAC BROCHET",
	"XLC": "DRUMMONDVILLE",
	"XLF": "LEAF BAY",
	"XLJ": "QUEBEC STATION",
	"XLK": "LEVIS",
	"XLM": "SAINT-LAMBERT",
	"XLO": "LONG XUYEN",
	"XLP": "MATAPEDIA",
	"XLQ": "GUILDWOOD",
	"XLS": "ST. LOUIS",
	"XLU": "LEO",
	"XLV": "NIAGARA FALLS",
	"XLW": "LEMWERDER",
	"XLY": "ALDERSHOT",
	"XLZ": "TRURO",
	"XMA": "MARAMAG",
	"XMB": "M'BAHIAKRO",
	"XMC": "MALLACOOTA",
	"XMD": "MADISON",
	"XMG": "MAHENDRANAGAR",
	"XMH": "MANIHI",
	"XMI": "MASASI",
	"XML": "MINLATON",
	"XMN": "XIAMEN",
	"XMP": "MACMILLAN PASS",
	"XMR": "CAPE CANAVERAL",
	"XMS": "MACAS",
	"XMY": "YAM ISLAND",
	"XNA": "Fayetteville",
	"XNG": "QUANG NGAI",
	"XNN": "XINING",
	"XNR": "AABENRAA",
	"XNS": "ANSAN",
	"XNT": "XINGTAI",
	"XNU": "NOUNA",
	"XOK": "OAKVILLE",
	"XON": "CARLETON",
	"XOX": "CHEONAN",
	"XPA": "PAMA",
	"XPB": "KANGIRSUK",
	"XPH": "PORT HOPE",
	"XPK": "PUKATAWAGAN",
	"XPL": "COMAYAGUA",
	"XPN": "BRAMPTON",
	"XPP": "POPLAR RIVER",
	"XPR": "PINE RIDGE",
	"XPU": "WEST KUPARUK",
	"XPX": "POINTE-AUX-TREMBLES",
	"XQP": "QUEPOS",
	"XQU": "QUALICUM",
	"XRA": "GRAASTEN",
	"XRP": "RIVIERE-A-PIERRE",
	"XRR": "ROSS RIVER",
	"XRY": "JEREZ DE LA FRONTERA",
	"XSC": "SOUTH CAICOS",
	"XSD": "TONOPAH",
	"XSE": "SEBBA",
	"XSH": "ST. PIERRE DES CORPS",
	"XSI": "SOUTH INDIAN LAKE",
	"XSM": "ST. MARY'S",
	"XSO": "SIOCON",
	"XSP": "SINGAPORE-SELETAR",
	"XTG": "THARGOMINDAH",
	"XTL": "TADOULE LAKE",
	"XTO": "TAROOM",
	"XTR": "TARA",
	"XTY": "STRATHROY",
	"XUZ": "XUZHOU",
	"XVL": "VINH LONG",
	"XVV": "BELLEVILLE",
	"XWA": "GRANITE POINT",
	"XWB": "COLLINSVILLE MINE",
	"XWC": "YANTARNI BAY",
	"XWD": "EMMEN",
	"XWE": "COPALIS",
	"XWF": "JEROME",
	"XWG": "KING KHALID MIL. CITY",
	"XWH": "AZTEC",
	"XWI": "ST. IGNATIUS",
	"XWJ": "DEXTER",
	"XWK": "PORT ORCHARD",
	"XWL": "BLACKFOOT",
	"XWM": "RIYADH",
	"XWN": "JUPITER",
	"XWO": "KIMBALL",
	"XWP": "AL KHARJ",
	"XWQ": "IWAKUNI",
	"XWR": "OOSIK SPLIT",
	"XWS": "OLIKTOK",
	"XWT": "NUSHAGAK BAY",
	"XWU": "MARION",
	"XWV": "MONROE",
	"XWW": "DRIGGS",
	"XWY": "MCGREGOR",
	"XWZ": "OKANOGAN",
	"XXB": "WOODFORD",
	"XXD": "CLARENDON",
	"XXE": "LAKEWOOD",
	"XXG": "CAMBRIDGE",
	"XXH": "HELICOPTER SCENIC",
	"XXM": "COVELO",
	"XXN": "WILLITS",
	"XXO": "HURRICANE",
	"XXP": "LAKEPORT",
	"XXQ": "FRANKFORT",
	"XXR": "MARIPOSA",
	"XXS": "CRESWELL",
	"XXT": "POLSON",
	"XXU": "ASHLAND",
	"XXV": "DECATUR SHORES",
	"XXW": "LAKE CITY",
	"XXX": "MENA",
	"XXY": "FORTUNA",
	"XXZ": "ZUNI PUEBLO",
	"XYA": "YANDINA",
	"XYD": "LYON-PRT DU RR",
	"XYE": "YE",
	"XYL": "LYON-PERACHE RR",
	"XYR": "YELLOW RIVER",
	"XYV": "LYON-SATOLAS RR",
	"XZA": "ZABRE",
	"XZB": "CASSELMAN",
	"XZC": "GLENCOE",
	"XZK": "AMHERST",
	"XZL": "EDMONTON",
	"XZP": "PASS (GENERIC)",
	"XZR": "RAIL (GENERIC)",
	"XZY": "ALZEY",
	"YAA": "ANAHIM LAKE",
	"YAB": "ARCTIC BAY",
	"YAC": "CAT LAKE",
	"YAD": "MOOSE LAKE",
	"YAF": "ASBESTOS HILL",
	"YAG": "FORT FRANCES",
	"YAH": "LAGRANDE 4",
	"YAI": "CHILLAN",
	"YAJ": "LYALL HARBOUR",
	"YAK": "YAKUTAT",
	"YAL": "ALERT BAY",
	"YAM": "SAULT STE. MARIE",
	"YAN": "YANGAMBI",
	"YAO": "YAOUNDE",
	"YAP": "YAP",
	"YAQ": "MAPLE BAY",
	"YAR": "LAGRANDE 3",
	"YAS": "YASAWA",
	"YAT": "ATTAWAPISKAT",
	"YAV": "MINER'S BAY",
	"YAX": "ANGLING LAKE",
	"YAY": "ST. ANTHONY",
	"YAZ": "TOFINO",
	"YBA": "BANFF",
	"YBB": "PELLY BAY",
	"YBC": "BAIE COMEAU",
	"YBD": "NEW WESTMINSTER",
	"YBE": "URANIUM CITY",
	"YBF": "BAMFIELD",
	"YBG": "SAGUENAY",
	"YBH": "BULL HARBOUR",
	"YBI": "BLACK TICKLE",
	"YBJ": "BAIE JOHAN BEETZ",
	"YBK": "BAKER LAKE",
	"YBL": "CAMPBELL RIVER",
	"YBM": "BRONSON CREEK",
	"YBN": "BORDEN",
	"YBO": "BOBQUINN LAKE",
	"YBP": "YIBIN",
	"YBQ": "TELEGRAPH HARBOUR",
	"YBR": "BRANDON",
	"YBS": "SUMMER BEAVER",
	"YBT": "LAC DU BROCHET",
	"YBV": "BERENS RIVER",
	"YBW": "BEDWELL HARBOR",
	"YBX": "BLANC SABLON",
	"YBY": "BONNYVILLE",
	"YBZ": "TORONTO",
	"YCA": "COURTENAY",
	"YCB": "CAMBRIDGE BAY",
	"YCC": "CORNWALL",
	"YCD": "NANAIMO",
	"YCF": "CORTES BAY",
	"YCG": "CASTLEGAR",
	"YCH": "CHATHAM",
	"YCI": "CARIBOU ISLAND",
	"YCJ": "CAPE ST. JAMES",
	"YCK": "COLVILLE LAKE",
	"YCL": "CHARLO",
	"YCM": "ST. CATHARINES",
	"YCN": "COCHRANE",
	"YCO": "COPPERMINE",
	"YCP": "CO-OP POINT",
	"YCQ": "CHETWYND",
	"YCR": "CROSS LAKE",
	"YCS": "CHESTERFIELD INLET",
	"YCT": "CORONATION",
	"YCU": "CULLATON LAKE",
	"YCV": "CARTIERVILLE",
	"YCW": "CHILLIWACK",
	"YCX": "GAGETOWN",
	"YCY": "CLYDE RIVER",
	"YCZ": "CRESTON",
	"YDA": "DAWSON CITY",
	"YDB": "BURWASH LANDINGS",
	"YDC": "DRAYTON VALLEY",
	"YDE": "PARADISE RIVER",
	"YDF": "DEER LAKE",
	"YDG": "DIGBY",
	"YDH": "DANIELS HARBOUR",
	"YDI": "DAVIS INLET",
	"YDJ": "HATCHET LAKE",
	"YDK": "MAIN DUCK ISLAND",
	"YDL": "DEASE LAKE",
	"YDN": "DAUPHIN",
	"YDO": "DOLBEAU",
	"YDP": "NAIN",
	"YDQ": "DAWSON CREEK",
	"YDR": "BROADVIEW",
	"YDS": "DESOLATION SOUND",
	"YDT": "VANCOUVER",
	"YDU": "KASBA LAKE",
	"YDV": "BLOODVEIN",
	"YDW": "OBRE LAKE",
	"YDX": "DOC CREEK",
	"YEA": "EDMONTON",
	"YEC": "YECHON",
	"YED": "EDMONTON",
	"YEG": "EDMONTON",
	"YEI": "ENNADAI LAKE",
	"YEK": "ESKIMO POINT",
	"YEL": "ELLIOT LAKE",
	"YEM": "MANITOWANING",
	"YEN": "ESTEVAN",
	"YEO": "YEOVILTON",
	"YEP": "ESTEVAN POINT",
	"YEQ": "YENKIS",
	"YER": "FORT SEVERN",
	"YET": "EDSON",
	"YEU": "EUREKA",
	"YEV": "INUVIK",
	"YEY": "AMOS",
	"YFA": "FORT ALBANY",
	"YFB": "FROBISHER BAY",
	"YFC": "FREDERICTON",
	"YFE": "FORESTVILLE",
	"YFG": "FONTANGES",
	"YFH": "FORT HOPE",
	"YFL": "FORT RELIANCE",
	"YFO": "FLIN FLON",
	"YFR": "FT. RESOLUTION",
	"YFS": "FT. SIMPSON",
	"YFV": "FT. VERMILLION",
	"YFX": "FOX HARBOUR",
	"YGA": "GAGNON",
	"YGB": "GILLIES BAY",
	"YGC": "GRANDE CACHE",
	"YGE": "GORGE HARBOR",
	"YGG": "GANGES HARBOR",
	"YGH": "FT. GOOD HOPE",
	"YGJ": "YONAGO",
	"YGK": "KINGSTON",
	"YGL": "LA GRANDE",
	"YGM": "GIMLI",
	"YGN": "GREENWAY SOUND",
	"YGO": "GODS NARROWS",
	"YGP": "GASPE",
	"YGQ": "GERALDTON",
	"YGR": "ILES DE MADELEINE",
	"YGS": "GERMANSEN",
	"YGT": "IGLOOLIK",
	"YGV": "HAVRE ST. PIERRE",
	"YGW": "GREAT WHALE",
	"YGX": "GILLAM",
	"YGY": "DECEPTION BAY",
	"YGZ": "GRISE FIORD",
	"YHA": "PORT HOPE SIMPSON",
	"YHB": "HUDSON BAY",
	"YHC": "HAKAI PASS",
	"YHD": "DRYDEN",
	"YHE": "HOPE",
	"YHF": "HEARST",
	"YHG": "CHARLOTTETOWN",
	"YHH": "CAMPBELL RIVER",
	"YHI": "HOLMAN ISLAND",
	"YHK": "GJOA HAVEN",
	"YHM": "HAMILTON",
	"YHN": "HORNEPAYNE",
	"YHO": "HOPEDALE",
	"YHP": "POPLAR HILL",
	"YHR": "HARRINGTON HARBOUR",
	"YHS": "SECHELT",
	"YHT": "HAINES JUNCTION",
	"YHU": "MONTREAL",
	"YHY": "HAY RIVER",
	"YHZ": "HALIFAX",
	"YIB": "ATIKOKAN",
	"YIC": "ISACHSEN",
	"YIF": "ST. AUGUSTIN",
	"YIG": "BIG BAY MARINA",
	"YIH": "YICHANG",
	"YIK": "IVUGIVIK",
	"YIN": "YINING",
	"YIO": "POND INLET",
	"YIP": "DETROIT",
	"YIV": "ISLAND LAKE",
	"YIW": "YIWU",
	"YIX": "PORT RADIUM",
	"YJA": "JASPER",
	"YJF": "FORT LIARD",
	"YJN": "ST. JEAN",
	"YJO": "JOHNNY MOUNTAIN",
	"YJP": "JASPER-HINTON",
	"YJT": "STEPHENVILLE",
	"YKA": "KAMLOOPS",
	"YKB": "KANGIQSUALUJJUAQ",
	"YKC": "COLLINS BAY",
	"YKD": "KINCARDINE",
	"YKE": "KNEE LAKE",
	"YKF": "KITCHENER",
	"YKG": "KANGIRSUK",
	"YKJ": "KEY LAKE",
	"YKK": "KITKATLA",
	"YKL": "SCHEFFERVILLE",
	"YKM": "YAKIMA",
	"YKN": "YANKTON",
	"YKQ": "RUPERT HOUSE",
	"YKS": "KAYES",
	"YKT": "KLEMTU",
	"YKU": "FORT GEORGE",
	"YKX": "KIRKLAND LAKE",
	"YKY": "KINDERSLEY",
	"YKZ": "TORONTO",
	"YLA": "LANGARA",
	"YLB": "LAC BICHE",
	"YLC": "LAKE HARBOUR",
	"YLD": "CHAPLEAU",
	"YLE": "LAC LA MARTRE",
	"YLF": "LAFORGES",
	"YLG": "YALGOO",
	"YLH": "LANSDOWNE",
	"YLI": "YLIVIESKA",
	"YLJ": "MEADOW LAKE",
	"YLL": "LLOYDMINSTER",
	"YLM": "CLINTON CREEK",
	"YLN": "YILAN",
	"YLP": "MINGAN",
	"YLQ": "LA TUQUE",
	"YLR": "LEAF RAPIDS",
	"YLS": "LEBEL-SUR-QUEVILLON",
	"YLT": "ALERT",
	"YLW": "KELOWNA",
	"YLY": "LYTTON",
	"YMA": "MAYO",
	"YMB": "MERRITT",
	"YMC": "MARICOURT AIRSTRIP",
	"YME": "MATANE",
	"YMF": "MONTAGUE HARBOR",
	"YMG": "MANITOUWADGE",
	"YMH": "MARY'S HARBOUR",
	"YMI": "MINAKI",
	"YMJ": "MOOSE JAW",
	"YML": "MURRAY BAY",
	"YMM": "FT. MCMURRAY",
	"YMN": "MAKKOVIK",
	"YMO": "MOOSONEE",
	"YMP": "PORT MCNEIL",
	"YMQ": "MONTREAL",
	"YMS": "YURIMAGUAS",
	"YMT": "CHIBOUGAMAU",
	"YMW": "MANIWAKI",
	"YMX": "MONTREAL",
	"YMY": "MONTREAL",
	"YNA": "NATASHQUAN",
	"YNB": "YANBU",
	"YNC": "PAINT HILLS",
	"YND": "GATINEAU/HULL",
	"YNE": "NORWAY HOUSE",
	"YNG": "YOUNGSTOWN",
	"YNH": "HUDSON HOPE",
	"YNI": "NITCHEQUON",
	"YNJ": "YANJI",
	"YNK": "NOOTKA SOUND",
	"YNL": "POINTS NORTH LANDING",
	"YNM": "MATAGAMI",
	"YNO": "NORTH SPIRIT LAKE",
	"YNR": "ARNES",
	"YNS": "NEMISCAU",
	"YNT": "YANTAI",
	"YNZ": "YANCHENG",
	"YOC": "OLD CROW",
	"YOD": "COLD LAKE",
	"YOE": "FALHER",
	"YOG": "OGOKI",
	"YOH": "OXFORD HOUSE",
	"YOJ": "HIGH LEVEL",
	"YOK": "YOKOHAMA",
	"YOL": "YOLA",
	"YOO": "OSHAWA",
	"YOP": "RAINBOW LAKE",
	"YOS": "OWEN SOUND",
	"YOW": "OTTAWA",
	"YPA": "PRINCE ALBERT",
	"YPB": "PORT ALBERNI",
	"YPC": "PAULATUK",
	"YPD": "PARRY SOUND",
	"YPE": "PEACE RIVER",
	"YPF": "ESQUIMALT",
	"YPG": "PORTAGE LA PRAIRIE",
	"YPH": "PORT HARRISON",
	"YPI": "PORT SIMPSON",
	"YPJ": "AUPALUK",
	"YPL": "PICKLE LAKE",
	"YPM": "PIKANGIKUM",
	"YPN": "PORT MENIER",
	"YPO": "PEAWANUCK",
	"YPP": "PINE POINT",
	"YPQ": "PETERBOROUGH",
	"YPR": "PRINCE RUPERT",
	"YPS": "PORT HAWKESBURY",
	"YPT": "PENDER HARBOR",
	"YPV": "PEAWANUK",
	"YPW": "POWELL RIVER",
	"YPX": "POVUNGNITUK",
	"YPY": "FT. CHIPEWYAN",
	"YPZ": "BURNS LAKE",
	"YQA": "MUSKOKA",
	"YQB": "QUEBEC",
	"YQC": "QUAQTAQ",
	"YQD": "THE PAS",
	"YQE": "KIMBERLEY",
	"YQF": "RED DEER",
	"YQG": "WINDSOR",
	"YQH": "WATSON LAKE",
	"YQI": "YARMOUTH",
	"YQK": "KENORA",
	"YQL": "LETHBRIDGE",
	"YQM": "MONCTON",
	"YQN": "NAKINA",
	"YQQ": "COMOX",
	"YQR": "REGINA",
	"YQS": "ST. THOMAS",
	"YQT": "THUNDER BAY",
	"YQU": "GRANDE PRAIRIE",
	"YQV": "YORKTON",
	"YQW": "NORTH BATTLEFORD",
	"YQX": "GANDER",
	"YQY": "SYDNEY",
	"YQZ": "QUESNEL",
	"YRA": "RAE LAKES",
	"YRB": "RESOLUTE",
	"YRD": "DEAN RIVER",
	"YRE": "RESOLUTION ISLAND",
	"YRF": "CARTWRIGHT",
	"YRG": "RIGOLET",
	"YRI": "RIVIERE DU LOUP",
	"YRJ": "ROBERVAL",
	"YRL": "RED LAKE",
	"YRM": "ROCKY MOUNTAIN HOUSE",
	"YRN": "RIVERS INLET",
	"YRQ": "THREE RIVERS",
	"YRR": "STUART ISLAND",
	"YRS": "RED SUCKER LAKE",
	"YRT": "RANKIN INLET",
	"YRV": "REVELSTOKE",
	"YSA": "SABLE ISLAND",
	"YSB": "SUDBURY",
	"YSC": "SHERBROOKE",
	"YSD": "SUFFIELD",
	"YSE": "SWAN RIVER",
	"YSF": "STONEY RAPIDS",
	"YSG": "SNOWDRIFT",
	"YSH": "SMITH FALLS",
	"YSI": "SANS SOUCI",
	"YSJ": "SAINT JOHN",
	"YSK": "SANIKILUAQ",
	"YSL": "ST. LEONARD",
	"YSM": "FT. SMITH",
	"YSN": "SALMON ARM",
	"YSO": "POSTVILLE",
	"YSP": "MARATHON",
	"YSR": "NANISIVIK",
	"YST": "ST. THERESE PT.",
	"YSU": "SUMMERSIDE",
	"YSV": "SAGLEK",
	"YSX": "SHEARWATER",
	"YSY": "SACHS HARBOUR",
	"YSZ": "SQUIRREL COVE",
	"YTA": "PEMBROKE",
	"YTB": "HARTLEY BAY",
	"YTC": "STURDEE",
	"YTD": "THICKET PORTAGE",
	"YTE": "CAPE DORSET",
	"YTF": "ALMA",
	"YTG": "SULLIVAN BAY",
	"YTH": "THOMPSON",
	"YTJ": "TERRACE BAY",
	"YTK": "TULUGAK",
	"YTL": "BIG TROUT LAKE",
	"YTN": "RIVIERE AU TONNERRE",
	"YTO": "TORONTO",
	"YTP": "TOFINO",
	"YTQ": "TASIUJUAQ",
	"YTR": "TRENTON",
	"YTS": "TIMMINS",
	"YTT": "TISDALE",
	"YTX": "TELEGRAPH CREEK",
	"YTZ": "TORONTO",
	"YUA": "YUANMOU",
	"YUB": "TUKTOYAKTUK",
	"YUD": "UMIUJAQ",
	"YUE": "YUENDUMU",
	"YUF": "PELLY BAY",
	"YUJ": "LADY FRANKLIN",
	"YUL": "MONTREAL",
	"YUM": "YUMA",
	"YUN": "JOHNSON POINT",
	"YUT": "REPULSE BAY",
	"YUX": "HALL BEACH",
	"YUY": "ROUYN - NORANDA",
	"YVA": "MORONI",
	"YVB": "BONAVENTURE",
	"YVC": "LAC LA RONGE",
	"YVD": "YEVA",
	"YVE": "VERNON",
	"YVG": "VERMILION",
	"YVM": "BROUGHTON",
	"YVN": "CAPE DYER",
	"YVO": "VAL D'OR",
	"YVP": "FT. CHIMO",
	"YVQ": "NORMAN WELLS",
	"YVR": "VANCOUVER",
	"YVT": "BUFFALO NARROWS",
	"YVV": "WIARTON",
	"YVZ": "DEER LAKE",
	"YWA": "PETAWAWA",
	"YWB": "KANGIQSUJUAQ",
	"YWF": "HALIFAX",
	"YWG": "WINNIPEG",
	"YWH": "VICTORIA",
	"YWJ": "FORT FRANKLIN",
	"YWK": "WABUSH",
	"YWL": "WILLIAMS LAKE",
	"YWM": "WILLIAMS HARBOUR",
	"YWN": "WINISK",
	"YWP": "WEBEQUIE",
	"YWR": "WHITE RIVER",
	"YWS": "WHISTLER",
	"YWY": "WRIGLEY",
	"YXC": "CRANBROOK",
	"YXD": "EDMONTON",
	"YXE": "SASKATOON",
	"YXF": "SNAKE RIVER",
	"YXH": "MEDICINE HAT",
	"YXI": "KILLALOE",
	"YXJ": "FT. ST. JOHN",
	"YXK": "RIMOUSKI",
	"YXL": "SIOUX LOOKOUT",
	"YXN": "WHALE COVE",
	"YXP": "PANGNIRTUNG",
	"YXQ": "BEAVER CREEK",
	"YXR": "EARLTON",
	"YXS": "PRINCE GEORGE",
	"YXT": "TERRACE",
	"YXU": "LONDON",
	"YXX": "ABBOTSFORD",
	"YXY": "WHITEHORSE",
	"YXZ": "WAWA",
	"YYA": "BIG BAY YACHT CLUB",
	"YYB": "NORTH BAY",
	"YYC": "CALGARY",
	"YYD": "SMITHERS",
	"YYE": "FT. NELSON",
	"YYF": "PENTICTON",
	"YYG": "CHARLOTTETOWN",
	"YYH": "SPENCE BAY",
	"YYI": "RIVERS",
	"YYJ": "VICTORIA",
	"YYL": "LYNN LAKE",
	"YYM": "COWLEY",
	"YYN": "SWIFT CURRENT",
	"YYQ": "CHURCHILL",
	"YYR": "GOOSE BAY",
	"YYT": "ST. JOHNS",
	"YYU": "KAPUSKASING",
	"YYW": "ARMSTRONG",
	"YYY": "MONT JOLI",
	"YYZ": "TORONTO",
	"YZA": "ASHCROFT",
	"YZC": "BEATTON RIVER",
	"YZE": "GORE BAY",
	"YZF": "YELLOWKNIFE",
	"YZG": "SUGLUK",
	"YZH": "SLAVE LAKE",
	"YZL": "LIARD RIVER",
	"YZM": "BUCHANS",
	"YZP": "SANDSPIT",
	"YZR": "SARNIA",
	"YZS": "CORAL HARBOUR",
	"YZT": "PORT HARDY",
	"YZU": "WHITECOURT",
	"YZV": "SEPT-ILES",
	"YZW": "TESLIN",
	"YZX": "GREENWOOD",
	"ZAA": "ALICE ARM",
	"ZAC": "YORK LANDING",
	"ZAD": "ZADAR",
	"ZAG": "ZAGREB",
	"ZAH": "ZAHEDAN",
	"ZAJ": "ZARANJ",
	"ZAK": "CHIUSA/KLAUSEN",
	"ZAL": "VALDIVIA",
	"ZAM": "ZAMBOANGA",
	"ZAO": "CAHORS",
	"ZAR": "ZARIA",
	"ZAT": "ZHAOTONG",
	"ZAV": "AVEIRO",
	"ZAW": "NYKOBING MORS",
	"ZAZ": "ZARAGOZA",
	"ZBE": "ZABREH",
	"ZBF": "BATHURST",
	"ZBJ": "SKIVE",
	"ZBK": "ZABLJAK",
	"ZBL": "BILOELA",
	"ZBM": "BROMONT",
	"ZBN": "BOZEN",
	"ZBO": "BOWEN",
	"ZBP": "BALTIMORE",
	"ZBQ": "RANDERS",
	"ZBR": "CHAH-BAHAR",
	"ZBS": "MESA",
	"ZBT": "AARHUS-BUS STN",
	"ZBU": "MALLING",
	"ZBV": "BEAVER CREEK",
	"ZBX": "BRANSON",
	"ZBY": "SAYABOURY",
	"ZCL": "ZACATECAS",
	"ZCO": "TEMUCO",
	"ZDF": "NABLUS",
	"ZDJ": "BERNE-RR STATION",
	"ZDL": "BRIG",
	"ZDM": "RAMALLAH",
	"ZDN": "BRNO-BUS STN",
	"ZDY": "GAZA",
	"ZEC": "SECUNDA",
	"ZEG": "SENGGO",
	"ZEL": "BELLA BELLA",
	"ZEM": "EAST MAIN",
	"ZEN": "ZENAG",
	"ZER": "ZERO",
	"ZFA": "FARO",
	"ZFB": "OLD FORT BAY",
	"ZFD": "FOND DU LAC",
	"ZFL": "SOUTH TROUT LAKE",
	"ZFM": "FT. MCPHERSON",
	"ZFN": "FT. NORMAN",
	"ZFT": "FT. LAUDERDALE",
	"ZFV": "FORT SEVERN",
	"ZFW": "FAIRVIEW",
	"ZGC": "LANZHOU-ZHONGCHUAN",
	"ZGF": "GRAND FORKS",
	"ZGI": "GODS RIVER",
	"ZGJ": "BRUGGE",
	"ZGK": "LEUVEN",
	"ZGL": "SOUTH GALWAY",
	"ZGM": "NGOMA",
	"ZGP": "MECHELEN",
	"ZGQ": "TOURNAI",
	"ZGR": "LITTLE GRAND RAPIDS",
	"ZGS": "GETHSEMANI",
	"ZGU": "GAUA",
	"ZGV": "WAVRE",
	"ZHA": "ZHANGJIANG",
	"ZHC": "PHILADELPHIA PA-NO PHIL RR",
	"ZHM": "SHAMSHERNAGAR",
	"ZHO": "HOUSTON",
	"ZHP": "HIGH PRAIRIE",
	"ZHZ": "HALLE",
	"ZIC": "VICTORIA",
	"ZIG": "ZIGUINCHOR",
	"ZIH": "ZIHUATANEJO",
	"ZIN": "INTERLAKEN OST",
	"ZJG": "JENPEG",
	"ZJN": "SWAN RIVER",
	"ZJO": "SAN JOSE",
	"ZJQ": "MORGES",
	"ZKB": "KASABA BAY",
	"ZKE": "KASHECHEWAN",
	"ZKG": "KEGASKA",
	"ZKH": "ST. MORITZ OLP",
	"ZKK": "SCHWYZ",
	"ZKL": "STEENKOOL",
	"ZKM": "SETTE CAMA",
	"ZKO": "SIERRE SIDERS",
	"ZKP": "KASOMPE",
	"ZKW": "WETZIKON",
	"ZLG": "LA GUERA",
	"ZLO": "MANZANILLO",
	"ZLT": "LA TABATIERE",
	"ZMB": "HAMBURG HBF",
	"ZMD": "SENA MADUREIRA",
	"ZME": "PATERSON",
	"ZMH": "108 MILE HOUSE",
	"ZMI": "NAPLES-MERGLNA RR",
	"ZML": "MILWAUKEE",
	"ZMM": "ZAMORA",
	"ZMR": "MERAN",
	"ZMS": "FLORENCE-NOVELLA RR",
	"ZMT": "MASSET",
	"ZMU": "MUNICH HBF",
	"ZNA": "NANAIMO",
	"ZNC": "NYAC",
	"ZND": "ZINDER",
	"ZNE": "NEWMAN",
	"ZNG": "NEGGINAN",
	"ZNQ": "INGOLSTADT",
	"ZNU": "NAMU",
	"ZNZ": "ZANZIBAR",
	"ZOF": "OCEAN FALLS",
	"ZOM": "ZOMBA",
	"ZOS": "OSORNO",
	"ZP1": "LOS ANGELES",
	"ZP2": "NEW YORK",
	"ZP5": "SAN FRANCISCO",
	"ZP6": "LOS ANGELES",
	"ZP7": "MULHOUSE/BASEL-BSL ARPT",
	"ZP8": "BASEL/MULHOUSE-MLH ARPT",
	"ZPA": "JOHNSTOWN",
	"ZPB": "SACHIGO LAKE",
	"ZPC": "PUCON",
	"ZPH": "ZEPHYRHILLS",
	"ZPL": "PITTSBURGH",
	"ZPO": "PINE HOUSE",
	"ZQN": "QUEENSTOWN",
	"ZQS": "QUEEN CHARLOTTE IS",
	"ZRA": "ATLANTIC CITY",
	"ZRB": "FRANKFURT HBF",
	"ZRC": "SAN PEDRO DE ALCANTARA",
	"ZRD": "BUFFALO",
	"ZRF": "ROCKFORD",
	"ZRG": "BRATISLAVA-BUS STN",
	"ZRH": "ZURICH",
	"ZRI": "SERUI",
	"ZRJ": "ROUND LAKE",
	"ZRL": "ALBANY",
	"ZRM": "SARMI",
	"ZRP": "NEWARK",
	"ZRR": "ARCTIC RED RIVER",
	"ZSA": "SAN SALVADOR",
	"ZSE": "SAINT-PIERRE DE LA REUNION",
	"ZSF": "SPRINGFIELD",
	"ZSH": "SANTA FE",
	"ZSJ": "SANDY LAKE",
	"ZSM": "SANTA CLARA",
	"ZSP": "ST.PAUL",
	"ZSS": "SASSANDRA",
	"ZST": "STEWART",
	"ZSV": "ST. LOUIS",
	"ZSW": "PRINCE RUPERT",
	"ZSZ": "SPIEZ",
	"ZTA": "TUREIA",
	"ZTB": "TETE A LA BALEINE",
	"ZTH": "ZAKINTHOS",
	"ZTJ": "INTERLAKEN WEST",
	"ZTL": "TELLURIDE",
	"ZTM": "SHAMATTAWA",
	"ZTR": "ZHITOMIR",
	"ZTS": "TAHSIS",
	"ZTY": "PHILADELPHIA",
	"ZUA": "MIAMI",
	"ZUC": "IGNACE",
	"ZUD": "ANCUD",
	"ZUE": "ZUENOULA",
	"ZUF": "GRAND RAPIDS",
	"ZUG": "GR/H.PT/WIN SAL",
	"ZUH": "ZHUHAI",
	"ZUJ": "JACKSONVILLE",
	"ZUK": "SAVANNAH",
	"ZUL": "ZILFI",
	"ZUM": "CHURCHILL FALLS",
	"ZUN": "CHICAGO",
	"ZUP": "HARRISBURG",
	"ZUR": "RALEIGH/DURHAM",
	"ZUS": "SPRINGFIELD",
	"ZUT": "DETROIT",
	"ZUW": "LOS ANGELES",
	"ZUX": "OXNARD",
	"ZVA": "MIANDRIVAZO",
	"ZVG": "SPRINGVALE",
	"ZVK": "SAVANNAKHET",
	"ZVR": "HANOVER HBF",
	"ZWA": "ANDAPA",
	"ZWI": "PHILADELPHIA",
	"ZWL": "WOLLASTON LAKE",
	"ZWP": "WEST PALM BEACH",
	"ZWS": "STUTTGART-MAIN",
	"ZWU": "WASHINGTON",
	"ZXA": "ROTA",
	"ZXB": "ALBUQUERQUE",
	"ZXC": "GASQUET",
	"ZXD": "SAN ISIDRO",
	"ZXE": "SCULTHORPE",
	"ZXF": "ILLINOIS CREEK",
	"ZXH": "CHOMONDELY SOUND",
	"ZXI": "COAL BAY",
	"ZXJ": "HAPPY HARBOUR",
	"ZXK": "LANCASTER COVE",
	"ZXL": "REID COVE",
	"ZXM": "SALTERY COVE",
	"ZXN": "CLOVER BAY",
	"ZXO": "EL CAPITAN",
	"ZXP": "SALMON BAY LAKE",
	"ZXR": "CARMEL VALLEY",
	"ZXS": "ELK RIVER",
	"ZXT": "CLOVERDALE",
	"ZXU": "NORTH KINGSTOWN",
	"ZXV": "NAPPANEE",
	"ZXW": "PARADISE",
	"ZXX": "KENNEWICK",
	"ZXY": "DELTA",
	"ZXZ": "WATERVILLE",
	"ZYH": "THE HAGUE-SPOOR RR",
	"ZYI": "ZUNYI",
	"ZYL": "SYLHET",
	"ZYO": "ROOSENDAAL",
	"ZYP": "NEW YORK",
	"ZYR": "BRUSSELS OLP",
	"ZYT": "MAASTRICHT-RR STN",
	"ZYU": "UTRECHT-CEN RR",
	"ZZA": "HARTFORD",
	"ZZB": "BALTIMORE",
	"ZZC": "NEW CARROLLTON",
	"ZZD": "NEWPORT NEWS",
	"ZZK": "NORFOLK",
	"ZZL": "RICHMOND",
	"ZZM": "SCHENECTADY",
	"ZZP": "POZAREVAC",
	"ZZR": "PRINCETON",
	"ZZT": "TRENTON",
	"ZZU": "MZUZU",
	"ZZV": "ZANESVILLE",
}

func GetRegionFromIATACode(code string) string {
	return iataCode[strings.ToUpper(strings.TrimSpace(code))]
}

// Lookup returns the region/city name corresponding to the 3-letter IATA code.
func Lookup(code string) (string, bool) {
	val, ok := iataCode[strings.ToUpper(strings.TrimSpace(code))]
	return val, ok
}
